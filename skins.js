(function () {
  ...
  var SIBLING_COMPONENTS = ['lampaconline', 'onlyonline', 'online'];
  ...
  function siblingInstalled() {
    return !!(window.nova_online_plugin || window.online_plugin);
  }
  ...
  var COLOR_PRESETS = {
    red: { solid: '#ef4444', soft: 'rgba(239,68,68,.28)', track: 'rgba(127,29,29,.72)', glow: 'rgba(239,68,68,.72)' },
    crimson: { solid: '#dc2626', soft: 'rgba(220,38,38,.28)', track: 'rgba(127,29,29,.78)', glow: 'rgba(220,38,38,.72)' },
    orange: { solid: '#f97316', soft: 'rgba(249,115,22,.28)', track: 'rgba(124,45,18,.76)', glow: 'rgba(249,115,22,.72)' },
    blue: { solid: '#3b82f6', soft: 'rgba(59,130,246,.28)', track: 'rgba(30,58,138,.76)', glow: 'rgba(59,130,246,.72)' },
    green: { solid: '#22c55e', soft: 'rgba(34,197,94,.28)', track: 'rgba(20,83,45,.76)', glow: 'rgba(34,197,94,.72)' },
    purple: { solid: '#a855f7', soft: 'rgba(168,85,247,.28)', track: 'rgba(91,45,145,.72)', glow: 'rgba(168,85,247,.72)' }
  };

  function colorPreset(key) {
    return COLOR_PRESETS[String(key || '')] || COLOR_PRESETS.red;
  }

  function episodeScale() {
    var value = String(get('nova_skin_episode_size', 'large') || 'large');
    if (value === 'normal') return { number: '2em', title: '1.25em', grid: '1.2em' };
    if (value === 'xlarge') return { number: '2.8em', title: '1.45em', grid: '1.6em' };
    return { number: '2.4em', title: '1.35em', grid: '1.4em' };
  }

  function applyTheme() {
    try {
      var style = document.getElementById('nova-skin-theme-css');
      if (!style) {
        style = document.createElement('style');
        style.id = 'nova-skin-theme-css';
        (document.body || document.head).appendChild(style);
      }

      var button = colorPreset(get('nova_skin_button_color', 'red'));
      var modal = colorPreset(get('nova_skin_modal_color', 'red'));
      var progress = colorPreset(get('nova_skin_progress_color', 'red'));
      var episode = episodeScale();

      style.textContent = [
        '.nova-skin-root .nova-btn{background:' + button.soft + '!important;color:#fff!important;border:1px solid ' + button.solid + '!important;box-shadow:0 .18em .7em ' + button.glow + '!important}',
        '.nova-skin-root .nova-btn--ghost{background:' + button.soft + '!important}',
        '.nova-skin-root .nova-btn.focus,.nova-skin-root .nova-btn--main,.nova-skin-root .nova-btn--main.focus{background:' + button.solid + '!important;color:#fff!important;box-shadow:0 .3em 1em ' + button.glow + '!important}',
        'body.nova-focus-ring .nova-skin-root .nova-btn--main{background:' + button.soft + '!important;color:#fff!important;border-color:' + button.solid + '!important}',
        'body .selectbox,body .selectbox__body,body .modal,body .modal__body,body .modal__content,body .popup,body .popup__body{background:linear-gradient(145deg,' + modal.soft + ',rgba(15,16,24,.98))!important;border:1px solid ' + modal.solid + '!important;box-shadow:0 .5em 2em ' + modal.glow + '!important}',
        'body .selectbox__item,body .selectbox .selector,body .modal .selector,body .popup .selector,body .menu .selector{border-radius:.45em}',
        'body .selectbox__item.focus,body .selectbox .selector.focus,body .modal .selector.focus,body .popup .selector.focus,body .menu .selector.focus{background:' + modal.solid + '!important;color:#fff!important;box-shadow:0 .2em .8em ' + modal.glow + '!important}',
        '.nova-skin-root .nova-hero__progress{height:1.15em!important;background:' + progress.track + '!important;box-shadow:0 0 .35em ' + progress.glow + '!important}',
        '.nova-skin-root .nova-hero__progress .time-line>div,.nova-skin-root .nova-card__line .time-line>div,.nova-skin-root .nova-card__line--body .time-line>div,.nova-skin-root .nova-loading__bar>div{background:' + progress.solid + '!important;box-shadow:0 0 .35em ' + progress.glow + '!important}',
        '.nova-skin-root.nova-serial .nova-card__num{font-size:' + episode.number + '!important;font-weight:800!important}',
        '.nova-skin-root.nova-serial .nova-card__title{font-size:' + episode.title + '!important;font-weight:700!important}',
        '.nova-skin-root.nova-serial.nova__list--grid .nova-card__num{font-size:' + episode.grid + '!important}',
        '@media screen and (max-width:580px){.nova-skin-root.nova-serial .nova-card__num{font-size:' + episode.number + '!important}.nova-skin-root.nova-serial .nova-card__title{font-size:1.15em!important}}'
      ].join('');
    } catch (e) {}
  }
  ...
  var OWN = {
    ...
    nova_skin_action: { ru: 'Действие', uk: 'Дія', en: 'Action' },
     nova_skin_calendar: { ru: 'Календарь', uk: 'Календар', en: 'Calendar' },
     nova_skin_calendar_title: { ru: 'Выход новых серий', uk: 'Вихід нових серій', en: 'Upcoming episodes' },
     nova_skin_calendar_loading: { ru: 'Загрузка календаря…', uk: 'Завантаження календаря…', en: 'Loading calendar…' },
     nova_skin_calendar_empty: { ru: 'Будущих серий пока нет', uk: 'Майбутніх серій поки немає', en: 'No upcoming episodes' },
    ...
    nova_skin_set_button_color: { ru: 'Цвет кнопок', uk: 'Колір кнопок', en: 'Button color' },
    nova_skin_modal_color: { ru: 'Цвет модальных окон', uk: 'Колір модальних вікон', en: 'Modal window color' },
    nova_skin_progress_color: { ru: 'Цвет полосы просмотра', uk: 'Колір смуги перегляду', en: 'Progress bar color' },
    nova_skin_color_descr: { ru: 'Выберите цвет акцента интерфейса', uk: 'Оберіть колір акценту інтерфейсу', en: 'Choose the interface accent color' },
    nova_skin_color_red: { ru: 'Красный', uk: 'Червоний', en: 'Red' },
    nova_skin_color_crimson: { ru: 'Малиновый', uk: 'Малиновий', en: 'Crimson' },
    nova_skin_color_orange: { ru: 'Оранжевый', uk: 'Помаранчевий', en: 'Orange' },
    nova_skin_color_blue: { ru: 'Синий', uk: 'Синій', en: 'Blue' },
    nova_skin_color_green: { ru: 'Зелёный', uk: 'Зелений', en: 'Green' },
    nova_skin_color_purple: { ru: 'Фиолетовый', uk: 'Фіолетовий', en: 'Purple' },
    nova_skin_set_episode_size: { ru: 'Размер серий', uk: 'Розмір серій', en: 'Episode size' },
    nova_skin_set_episode_size_descr: { ru: 'Сделать номера и названия серий заметнее', uk: 'Зробити номери та назви серій помітнішими', en: 'Make episode numbers and titles more visible' },
    nova_skin_episode_normal: { ru: 'Обычный', uk: 'Звичайний', en: 'Normal' },
    nova_skin_episode_large: { ru: 'Большой', uk: 'Великий', en: 'Large' },
    nova_skin_episode_xlarge: { ru: 'Очень большой', uk: 'Дуже великий', en: 'Extra large' }
  };
  ...
  var ICON = {
    ...
    list: '<svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor"><rect x="3" y="4" width="18" height="4" rx="1.4"></rect><rect x="3" y="10" width="18" height="4" rx="1.4"></rect><rect x="3" y="16" width="18" height="4" rx="1.4"></rect></svg>',
     ...
     calendar: '<svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4.5" width="18" height="16" rx="2"></rect><path d="M7 2.5v4M17 2.5v4M3 9h18M7 13h.01M12 13h.01M17 13h.01M7 17h.01M12 17h.01" stroke-linecap="round"></path></svg>'
  };
  ...
  function addCSS() {
    if (!document.getElementById('nova-skin-css')) {
      var style = document.createElement('style');
      style.id = 'nova-skin-css';
      style.textContent = SKIN_CSS + EXTRA_CSS + CARD_CSS + FOCUS_CSS + FULL_CSS + FADE_CSS + MARK_CSS + ACCENT_CSS;
      (document.body || document.head).appendChild(style);
    }
    applyTheme();
  }
  ...
  function heroKey(key) {
    return key === 'hero' || key === 'hero-next' || key === 'hero-calendar';
  }
  ...
  function rebind() {
    ...
    var calendar = ui.root.find('[data-nova-focus="hero-calendar"]').first();
    if (calendar.length) {
      ui.calendar = calendar;
      bindCalendar(calendar.off('hover:enter hover:focus hover:long'));
    }
  ...
  function calendarButton() {
    if (!ui.calendar || !ui.calendar.length) {
      ui.calendar = $('<div class="nova-btn nova-btn--ghost selector" data-nova-focus="hero-calendar">' +
        ICON.calendar + '<span class="nova-btn__label"></span></div>');
      ui.calendar.find('.nova-btn__label').text(text('nova_calendar', 'nova_skin_calendar'));
      bindCalendar(ui.calendar);
    }
    return ui.calendar;
  }
  ...
  function calendarSeasonNumbers() {
    var out = [];
    var add = function (value) {
      var number = parseInt(value, 10);
      if (isNaN(number) || number < 1 || out.indexOf(number) !== -1) return;
      out.push(number);
    };

    try {
      (movie.seasons || []).forEach(function (season) {
        add(season && (season.season_number !== undefined ? season.season_number : season.number));
      });
    } catch (e) {}

    var total = parseInt(movie && movie.number_of_seasons, 10) || 0;
    if (!out.length && total) {
      for (var i = 1; i <= Math.min(total, 40); i++) add(i);
    }

    try {
      if (movie.next_episode_to_air) add(movie.next_episode_to_air.season_number);
    } catch (e) {}
  ...
  function upcomingEpisodes(done) {
    var id = tmdbId();
    if (!id || !movie) return done([]);

    var cache = cached('nova_skin_calendar_cache', 500, {});
    var saved = cache[String(id)];
    if (saved && saved.t && Date.now() - saved.t < 1800000 && Object.prototype.toString.call(saved.list) === '[object Array]') {
      return done(saved.list);
    }

    var today = calendarToday();
    var list = [];
    var known = {};
    var add = function (episode, fallbackSeason) {
      if (!episode || !episode.air_date || String(episode.air_date).slice(0, 10) < today) return;
      var season = parseInt(episode.season_number, 10) || parseInt(fallbackSeason, 10) || 0;
      var number = parseInt(episode.episode_number, 10) || 0;
      var key = season + ':' + number + ':' + String(episode.air_date).slice(0, 10);
      if (known[key]) return;
      known[key] = true;
      list.push({
        season: season,
        episode: number,
        date: String(episode.air_date).slice(0, 10),
        name: episode.name || ''
      });
    };

    try { add(movie.next_episode_to_air, movie.next_episode_to_air && movie.next_episode_to_air.season_number); } catch (e) {}

    var api = null;
    try { api = Lampa.Api.sources.tmdb; } catch (e) { api = null; }
    var seasons = calendarSeasonNumbers();
    if (!api || typeof api.get !== 'function' || !seasons.length) {
      list.sort(function (a, b) { return a.date.localeCompare(b.date) || a.season - b.season || a.episode - b.episode; });
      cache[String(id)] = { t: Date.now(), list: list };
      save('nova_skin_calendar_cache', cache);
      return done(list);
    }
    ...
    var next = function () {
      if (at >= seasons.length) return finish();
      var season = seasons[at++];
      try {
        api.get('tv/' + id + '/season/' + season, {}, function (data) {
          (data && data.episodes || []).forEach(function (episode) { add(episode, season); });
          next();
        }, function () {
          next();
        });
  ...
  function calendarItems(list) {
    if (!list.length) return [{ title: text('nova_calendar_empty', 'nova_skin_calendar_empty') }];
    return list.map(function (episode) {
      var season = episode.season ? 'S' + episode.season + ' ' : '';
      var number = episode.episode ? 'E' + episode.episode + ' · ' : '';
      return { title: season + number + episode.date + (episode.name ? ' · ' + episode.name : '') };
    });
  }

  function showCalendar() {
    if (!serial || !movie) return;

    var active;
    try { active = Lampa.Controller.enabled().name; } catch (e) { active = 'content'; }
    var token = ++calendar_request;
    calendar_open = true;
    var close = function () {
      calendar_open = false;
      calendar_request++;
    ...
    var loading = {
      title: text('nova_calendar_loading', 'nova_skin_calendar_loading'),
      onBack: close,
      onSelect: close
    };
    ...
    try {
      Lampa.Select.show({
        title: text('nova_calendar_title', 'nova_skin_calendar_title'),
        items: [loading],
        onBack: close,
        onSelect: close
      });
    ...
    upcomingEpisodes(function (list) {
      if (!calendar_open || token !== calendar_request) return;
      try { if (typeof Lampa.Select.hide === 'function') Lampa.Select.hide(); } catch (e) {}
      try {
        Lampa.Select.show({
          title: text('nova_calendar_title', 'nova_skin_calendar_title'),
          items: calendarItems(list),
          onBack: close,
          onSelect: close
        });
      } catch (e) {
  ...
  function bindCalendar(node) {
    return bind(node, showCalendar);
  }
  ...
  function buildHero() {
    ...
    if (!ui.hero) {
      ...
      ui.hero = $('<div class="nova-hero">' +
        ...
        '<div class="nova-hero__body">' +
         (withArt ? '<div class="nova-hero__title"></div><div class="nova-hero__meta"></div>' : '') +
    ...
    if (!target) {
      button.detach();
      nextButton().detach();
      if (serial) ui.hero.find('.nova-hero__actions').append(calendarButton());
      else calendarButton().detach();
      heroStatic(withArt);
      return null;
    }
    ...
    if (serial) {
      if (next && started) next_button.after(calendarButton());
      else button.after(calendarButton());
    } else calendarButton().detach();
  ...
  function draw() {
    ...
    root.toggleClass('nova-serial', serial);
  ...
  function settings() {
    try {
      ...
      var colorValues = {
        red: label('nova_skin_color_red'),
        crimson: label('nova_skin_color_crimson'),
        orange: label('nova_skin_color_orange'),
        blue: label('nova_skin_color_blue'),
        green: label('nova_skin_color_green'),
        purple: label('nova_skin_color_purple')
      };

      Lampa.SettingsApi.addParam({
        component: 'nova_skin',
        param: {
          name: 'nova_skin_button_color',
          type: 'select',
          values: colorValues,
          default: 'red'
        },
        field: {
          name: label('nova_skin_set_button_color'),
          description: label('nova_skin_color_descr')
        },
      ...
      Lampa.SettingsApi.addParam({
        component: 'nova_skin',
        param: {
          name: 'nova_skin_modal_color',
          type: 'select',
          values: colorValues,
          default: 'red'
        },
        field: {
          name: label('nova_skin_modal_color'),
          description: label('nova_skin_color_descr')
        },
        onChange: function () { applyTheme(); }
      });
      ...
      Lampa.SettingsApi.addParam({
        component: 'nova_skin',
        param: {
          name: 'nova_skin_progress_color',
          type: 'select',
          values: colorValues,
          default: 'red'
        },
        field: {
          name: label('nova_skin_progress_color'),
          description: label('nova_skin_color_descr')
        },
        onChange: function () { applyTheme(); }
      });
      ...
      Lampa.SettingsApi.addParam({
        component: 'nova_skin',
        param: {
          name: 'nova_skin_episode_size',
          type: 'select',
          values: {
            normal: label('nova_skin_episode_normal'),
            large: label('nova_skin_episode_large'),
            xlarge: label('nova_skin_episode_xlarge')
          },
          default: 'large'
        },
        field: {
          name: label('nova_skin_set_episode_size'),
          description: label('nova_skin_set_episode_size_descr')
        },
        onChange: function () { applyTheme(); }
      });
    } catch (e) {}
  ...
  /* Default red accent: dynamic theme rules below can replace it from settings. */
  var ACCENT_CSS = [
    '.nova-skin-root .nova-hero__progress{height:1.15em!important;background:rgba(127,29,29,.72)!important;box-shadow:0 0 .35em rgba(239,68,68,.55)}',
    '.nova-skin-root .nova-hero__progress .time-line>div{background:#ef4444!important;box-shadow:0 0 .35em rgba(239,68,68,.9)}',
    '.nova-skin-root .nova-card__line .time-line>div,.nova-skin-root .nova-card__line--body .time-line>div{background:#ef4444!important;box-shadow:0 0 .2em rgba(239,68,68,.7)}',
    '.nova-skin-root .nova-loading__bar>div{background:#ef4444!important}',
    '.nova-skin-root .nova-btn.focus,.nova-skin-root .nova-btn--main.focus{background:#ef4444!important;color:#fff!important;box-shadow:0 .25em 1em rgba(239,68,68,.5)!important}',
    'body.nova-focus-ring .nova-skin-root .nova-btn--main{background:rgba(239,68,68,.28);color:#fff;border-color:#ef4444}',
    'body.nova-skin-fade .nova-skin-root .nova-hero__progress{left:0!important;right:0!important;bottom:0!important;border-radius:0!important}',
    '@media screen and (max-width:580px){.nova-skin-root .nova-hero__progress{height:.85em!important}}'
  ].join('');
