...
(function () {
    ...
    function setupSettings() {
        ...
        add({
            ...
            field: { name: 'Нижняя полоса пилюль (4K / HDR / DV / Atmos / REMUX)' }
        });
    ...
    function renderPosterOverlay(cardEl, movie, data) {
        ...
        // Quality
        if (data.resolution === '8K')      slot.strip.appendChild(makePosterPill('8K',  'ULTRA HD', 'res4k'));
        else if (data.resolution === '4K') slot.strip.appendChild(makePosterPill('4K',  'ULTRA HD', 'res4k'));
        else if (data.resolution === '2K') slot.strip.appendChild(makePosterPill('2K',  'QHD',      'res2k'));
        else if (data.resolution === 'FHD')slot.strip.appendChild(makePosterPill('FHD', '1080p',    'resfhd'));
        else if (data.resolution === 'HD') slot.strip.appendChild(makePosterPill('HD',  '720p',     'reshd'));

        // HDR / Dolby Vision (одна объединённая логика, как в твоём примере)
        if (data.hdr && !data.dolbyVision) {
        ...
        // Audio
        if (data.audio && data.audio.atmos)  slot.strip.appendChild(makePosterPill('DOLBY', 'ATMOS', 'atmos'));
        else if (data.audio && data.audio.truehd) slot.strip.appendChild(makePosterPill('TRUE', 'HD', 'truehd'));
        else if (data.audio && data.audio.dtsHd)  slot.strip.appendChild(makePosterPill('DTS', 'HD', 'dts'));
        else if (data.audio && data.audio.dts)    slot.strip.appendChild(makePosterPill('DTS', null, 'dts'));

        // Source
        if (data.special && data.special.remux)        slot.strip.appendChild(makePosterPill('REMUX', null, 'remux'));
        else if (data.special && data.special.bluray)  slot.strip.appendChild(makePosterPill('BLURAY', null, 'bluray'));
        else if (data.special && data.special.web)     slot.strip.appendChild(makePosterPill('WEB-DL', null, 'web'));
    }
