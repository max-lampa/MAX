(function () {
  ...
  window.nova_torrents_version = '7-android-tv';
  ...
  function dpadCode(event) {
    ...
    if (key === 'arrowup' || code === 19) return 'up';
    if (key === 'arrowdown' || code === 20) return 'down';
    if (key === 'arrowleft' || code === 21) return 'left';
    if (key === 'arrowright' || code === 22) return 'right';
    if (key === 'enter' || key === 'select' || code === 23 || code === 66) return 'enter';
    if (key === 'escape' || key === 'back' || key === 'browserback' || code === 4 || code === 27) return 'back';
    return '';
  }
  ...
  function installDpadNavigation() {
    try { $(document).off('keydown.nova_tor_nav'); } catch (e) {}
    try {
      $(document).on('keydown.nova_tor_nav', function (event) {
