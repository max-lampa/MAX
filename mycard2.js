(function () {
    ...
    const CACHE_LIFETIME = 1000 * 60 * 60 * 24; // 24 часа

    // Встроенные иконки рейтингов (SVG)
    const ICONS = {
        tmdb: '<svg viewBox="0 0 24 24" width="24" height="24"><circle cx="12" cy="12" r="10" fill="#01b4e4"/><text x="12" y="16" font-size="12" text-anchor="middle" fill="white" font-weight="bold">T</text></svg>',
        cub: '<svg viewBox="0 0 24 24" width="24" height="24"><circle cx="12" cy="12" r="10" fill="#f5a623"/><text x="12" y="16" font-size="12" text-anchor="middle" fill="white" font-weight="bold">C</text></svg>'
    };

    // Встроенные иконки качества (SVG с текстом)
    const QUALITY_ICONS = {
        '4K': '<svg viewBox="0 0 40 20" width="40" height="20"><rect width="40" height="20" rx="4" fill="#e67e22"/><text x="20" y="14" font-size="12" text-anchor="middle" fill="white" font-weight="bold">4K</text></svg>',
        'FULL HD': '<svg viewBox="0 0 60 20" width="60" height="20"><rect width="60" height="20" rx="4" fill="#27ae60"/><text x="30" y="14" font-size="12" text-anchor="middle" fill="white" font-weight="bold">FULL HD</text></svg>',
        'HD': '<svg viewBox="0 0 40 20" width="40" height="20"><rect width="40" height="20" rx="4" fill="#2980b9"/><text x="20" y="14" font-size="12" text-anchor="middle" fill="white" font-weight="bold">HD</text></svg>',
        'HDR': '<svg viewBox="0 0 40 20" width="40" height="20"><rect width="40" height="20" rx="4" fill="#8e44ad"/><text x="20" y="14" font-size="12" text-anchor="middle" fill="white" font-weight="bold">HDR</text></svg>',
        'Dolby Vision': '<svg viewBox="0 0 70 20" width="70" height="20"><rect width="70" height="20" rx="4" fill="#9b59b6"/><text x="35" y="14" font-size="12" text-anchor="middle" fill="white" font-weight="bold">Dolby Vision</text></svg>',
        'UKR': '<svg viewBox="0 0 40 20" width="40" height="20"><rect width="40" height="20" rx="4" fill="#3498db"/><text x="20" y="14" font-size="12" text-anchor="middle" fill="white" font-weight="bold">UKR</text></svg>'
    };
    ...
    function addSettings() {
        ...
        Lampa.SettingsApi.addParam({
            ...
            field: { name: 'Качество логотипа' }, onChange: applySettings
        });

        Lampa.SettingsApi.addParam({
            component: PLUGIN_ID,
            param: { name: 'cas_logo_scale', type: 'select', values: { '70':'70%','80':'80%','90':'90%','100':'100%','110':'110%','120':'120%' }, default: '100' },
            field: { name: 'Размер логотипа' }, onChange: applySettings
        });

        Lampa.SettingsApi.addParam({
            component: PLUGIN_ID,
            param: { name: 'cas_meta_size', type: 'select', values: { '1.2': 'Маленький', '1.3': 'Стандартный', '1.4': 'Увеличенный', '1.5': 'Большой' }, default: '1.3' },
            field: { name: 'Размер шрифта' }, onChange: applySettings
        });

        Lampa.SettingsApi.addParam({
            component: PLUGIN_ID,
            param: { name: 'cas_blocks_gap', type: 'select', values: { '15':'Тесно','20':'Стандарт','25':'Просторно' }, default: '20' },
            field: { name: 'Отступы между блоками' }, onChange: applySettings
        });

        Lampa.SettingsApi.addParam({
            component: PLUGIN_ID,
            param: { name: 'cas_bg_animation', type: 'trigger', default: true },
            field: { name: 'Анимация фона (Ken Burns)' }, onChange: applySettings
        });

        Lampa.SettingsApi.addParam({ component: PLUGIN_ID, param: { name: 'cas_slideshow_enabled', type: 'trigger', default: true }, field: { name: 'Слайд-шоу фона' } });
        Lampa.SettingsApi.addParam({ component: PLUGIN_ID, param: { name: 'cas_show_studios', type: 'trigger', default: true }, field: { name: 'Показывать студии' } });
        Lampa.SettingsApi.addParam({ component: PLUGIN_ID, param: { name: 'cas_show_quality', type: 'trigger', default: true }, field: { name: 'Показывать качество' } });
        Lampa.SettingsApi.addParam({ component: PLUGIN_ID, param: { name: 'cas_show_rating', type: 'trigger', default: true }, field: { name: 'Показывать рейтинги' } });
        Lampa.SettingsApi.addParam({ component: PLUGIN_ID, param: { name: 'cas_show_description', type: 'trigger', default: true }, field: { name: 'Описание фильма' } });
...
.cas-quality-item svg { height: 20px; width: auto; margin-right: 2px; }
.cas-rate-item svg { height: 20px; width: 20px; margin-right: 4px; }
            ...
            if (event.type === 'complite') {  
                ...
                if (data && data.id) {
                    ...
                    const processImages = (res) => {
                        ...
                        if (Lampa.Storage.get('cas_slideshow_enabled') && res.backdrops?.length > 1) {
                            ...
                            window.casBgInterval = setInterval(() => {
                                const bg = render.find('.full-start__background img');
                    ...
                    if (Lampa.Storage.get('cas_show_rating')) {
                        ...
                        if (tmdbV > 0) ratesHtml += `<div class="cas-rate-item">${ICONS.tmdb} <span style="color:${getRatingColor(tmdbV)}">${tmdbV}</span></div>`;
                        ...
                        if (event.data.reactions && event.data.reactions.result) {
                            ...
                            if (cnt >= 5) {
                                ...
                                ratesHtml += `<div class="cas-rate-item">${ICONS.cub} <span style="color:${getRatingColor(cubV)}">${cubV}</span></div>`;
                            }
                        }
                    }
                    ...
                    if (Lampa.Storage.get('cas_show_quality') && Lampa.Parser && Lampa.Parser.get) {
                        Lampa.Parser.get({ search: data.title || data.name, movie: data, page: 1 }, (res) => {
                            let items = res;
                            if (res && res.Results && Array.isArray(res.Results)) items = res.Results;
                            ...
                            if (items.length > 0) {
                                ...
                                items.slice(0, 15).forEach(i => {
                                    ...
                                    if (t.includes('4k') || t.includes('2160') || t.includes('2160p')) b.res = '4K';
                                    else if (!b.res && (t.includes('1080') || t.includes('1080p') || t.includes('fhd'))) b.res = 'FULL HD';
                                    else if (!b.res && (t.includes('720') || t.includes('720p') || t.includes('hd'))) b.res = 'HD';
                                    
                                    if (t.includes('hdr') || t.includes('hdr10')) b.hdr = true;
                                    if (t.includes('dv') || t.includes('dovi') || t.includes('vision') || t.includes('dolby vision')) b.dv = true;
                                    if (t.includes('ukr') || t.includes('укр') || t.includes('українська')) b.ukr = true;
                                });
                                let qH = '';
                                if (b.res) qH += `<div class="cas-quality-item">${QUALITY_ICONS[b.res]}</div>`;
                                if (b.dv) qH += `<div class="cas-quality-item">${QUALITY_ICONS['Dolby Vision']}</div>`;
                                else if (b.hdr) qH += `<div class="cas-quality-item">${QUALITY_ICONS['HDR']}</div>`;
                                if (b.ukr) qH += `<div class="cas-quality-item">${QUALITY_ICONS['UKR']}</div>`;
