(function () {
    ...
    try {
        ...
        // Инициализация настроек, если Storage доступен
        if (Lampa.Storage) {
        ...
        function getCubRating(e) {
            try {
                ...
                log('Ошибка в getCubRating: ' + e.message);
            }
            return null;
        }
        ...
        function renderRatings(container, e) {
            try {
                ...
                log('Ошибка в renderRatings: ' + e.message);
            }
        }
            ...
            try {
                ...
                [data.networks, data.production_companies].forEach(function(source) {
                    if (source) source.forEach(function(item) {
                        if (item.logo_path) {
                            // Используем Lampa.TMDB.image, если доступно
                            var url = null;
                            if (Lampa.TMDB && Lampa.TMDB.image) {
                                url = Lampa.TMDB.image('/t/p/w200' + item.logo_path);
                            } else {
                                url = 'https://image.tmdb.org/t/p/w200' + item.logo_path;
                            }
                            if (url && !logos.some(function(l) { return l.url === url; })) {
                ...
                logos.forEach(function(logo) {
                    ...
                    // Инвертирование тёмных логотипов
                    var img = new Image();
                ...
                log('Ошибка в renderStudioLogos: ' + e.message);
            }
        }
                    ...
                    success: function(res) {
                        try {
                            ...
                            for (var i = 0; i < langPriority.length; i++) {
                                logo = res.logos.filter(function(l) { return l.iso_639_1 === langPriority[i]; })[0];
                                if (logo) break;
                            }
                            if (!logo) logo = res.logos[0];

                            if (logo) {
                                var quality = Lampa.Storage.get('tv_interface_logo_quality', 'w500');
                                var imgUrl = Lampa.TMDB.image('/t/p/' + quality + logo.file_path.replace('.svg', '.png'));
                                logoCache[movieId] = imgUrl;
                            ...
                            log('Ошибка обработки images: ' + e.message);
                        }
                    },
                    error: function() {
                        log('Не удалось загрузить изображения с TMDB');
                    }
                });
            } catch (e) {
                log('Ошибка в loadMovieLogo: ' + e.message);
            }
        }
        ...
        function startPosterSlideshow($poster, items) {
            try {
                ...
                slideshowTimer = setInterval(function() {
                    ...
                    var imgUrl = Lampa.TMDB.image('/t/p/' + quality + items[index].file_path);
                ...
                log('Ошибка в startPosterSlideshow: ' + e.message);
            }
        }
                            ...
                            if (Lampa.Storage.get('tv_interface_quality') && Lampa.Parser && Lampa.Parser.get) {
                                Lampa.Parser.get({
                                    ...
                                    try {
                                        ...
                                        log('Ошибка обработки результатов парсера: ' + e.message);
                                    }
                                });
                            }
                        }
                    } catch (e) {
                        log('Ошибка в обработчике full: ' + e.message);
                    }
                });
                log('Слушатель full добавлен');
            } catch (e) {
                log('Ошибка в init: ' + e.message);
            }
        }
        ...
        function setupSettings() {
            try {
                if (!Lampa.SettingsApi) {
                    log('Lampa.SettingsApi не найден');
                    return;
                }
                ...
                log('Настройки добавлены');
            } catch (e) {
                log('Ошибка в setupSettings: ' + e.message);
            }
        }
        ...
        function startPlugin() {
            try {
                ...
                setupSettings();
                init();
                log('Плагин успешно запущен');
            } catch (e) {
                log('Критическая ошибка при запуске: ' + e.message);
            }
        }
        ...
        if (typeof Lampa !== 'undefined' && Lampa.log) {
            Lampa.log('TV Interface fatal error: ' + e.message);
        }
    }
})();