(function() {
    ...
    var YEARS_BACK = 5;
    var PAGES_TO_LOAD = 5; // Загружаем 5 страниц (100 фильмов вместо 20)
    ...
    // Сортировка по количеству оценок (популярность)
    function sortByVoteCount(items) {
        return items.sort(function(a, b) {
            var votesA = a.vote_count || 0;
            var votesB = b.vote_count || 0;
            
            // Если количество оценок одинаковое, сортируем по рейтингу
            if (votesB === votesA) {
    ...
    // Фильтруем только популярные (минимум 1000 оценок)
    function filterPopular(items) {
        return items.filter(function(item) {
            return (item.vote_count || 0) >= 1000;
        });
    }
        ...
        full: function(params, success, error) {
            ...
            function processResults(data, mediaType) {
                ...
                return data.results.map(function(item) {
                    ...
                    // Добавляем информацию о рейтинге и оценках
            ...
            // Функция для загрузки нескольких страниц
            function loadMultiplePages(baseUrl, mediaType, callback) {
                var allResults = [];
                var loadedPages = 0;

                for (var page = 1; page <= PAGES_TO_LOAD; page++) {
                    (function(currentPage) {
                        var url = baseUrl + '&page=' + currentPage;
                        
                        network.silent(url, function(data) {
                            if (data && data.results) {
                                allResults = allResults.concat(processResults(data, mediaType));
                            }
                            
                            loadedPages++;
                            if (loadedPages === PAGES_TO_LOAD) {
                                callback(allResults);
                            }
                        }, function() {
                            loadedPages++;
            ...
            if (type === 'all' || type === 'random') {
                var movieUrl = API_BASE + '/movie/popular?api_key=' + apiKey + '&language=' + lang;
                var tvUrl = API_BASE + '/tv/popular?api_key=' + apiKey + '&language=' + lang;
                ...
                function checkComplete() {
                    ...
                    if (completed === 2) {
                        ...
                        allResults = filterPopular(allResults);
                        allResults = sortByVoteCount(allResults);
                ...
                loadMultiplePages(movieUrl, 'movie', function(results) {
                    movies = results;
                    checkComplete();
                });

                loadMultiplePages(tvUrl, 'tv', function(results) {
                    tvShows = results;
                    checkComplete();
                });
            } else {
                ...
                var endpoint = (type === 'new_movies' || type === 'new_tv') ? 'top_rated' : 'popular';
                var baseUrl = API_BASE + '/' + mediaType + '/' + endpoint + '?api_key=' + apiKey + '&language=' + lang;

                loadMultiplePages(baseUrl, mediaType, function(results) {
                    results = filterPopular(results);
    ...
    function startPlugin() {
        ...
        var manifest = {
            ...
            version: '1.1.0',
            name: 'Топ TMDB',
            description: 'Популярные фильмы и сериалы по оценкам зрителей'
        };
