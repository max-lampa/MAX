(function () {
    ...
    // Применение сохранённых стилей к часам
    ...
    Lampa.Component.add('bubble_clock_menu', function (object) {
        ...
        this.create = function () {
            ...
            params.forEach(function (item) {
                ...
                // Обработка клика / нажатия на пульте
                row.on('click tap hover:enter', function () {
                    Lampa.Input.box('Значение', value, function (new_val) {
                        if (new_val) {
                            ...
                            applyStyles(); // сразу обновляем вид часов
                        }
                    }, false, { type: 'number' });
                });
        ...
        this.render = function () {
            // Гарантируем, что список создан перед рендером
            if (!this.list) this.create();

            // Возвращаем контейнер со скроллом — Lampa сама добавит его в интерфейс
            return scroll.render();
        };
    });

    // Создание и обновление часов в шапке
    ...
    // Инициализация: добавляем пункт в настройки и создаём часы
    function init() {
