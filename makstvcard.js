(function () {
  ...
  /* Use --card-glow (r,g,b extracted from poster) for coloured shadow fallback to 0,0,0 */
  var FOCUS_SHADOW = [
    '0 6px 18px rgba(var(--card-glow,0,0,0),.42)',
    '0 8px 24px rgba(var(--card-glow,0,0,0),.48)',
    '0 10px 30px rgba(var(--card-glow,0,0,0),.54)',
    '0 14px 36px rgba(var(--card-glow,0,0,0),.58)',
    '0 16px 44px rgba(var(--card-glow,0,0,0),.62), 0 0 24px rgba(var(--card-glow,0,0,0),.22)',
    '0 20px 54px rgba(var(--card-glow,0,0,0),.66), 0 0 36px rgba(var(--card-glow,0,0,0),.28)',
    '0 24px 64px rgba(var(--card-glow,0,0,0),.72), 0 0 52px rgba(var(--card-glow,0,0,0),.36)'
  ];
  ...
  function buildCss() {
    ...
    return [
      ...
      /* Trigger shimmer animation on focus/hover */
      B + '[' + KEYS.GLASS_ATTR + '="on"] .card.focus .makstv-shimmer, ' +
      B + '[' + KEYS.GLASS_ATTR + '="on"] .card.hover .makstv-shimmer, ' +
      B + '[' + KEYS.GLASS_ATTR + '="on"] .card-episode.focus .makstv-shimmer, ' +
      B + '[' + KEYS.GLASS_ATTR + '="on"] .card-episode.hover .makstv-shimmer { animation:makstv-shimmer .75s cubic-bezier(.4,0,.2,1) both; }',
      ...
      B + '[' + KEYS.DEPTH_BLUR_ATTR + '="on"] .items-line[data-makstv-row-focus="1"] .card:not(.focus):not(.hover), ' +
      B + '[' + KEYS.DEPTH_BLUR_ATTR + '="on"] .items-line[data-makstv-row-focus="1"] .card-episode:not(.focus):not(.hover) { -webkit-filter:blur(.6px) brightness(.78) saturate(.85) !important; filter:blur(.6px) brightness(.78) saturate(.85) !important; -webkit-transform:scale(.96) !important; transform:scale(.96) !important; transition:-webkit-filter .26s ease, filter .26s ease, -webkit-transform .26s ease, transform .26s ease !important; }',
  ...
  function resetSettings() {
    var keys = [KEYS.ENABLE_KEY,KEYS.BACKDROP_KEY,KEYS.BADGE_KEY,KEYS.RATING_KEY,KEYS.RATING_STYLE_KEY,
                ...
                KEYS.GLASS_KEY,KEYS.DEPTH_BLUR_KEY];
