(function () {
  ...
  const AGNATIVE_KEYS = {
    ...
    CARD_FOCUS_SCALE_KEY: 'appletv_agnative_card_focus_scale',
    CARD_FOCUS_BORDER_KEY: 'appletv_agnative_card_focus_border',
    CARD_FOCUS_SCALE_ATTR: 'data-agnative-focus-scale',
    CARD_FOCUS_BORDER_ATTR: 'data-agnative-focus-border'
  };

  const ru = {
    nav_feed: 'Лента',
    badge_movie: 'ФИЛЬМ', badge_tv: 'СЕРИАЛ',
    set_about_desc: 'AppleMax',
    set_main_title: 'Основные настройки',
    set_enable_name: 'AppleMax',
    ...
    set_focus_scale_name: 'Масштаб фокуса карточки',
    set_focus_scale_desc: 'Увеличение карточки при фокусе — от минимального до максимального значения',
    val_focus_scale_none: 'Отключить (нет масштаба)',
    set_focus_border_name: 'Рамка при фокусе',
    set_focus_border_desc: 'Показывать рамку вокруг карточки при фокусе. Использует цвет плагина Colors, если он активен'
  };
  ...
  const en = {
    ...
    set_focus_scale_name: 'Card focus scale',
    set_focus_scale_desc: 'Card zoom on focus — from minimum to maximum value',
    val_focus_scale_none: 'Disable (no zoom)',
    set_focus_border_name: 'Focus border',
    set_focus_border_desc: 'Show border around card on focus. Uses Colors plugin color if it is active'
  };
  ...
  const uk = {
    ...
    set_focus_scale_name: 'Масштаб фокуса картки',
    set_focus_scale_desc: 'Збільшення картки при фокусі — від мінімального до максимального значення',
    val_focus_scale_none: 'Вимкнути (без масштабу)',
    set_focus_border_name: 'Рамка при фокусі',
    set_focus_border_desc: 'Показувати рамку навколо картки при фокусі. Використовує колір плагіна Colors, якщо він активний'
  };
  ...
  const be = {
    ...
    set_focus_scale_name: 'Маштаб фокуса карткі',
    set_focus_scale_desc: 'Павелічэнне карткі пры фокусе — ад мінімальнага да максімальнага значэння',
    val_focus_scale_none: 'Адключыць (без маштабу)',
    set_focus_border_name: 'Рамка пры фокусе',
    set_focus_border_desc: 'Паказваць рамку вакол карткі пры фокусе. Выкарыстоўвае колер плагіна Colors, калі ён актыўны'
  };
  ...
  (function () {
    ...
    var {
      ...
      LOGO_TITLE_KEY,
      CARD_FOCUS_SCALE_KEY,
      CARD_FOCUS_BORDER_KEY,
      CARD_FOCUS_SCALE_ATTR,
      CARD_FOCUS_BORDER_ATTR
    } = AGNATIVE_KEYS;
    ...
    function getCardFocusScale() {
      try {
        if (!window.Lampa || !Lampa.Storage) return 'md';
        var v = Lampa.Storage.get(CARD_FOCUS_SCALE_KEY, 'md') || 'md';
        if (['none','xs','sm','md','lg','xl'].indexOf(v) > -1) return v;
        return 'md';
      } catch (e) { return 'md'; }
    }

    function getCardFocusBorderOn() {
      try {
        if (!window.Lampa || !Lampa.Storage) return 'on';
        return Lampa.Storage.get(CARD_FOCUS_BORDER_KEY, 'on') !== 'off' ? 'on' : 'off';
      } catch (e) { return 'on'; }
    }

    function getColorsPluginColor() {
      try {
        var color = null;
        var enabled = false;
        if (window.Lampa && Lampa.Storage) {
          enabled = Lampa.Storage.get('color_plugin_enabled', 'false') === 'true';
          if (!enabled) { try { enabled = localStorage.getItem('color_plugin_enabled') === 'true'; } catch(e){} }
          if (enabled) {
            color = Lampa.Storage.get('color_plugin_main_color', null) || null;
            if (!color) { try { color = localStorage.getItem('color_plugin_main_color'); } catch(e){} }
          }
        }
        if (color === '#353535' || color === 'default' || color === 'null' || !color) return null;
    ...
    function removePluginUi() {
      try {
        if (document.body) {
          ...
          document.body.removeAttribute(CARD_FOCUS_SCALE_ATTR);
          document.body.removeAttribute(CARD_FOCUS_BORDER_ATTR);
        }
        var focusStyle = document.getElementById('applemax-focus-color-style');
    ...
    function syncFocusScale() {
      if (!document.body) return;
      document.body.setAttribute(CARD_FOCUS_SCALE_ATTR, getCardFocusScale());
      document.body.setAttribute(CARD_FOCUS_BORDER_ATTR, getCardFocusBorderOn());
    }

    function syncFocusColorStyle() {
      var styleId = 'applemax-focus-color-style';
      var existing = document.getElementById(styleId);
      var color = getColorsPluginColor();
      var borderOn = getCardFocusBorderOn() !== 'off';
      var css = '';
      if (!borderOn) {
        css = 'body.' + BODY_CLASS + ' { --applemax-border-width: 0px !important; }';
      } else if (color) {
        css = 'body.' + BODY_CLASS + ' { --applemax-focus-color: ' + color + ' !important; }';
      }
      if (!css) {
      ...
      if (!existing) {
        ...
        existing.id = styleId;
        (document.head || document.body).appendChild(existing);
      }
    ...
    function resetSettings() {
      try {
        ...
        Lampa.Storage.set(CARD_FOCUS_SCALE_KEY, 'md');
        Lampa.Storage.set(CARD_FOCUS_BORDER_KEY, 'on');
    ...
    function registerSettings() {
      try {
        ...
        Lampa.SettingsApi.addParam({
          component: SETTINGS_COMPONENT,
          param: {
            name: CARD_FOCUS_BORDER_KEY,
            type: 'select',
            values: { on: t('val_on'), off: t('val_off') },
            default: 'on'
          },
          field: {
            name: t('set_focus_border_name'),
            description: t('set_focus_border_desc')
          },
          onChange: function () {
            syncFocusScale();
            syncFocusColorStyle();
    ...
    function bindRuntimeListeners() {
      ...
      if (!storageListenerBound) {
        ...
        Lampa.Storage.listener.follow('change', function (e) {
          ...
          if (e.name === CARD_FOCUS_SCALE_KEY) {
            syncFocusScale();
            return;
          }

          if (e.name === CARD_FOCUS_BORDER_KEY) {
            syncFocusScale();
            syncFocusColorStyle();
            return;
          }
    ...
    function injectStyle() {
      ...
      var text = [
        ...
        'body.' + BODY_CLASS + ' { --applemax-focus-scale: 1.06; --applemax-focus-color: rgba(86,141,255,.92); --applemax-border-width: 2px; }',
        'body.' + BODY_CLASS + '[' + CARD_FOCUS_SCALE_ATTR + '="none"] { --applemax-focus-scale: 1; }',
        'body.' + BODY_CLASS + '[' + CARD_FOCUS_SCALE_ATTR + '="xs"] { --applemax-focus-scale: 1.02; }',
        'body.' + BODY_CLASS + '[' + CARD_FOCUS_SCALE_ATTR + '="sm"] { --applemax-focus-scale: 1.04; }',
        'body.' + BODY_CLASS + '[' + CARD_FOCUS_SCALE_ATTR + '="md"] { --applemax-focus-scale: 1.06; }',
        'body.' + BODY_CLASS + '[' + CARD_FOCUS_SCALE_ATTR + '="lg"] { --applemax-focus-scale: 1.09; }',
        'body.' + BODY_CLASS + '[' + CARD_FOCUS_SCALE_ATTR + '="xl"] { --applemax-focus-scale: 1.13; }',
        'body.' + BODY_CLASS + '[' + CARD_FOCUS_BORDER_ATTR + '="off"] { --applemax-border-width: 0px; }',
        'body.' + BODY_CLASS + ' .card.focus .card__view { transform: translateY(-.08em) scale(var(--applemax-focus-scale, 1.06)) !important; filter: saturate(1.06) brightness(1.02) !important; box-shadow: inset 0 1px 0 rgba(255,255,255,.22), 0 0 0 var(--applemax-border-width, 2px) var(--applemax-focus-color, rgba(86,141,255,.92)), 0 18px 42px rgba(0,0,0,.26), 0 8px 20px rgba(0,0,0,.14) !important; }',
        ...
        'body.' + BODY_CLASS + ' .card-episode.focus .full-episode__img { filter: saturate(1.06) brightness(1.02) !important; box-shadow: inset 0 1px 0 rgba(255,255,255,.22), 0 0 0 var(--applemax-border-width, 2px) var(--applemax-focus-color, rgba(86,141,255,.92)), 0 18px 42px rgba(0,0,0,.26), 0 8px 20px rgba(0,0,0,.14) !important; }',
        ...
        'body.' + BODY_CLASS + ':not(.' + GLARE_CLASS + ') .card-episode.focus .full-episode { transform: translateY(-.08em) scale(var(--applemax-focus-scale, 1.06)) !important; }',
        ...
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .card .card__view { transition: transform .15s ease, box-shadow .15s ease !important; }',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .card-episode .full-episode { transition: transform .15s ease !important; }',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .card-episode .full-episode__img { transition: box-shadow .15s ease !important; }',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .card.focus .card__view { filter: none !important; box-shadow: 0 0 0 var(--applemax-border-width, 2px) var(--applemax-focus-color, rgba(86,141,255,.92)), 0 6px 14px rgba(0,0,0,.22) !important; }',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .card.hover .card__view { filter: none !important; box-shadow: 0 0 0 1px var(--applemax-focus-color, rgba(86,141,255,.60)), 0 6px 14px rgba(0,0,0,.18) !important; }',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .card-episode.focus .full-episode__img { filter: none !important; box-shadow: 0 0 0 var(--applemax-border-width, 2px) var(--applemax-focus-color, rgba(86,141,255,.92)), 0 6px 14px rgba(0,0,0,.22) !important; }',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="low"] .card-episode.hover .full-episode__img { filter: none !important; box-shadow: 0 0 0 1px var(--applemax-focus-color, rgba(86,141,255,.60)), 0 6px 14px rgba(0,0,0,.18) !important; }',
        ...
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="ultra"] .card.focus .card__view { transform: none !important; filter: none !important; box-shadow: 0 2px 6px rgba(0,0,0,.4) !important; box-sizing: border-box !important; border-color: var(--applemax-focus-color, rgba(86,141,255,.95)) !important; }',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="ultra"] .card.hover .card__view { transform: none !important; filter: none !important; box-shadow: 0 2px 6px rgba(0,0,0,.4) !important; box-sizing: border-box !important; border-color: var(--applemax-focus-color, rgba(86,141,255,.65)) !important; }',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="ultra"] .card-episode.focus .full-episode__img { filter: none !important; box-shadow: 0 2px 6px rgba(0,0,0,.4) !important; box-sizing: border-box !important; border-color: var(--applemax-focus-color, rgba(86,141,255,.95)) !important; }',
        'body.' + BODY_CLASS + '[' + PERF_ATTR + '="ultra"] .card-episode.hover .full-episode__img { filter: none !important; box-shadow: 0 2px 6px rgba(0,0,0,.4) !important; box-sizing: border-box !important; border-color: var(--applemax-focus-color, rgba(86,141,255,.65)) !important; }',
