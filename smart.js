(function () {
  ...
  function detectBaseComponentFromManifest() {
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      if (/online|\u043E\u043D\u043B\u0430\u0439\u043D|nextgen|lampa|serial|film|movie|\u0444\u0438\u043B\u044C\u043C|\u0441\u0435\u0440\u0438\u0430\u043B/i.test(text))
        return p.component;
    }
  ...
  function getVoiceMode() {
    return String(settingValue(CFG_VOICE_MODE, 'ru')); // ИЗМЕНЕНО НА ru
  }
  ...
  function preferredQuality() {
    var target = String(settingValue(CFG_QUALITY_TARGET, '1080')); // ИЗМЕНЕНО НА 1080
    if (target !== 'player') {
  ...
  // ИЗМЕНЕНО: теперь ищем русскую озвучку вместо украинской
  function isRussianVoice(name) {
    var text = normalize(name);
    if (!text)
      return false;

    return /(lostfilm|novamedia|amedia|hdrezka|\u0440\u0443\u0441|\u0440\u043E\u0441|russian|dub|professional|coldfilm|кубик|кураж|бамбук)/i.test(text);
  }
  ...
  function voiceWeight(name) {
    ...
    if (/(lostfilm|novamedia|amedia|hdrezka)/i.test(text))
      score += 10;
    if (mode !== 'neutral' && /(\u0443\u043A\u0440|ukrainian)/i.test(text))
      score -= 25;
    if (/(\u043E\u0440\u0438\u0433\u0438\u043D\u0430\u043B|original)/i.test(text))
      score -= 8;
  ...
  function rankVoices(buttons, sourceName, context) {
    ...
    return buttons.map(function (button, index) {
      ...
      return {
        ...
        score: score,
        isRussian: isRussianVoice(title) // ИЗМЕНЕНО
      };
    }).sort(function (a, b) {
      if (voiceMode === 'ru' && a.isRussian !== b.isRussian) // ИЗМЕНЕНО
  ...
  function buildQueue(videos, sourceName, fallbackVoice, context) {
    ...
    videos.forEach(function (item) {
      ...
      var candidate = {
        ...
        quality: quality,
        score: score,
        isRussian: isRussianVoice(voiceName) // ИЗМЕНЕНО
      };
  ...
  function addLang() {
    Lampa.Lang.add({
      ...
      lampac_smart_descr: { ru: 'Автовыбор потока', en: 'Auto stream select', uk: 'Автовибір потоку', zh: 'Auto stream select' },
  ...
  function registerSettings() {
    ...
    Lampa.SettingsApi.addParam({
      ...
      param: {
        ...
        type: 'select',
        values: {
          ru: 'Русская в приоритете', // ИЗМЕНЕНО
  ...
  function registerManifest() {
    ...
    if (Array.isArray(plugins)) {
      ...
      for (var i = 0; i < plugins.length; i++) {
        ...
        if (!p)
          continue;
        if (p.name === 'Smart Online' || p.name === 'Смарт Online' || p.component === smartComponentName()) {
      ...
      if (plugins.name === 'Smart Online' || plugins.name === 'Смарт Online' || plugins.component === smartComponentName()) {
  ...
  function buildOnlineScriptUrl() {
    // ЗАМЕНА АДРЕСА: http://bwa.ad/rc
    return 'http://bwa.ad/rc';
  }
