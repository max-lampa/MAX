(function () {
  ...
  var OWN = {
    ...
    nova_skin_set_enable_descr: { ru: 'Единый интерфейс для списка торрентов', uk: 'Єдиний інтерфейс для списку торрентів', en: 'One interface for the torrent list' },
    nova_skin_set_enable_descr_alt: { ru: 'Единый интерфейс для списка торрентов, кроме тех, где он уже встроен', uk: 'Єдиний інтерфейс для списку торрентів, крім тих, де він уже вбудований', en: 'One interface for the torrent list except those that already ship with it' },
  ...
  function bridgeActiveController() {
    try {
      if (!Lampa.Controller || Lampa.Controller.nova_active_bridge) return;
      var active = Lampa.Controller.enabled && Lampa.Controller.enabled();
      if (!active || active.nova_active_bridge) return;
      ['up', 'down', 'left', 'right'].forEach(function (direction) {
        if (typeof active[direction] !== 'function') return;
        var original = active[direction];
        active[direction] = function () {
          if (inSkin()) {
            if (direction === 'up' && novaUp()) return;
            if (direction === 'down' && novaDown()) return;
            if (direction === 'right' && novaRight()) return;
          }
          return original.apply(this, arguments);
        };
      });
      active.nova_active_bridge = true;
      Lampa.Controller.nova_active_bridge = true;
    } catch (e) {}
  }
  ...
  function probeAnswer(key, answer) {
    ...
    if (!/torrent-item|online-prestige|videos__|"method"|"data"/i.test(body)) return probeMark(key, 'skip');
  ...
  function torrentText(origin, selectors) {
    var out = [];
    for (var i = 0; i < selectors.length; i++) {
      origin.find(selectors[i]).each(function () {
  ...
  function torrentTitle(origin) {
    var value = '';
    var selectors = ['.torrent-item__title', '.torrent-item__name', '[data-title]'];
    for (var i = 0; i < selectors.length && !value; i++) {
      var node = origin.find(selectors[i]).first();
      value = (node.attr('data-title') || node.text() || '').replace(/\s+/g, ' ').trim();
    }
    return value || ('Раздача ' + (arguments[1] || 1));
  }

  function torrentPicture(origin) {
    return origin.find('.torrent-item__img img,.torrent-item__poster img,.torrent-item__cover img,img').first();
  }

  function rawNumber(value) {
    var match = String(value === undefined || value === null ? '' : value)
      .replace(/\s/g, '').replace(',', '.').match(/-?\d+(?:\.\d+)?/);
    return match ? parseFloat(match[0]) : 0;
  }

  function torrentSeeders(origin) {
    var value = origin.attr('data-seeders') || origin.attr('data-seed') ||
      origin.attr('data-sid') || origin.find('.torrent-item__seeds,.torrent-item__seeders').first().text();
    return Math.max(0, Math.round(rawNumber(value)));
  }

  function torrentSizeBytes(origin) {
    var value = origin.attr('data-size') || origin.attr('data-size-bytes') ||
      origin.find('.torrent-item__size').first().text();
    var match = String(value || '').replace(',', '.').match(/(\d+(?:\.\d+)?)\s*(b|kb|kib|mb|mib|gb|gib|tb|tib)?/i);
    if (!match) return 0;
    var unit = (match[2] || 'b').toLowerCase();
    var power = { b: 0, kb: 1, kib: 1, mb: 2, mib: 2, gb: 3, gib: 3, tb: 4, tib: 4 }[unit];
    return Math.round(parseFloat(match[1]) * Math.pow(1024, power === undefined ? 0 : power));
  }

  function readCard(node, index) {
    var origin = $(node);
    var title = torrentTitle(origin, index + 1);
    var meta = torrentText(origin, [
      '.torrent-item__tracker', '.torrent-item__seeders', '.torrent-item__peers',
      '.torrent-item__size', '.torrent-item__bitrate', '.torrent-item__quality',
      '.torrent-item__info', '.torrent-item__details', '.torrent-item__meta'
    ]);
    return {
      origin: origin, index: index, folder: false, soon: false, percent: 0, hash: '',
      line: $(), viewed: false, num: index + 1, numbered: false, title: title,
      meta: meta, time: torrentText(origin, ['.torrent-item__size']).join(' '),
      quality: shortQuality(title + ' ' + meta.join(' ')), picture: torrentPicture(origin),
      seeders: torrentSeeders(origin), sizeBytes: torrentSizeBytes(origin)
    };
  }
  ...
  function buildCard(item, compact, grid) {
    ...
    card.attr('data-nova-seeders', item.seeders || 0);
    card.attr('data-nova-size', item.sizeBytes || 0);
  ...
  function novaDown() {
    ...
    // On Android TV the first DOWN comes from the hero button, not the toolbar.
    // Handle both paths explicitly so the native Controller cannot escape the skin.
    if (rowsFocused() && !toolbarFocused()) {
      lockRelease();
      var heroTarget = pickResume(items);
      if (heroTarget && heroTarget.card && heroTarget.card.length) return focusNode(heroTarget.card);
      var first = ui.list.find('.nova-card.selector').first();
      return first.length ? focusNode(first) : false;
    }
    ...
    var fallback = ui.list.find('.nova-card.selector').first();
    return fallback.length ? focusNode(fallback) : false;
  }
    ...
    try {
      ...
      Lampa.SettingsApi.addParam({
        ...
        field: {
          ...
          description: label('nova_skin_set_enable_descr')
        },
  ...
  EXTRA_CSS += '.nova-skin-root .nova-hero__season{color:inherit!important;font-weight:400!important;opacity:.58!important}' +
    '.nova-skin-root .nova-hero__progress{height:.34em!important;background:rgba(255,255,255,.25)!important;border:0!important;box-shadow:none!important}' +
    '.nova-skin-root .nova-hero__progress .time-line>div{background:rgba(255,255,255,.92)!important}' +
    '.nova-skin-root .nova-card__line{height:.34em!important;background:rgba(255,255,255,.2)!important}' +
    '.nova-skin-root .nova-card__line .time-line>div{background:rgba(255,255,255,.92)!important}' +
    '.nova-skin-root .nova-btn--main{background:#c83288!important;color:#fff!important}' +
    '.nova-skin-root .nova-btn--main.focus{background:#dd3f98!important}' +
    '.nova-skin-root .nova-chip.focus,.nova-skin-root .nova-card.focus,.nova-skin-root .nova-group.focus{background:#fff!important;color:#111!important}';
  ...
  (function unifiedTorrentControls() {
    var SORT_KEY = 'nova_torrent_sort_mode';
    var sortTimer = null;
    var lastMode = null;

    function torrentRoot() {
      var box = $('.nova-skin-root').last();
      return box.length && box.find('.nova-card').length ? box : $();
    }
    function num(v) {
      var m = String(v || '').replace(/[, ]/g, '').match(/-?\d+(?:\.\d+)?/);
      return m ? parseFloat(m[0]) : 0;
    }
    function metric(node, mode) {
      var value = $(node).attr(mode === 'size' ? 'data-nova-size' : 'data-nova-seeders');
    ...
    function sort(mode) {
      var box = torrentRoot();
      if (!box.length) return;
      var cards = box.find('.nova-card').get();
      cards.sort(function(a,b) {
        var delta = metric(b, mode) - metric(a, mode);
        if (delta) return delta;
        return parseInt($(a).attr('data-nova-focus').replace(/\D/g, ''), 10) -
          parseInt($(b).attr('data-nova-focus').replace(/\D/g, ''), 10);
      });
      var parent = $(cards[0]).parent();
      cards.forEach(function(card) { parent.append(card); });
      lastMode = mode;
    }
    function focusables() {
      var box = torrentRoot();
      return box.find('.nova-btn.selector,.nova-chip.selector,.nova-card.selector,.nova-group.selector').filter(':visible');
    }
    function move(dir) {
      var list = focusables(); if (!list.length) return false;
      var from = list.filter('.focus').first(); if (!from.length) from = list.first();
      var i = list.index(from), target = null;
      if (dir === 'left') target = list.eq(i - 1);
      if (dir === 'right') target = list.eq(i + 1);
      if (dir === 'up' || dir === 'down') {
        var r = from[0].getBoundingClientRect(), best = Infinity;
        list.each(function() {
          if (this === from[0]) return;
          var x = this.getBoundingClientRect();
          var ok = dir === 'down' ? x.top >= r.bottom - 10 : x.bottom <= r.top + 10;
          if (!ok) return;
          var d = Math.abs(x.left-r.left) + Math.abs(x.top-r.top) * .3;
          if (d < best) { best=d; target=$(this); }
        });
      }
      if (!target || !target.length) return false;
      try { Lampa.Controller.collectionFocus(target, torrentRoot()); } catch(e) { list.removeClass('focus'); target.addClass('focus'); }
      try { target[0].scrollIntoView({block:'nearest',inline:'nearest'}); } catch(e) {}
      return true;
    }
    function enter() {
      var node = focusables().filter('.focus').first();
      if (!node.length) node = focusables().first();
      if (!node.length) return false;
      try { node.trigger('hover:enter'); return true; } catch(e) { return false; }
    }
    $(document).on('keydown.novaTorrentUnified', function(e) {
      if (!torrentRoot().length) return;
      var k=e.key || e.code, ok=false;
      if (k==='ArrowLeft'||k==='Left') ok=move('left');
      else if (k==='ArrowRight'||k==='Right') ok=move('right');
      else if (k==='ArrowUp'||k==='Up') ok=move('up');
      else if (k==='ArrowDown'||k==='Down') ok=move('down');
      else if (k==='Enter'||k==='NumpadEnter'||k==='OK') ok=enter();
      if (ok) { e.preventDefault(); e.stopPropagation(); }
    });
    function scan() {
      var box=torrentRoot(); if (!box.length) return;
      if (!box.find('.nova-torrent-sort').length) {
        var bar=box.find('.nova-toolbar').first();
        if (bar.length) {
          ['seeders','size'].forEach(function(mode) {
            var b=$('<div class="nova-chip selector nova-torrent-sort"><span>'+(mode==='size'?'Размер':'Сидеры')+'</span></div>');
            b.attr('data-sort-mode',mode).on('hover:enter',function(){localStorage.setItem(SORT_KEY,mode);sort(mode);});
            bar.append(b);
          });
        }
      }
      var mode=localStorage.getItem(SORT_KEY)==='size'?'size':'seeders';
      if (mode!==lastMode) sort(mode);
    }
    function schedule(){clearTimeout(sortTimer);sortTimer=setTimeout(scan,120);}
    try { Lampa.Listener.follow('activity',function(e){if(e.type==='start'||e.type==='archive') schedule();}); } catch(e) {}
    setTimeout(scan,350);
