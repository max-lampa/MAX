...
// @version      7.3.0
// @description  Стабильная заставка для Lampa: MP4/HLS, резервные потоки, защита от зависаний, исправленный звук. Android TV 9+
...
(function() {
  ...
  function createNoiseSource(ctx) {
    var length = Math.max(1, ctx.sampleRate * 2);
  ...
  function stopAmbientSound() {
    ...
    try { clearInterval(ambientNodes.timer); } catch(e) {}
    try { ambientNodes.gain.gain.cancelScheduledValues(0); } catch(e) {}
    try { ambientNodes.source.stop(0); } catch(e) {}
    ambientNodes = null;
  }

  function startAmbientSound() {
    stopAmbientSound();
    if (!storageBool('atm_sound', false)) return;
    ...
    gain.gain.value = Math.max(0.01, volume * 0.16);
    ...
    ambientNodes = { source: source, filter: filter, gain: gain, timer: null };

    if (mode === 'fire') {
      ambientNodes.timer = setInterval(function() {
        if (!ambientNodes || !screensaverDiv) return;
        var now = ctx.currentTime;
        var target = Math.max(0.01, volume * (0.07 + Math.random() * 0.34));
        try {
          filter.frequency.setTargetAtTime(260 + Math.random() * 1350, now, 0.03);
          gain.gain.cancelScheduledValues(now);
          gain.gain.setTargetAtTime(target, now, 0.025);
          gain.gain.setTargetAtTime(Math.max(0.01, volume * 0.13), now + 0.08, 0.12);
        } catch(e) {}
      }, 180);
    } else {
      ambientNodes.timer = setInterval(function() {
        ...
        var now = ctx.currentTime;
        try {
          filter.frequency.setTargetAtTime(620 + Math.random() * 680, now, 0.8);
          gain.gain.setTargetAtTime(Math.max(0.01, volume * (0.11 + Math.random() * 0.08)), now, 0.8);
        } catch(e) {}
      }, 900);
    }
  }
  ...
  /*
   * Проблема оригинала: при блокировке autoplay браузер/TV отклоняет play() если
   * video.muted=false. Старый код делал muted=true → play() → then(updateVideoAudio),
   * но updateVideoAudio сразу ставил muted=false/volume=X, что вызывало повторное
   * исключение на Android TV (политика autoplay запрещает unmute без жеста).
   *
   * Решение:
   * 1. Всегда стартуем с muted=true (гарантированный autoplay на любом TV).
   * 2. После события 'playing' ждём 500мс и пробуем плавно включить звук.
   * 3. Если unmute снова заблокирован — оставляем muted, звук не вешает стрим.
   * 4. Отдельная функция applyAudioSettings применяет настройки без сброса muted.
   */
  function applyAudioSettings(video) {
    if (!video) return;
    ...
    startAmbientSound();
    /* Звук включён пользователем — пробуем unmute */
    if (video.muted) {
      video.muted = false;
      /* Форсируем громкость после unmute — Android TV иногда сбрасывает */
      try { video.volume = volume; } catch(e) {}
    }
  }

  function tryUnmuteAfterPlay(video) {
    if (!video) return;
    var soundOn = storageBool('atm_sound', false);
    if (!soundOn) { video.muted = true; stopAmbientSound(); return; }
    startAmbientSound();
    var volume = storageNumber('atm_volume', 50, 0, 100) / 100;
    /* Небольшая задержка — даём плееру стабилизироваться перед unmute */
    setTimeout(function() {
      if (!video || !screensaverDiv) return;
      try {
        video.volume = volume;
        video.muted = false;
        /* Проверяем через 200мс — если браузер откатил muted, не трогаем */
        setTimeout(function() {
          if (!video || !screensaverDiv) return;
          if (video.muted && soundOn) {
            /* Повторная попытка через userInteraction-safe путь */
            video.volume = volume;
            var p = video.play();
            if (p && p.catch) p.catch(function() { /* silently ignore */ });
          }
        }, 200);
      } catch(e) { /* Политика autoplay — оставляем muted */ }
    }, 500);
  }
  ...
  /* ─── playVideo: всегда стартуем muted, unmute — отдельно ─── */
  function playVideo(video, onFail) {
    if (!video || !screensaverDiv) return;
    /* Стартуем всегда muted — гарантированный autoplay на Android TV */
    video.muted = true;
    var promise;
    try { promise = video.play(); } catch(e) { if (onFail) onFail(e); return; }
    if (promise && promise.catch) {
      promise.catch(function(err) {
        if (!screensaverDiv) return;
        /* play() отклонён даже с muted — серьёзная ошибка, переключаем стрим */
  ...
  function startWatchdog(video) {
    ...
    watchdogInterval = setInterval(function() {
      ...
      if (video.paused && video.readyState >= 2) {
        /* Видео встало — пробуем перезапустить */
        video.muted = true;
        var p;
        try { p = video.play(); } catch(e) { nextStream('paused'); return; }
        if (p && p.catch) p.catch(function() { nextStream('paused'); });
        return;
      }
  ...
  function loadMp4(video, stream, token) {
    ...
    video.loop = true;
    video.preload = 'auto';
    /* Явно muted до первого play — политика autoplay */
    video.muted = true;
    ...
    function requestPlay() {
      ...
      playRequested = true;
      playVideo(video, function() {
        if (token !== loadSeq) return;
        nextStream('mp4-play-error');
      });
    }
    ...
    onVideo(video, 'playing', function() {
      if (token !== loadSeq) return;
      clearStartTimeout();
      /* Применяем звук после успешного старта воспроизведения */
      tryUnmuteAfterPlay(video);
      startWatchdog(video);
    });
  ...
  function loadNativeHls(video, stream, token) {
    ...
    function requestPlay() {
      ...
      playVideo(video, function() {
        if (token !== loadSeq) return;
        nextStream('native-hls-play-error');
      });
    }
  ...
  function loadStream(video) {
    ...
    /* Начальное состояние — всегда muted */
    video.muted = true;
  ...
  function showScreensaver() {
    ...
    /* Всегда muted при создании — unmute после 'playing' через tryUnmuteAfterPlay */
    video.muted = true;
  ...
  function addSettings() {
    ...
    Lampa.SettingsApi.addParam({ component:'atm_settings', param:{name:'atm_sound',type:'select',values:{'true':'Включен','false':'Выключен'},default:'false'}, field:{name:'Звук'}, onChange:function(){ applyAudioSettings(activeVideo); } });
    Lampa.SettingsApi.addParam({ component:'atm_settings', param:{name:'atm_volume',type:'select',values:{'0':'0%','10':'10%','30':'30%','50':'50%','70':'70%','100':'100%'},default:'50'}, field:{name:'Громкость'}, onChange:function(){ applyAudioSettings(activeVideo); } });
    Lampa.SettingsApi.addParam({ component:'atm_settings', param:{type:'button',name:'atm_test_now'}, field:{name:'Запустить заставку сейчас'}, onChange:function(){ unlockAudioContext(); showScreensaver(); } });
  }
