(function() {
    ...
    var SafeStyle = {
        ...
        settings: {
            theme: 'custom_color',
            custom_color: '#c22222', // Начальный цвет (Красный)
            enabled: true, // Состояние плагина (включено/выключено)
            button_styles_enabled: true // Состояние управления стилями кнопок
        }
    };
    ...
    function applyTheme(theme, color) {
        ...
        // Если плагин отключен или выбрано "default", сбрасываем стили
        if (!SafeStyle.settings.enabled || theme === 'default') return;

        // Используем переданный цвет или сохраненный, если тема "custom_color"
        var selectedColor = (theme === 'custom_color') ? (color || SafeStyle.settings.custom_color || '#c22222') : '#c22222';

        // Код SVG для лоадера с выбранным цветом
        ...
        // Динамический стиль на основе выбранного цвета
        var dynamicTheme = `
            ...
            .full-start__button svg {
                fill: #ffffff; /* Цвет иконок без фокуса */
            }
            .full-start__button.focus svg {
                fill: ${selectedColor}; /* Цвет иконок в фокусе */
            }
            .menu__item svg {
                fill: #ffffff; /* Цвет иконок в меню без фокуса */
            }
            .menu__item.focus svg,
            .menu__item.traverse svg,
            .menu__item.hover svg {
                fill: ${selectedColor}; /* Цвет иконок в меню при фокусе/наведении */
            }
            .simple-button svg {
                fill: #ffffff; /* Цвет иконок в простых кнопках без фокуса */
            }
            .simple-button.focus svg {
                fill: ${selectedColor}; /* Цвет иконок в простых кнопках при фокусе */
            }
        `;
    ...
    function AddIn() {
        ...
        // Шаблон карточки фильма/сериала (с возвращением рейтинга TMDB)
        ...
        // Базовые стили (без стилей для рейтинга)
        var style = `
            <style>
                ...
                /* Скрываем параметр "Цвет темы" по умолчанию */
                div[data-name="safe_style_color"] {
                    display: none;
                }
                /* Показываем "Цвет темы", если выбрана Пользовательская */
        ...
        // Добавляем новый компонент в меню настроек
        ...
        // Функция для обновления видимости параметра "Цвет темы"
        ...
        // Добавляем параметры в компонент safe_style
        if (Lampa.SettingsApi) {
            ...
            Lampa.SettingsApi.addParam({
                ...
                field: {
                    name: Lampa.Lang.translate('Цвет темы'),
                    description: 'Выберите цвет для пользовательской темы'
                },
                onChange: function(value) {
            ...
            Lampa.SettingsApi.addParam({
                ...
                field: {
                    name: Lampa.Lang.translate('Включить стили кнопок'),
                    description: 'Включить или выключить управление стилями кнопок'
                },
                onChange: function(value) {
            ...
            Lampa.SettingsApi.addParam({
                ...
                field: {
                    name: Lampa.Lang.translate('Включить плагин'),
                    description: 'Включить или выключить плагин тем'
                },
                onChange: function(value) {
        ...
        // Инициализация видимости параметра "Цвет темы" при открытии настроек
    ...
    Lampa.Listener.follow('app', function(e) {
        if (e.type === 'ready') {
            ...
            // Применяем шаблоны и стили с задержкой
            setTimeout(function() {
                if (Lampa.SettingsApi) {
                    AddIn();
                    applyTheme(SafeStyle.settings.theme);
