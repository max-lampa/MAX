(function () {
    ...
    /* ═══════════ CSS ═══════════ */
    var CSS = [
        '.sbru-row{display:flex;flex-wrap:nowrap;gap:3px 5px;margin-top:6px;align-items:center;',
        'overflow:hidden;max-width:100%}',
        '.sbru-b{display:inline-block;padding:1px 7px;border-radius:4px;flex-shrink:0;',
    ...
    function processEl(el) {
        ...
        // Skip if a parent was already processed (avoid double-injection)
        if (el.closest && el.closest('[' + ATTR + '="1"]')) return;
    ...
    // Content-based fallback: scan DIRECT children of known scroll/list containers
    function scanByContent() {
        // Lampa renders stream lists inside scroll bodies / list wrappers
        var containers = document.querySelectorAll(
            '.scroll__body,.scroll__list,.online-prestige,.online-list,' +
            '[class*="scroll__body"],[class*="scroll__list"],[class*="online-prestige"],' +
            '[class*="online-list"],[class*="torrent-list"],[class*="stream-list"]'
        );

        containers.forEach(function (container) {
            // Only direct children — prevents parent/child double processing
            Array.from(container.children).forEach(function (el) {
                if (el.tagName !== 'DIV' && el.tagName !== 'LI') return;
                if (el.getAttribute(ATTR))                        return;
                if (el.classList.contains('sbru-row'))            return;
                if (el.classList.contains('sbru-b'))              return;
                // Must have children (title + info) but not be a giant container
                if (el.children.length < 1 || el.children.length > 20) return;

                var txt = (el.textContent || '');
                if (txt.length < 5 || txt.length > 800) return;
                ...
                // Skip if any direct child is already processed (we're the parent)
                var alreadyDone = Array.from(el.children).some(function (c) {
                    return c.getAttribute(ATTR) === '1';
                });
    ...
    function hookEvents() {
        ...
        // Activity open → light reset (only stream elements) + rescan
        ['full','activity','card'].forEach(function (ev) {
            Lampa.Listener.follow(ev, function (e) {
                if (e.type === 'complite' || e.type === 'open' || e.type === 'start') {
                    // Reset only elements inside known stream containers, not the whole page
                    var streamContainers = document.querySelectorAll(
                        '.online-prestige,.online-list,.torrent-list,' +
                        '[class*="online-prestige"],[class*="online-list"],[class*="torrent-list"]'
                    );
