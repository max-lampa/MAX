(function () {
    ...
    function CustomThemesPlugin() {
        ...
        var html = $('<div class="settings-list layer--wheight"></div>');
            ...
            myCustomStyles.forEach(function (item) {
                ...
                var line = $(`
                    <div class="settings-param selector">
                        ...
                        <div style="width: 12px; height: 12px; background: ${item.main}; border-radius: 50%; margin-left: 10px; border: 1px solid #fff;"></div>
                    </div>
                `);
                ...
                line.on('hover:enter', function () {
                    ...
                    Lampa.Noty.show('Применено: ' + item.title);
                    _this.activity.replace({ title: "Темы", component: "custom_themes_v10", page: 1 });
                });

                container.append(line);
                items_elements.push(line);
            });
        ...
        this.start = function () {
            ...
            Lampa.Controller.add("content", {
                toggle: function () {
                    ...
                    // Явное указание контроллеру на список элементов
                    Lampa.Controller.collectionFocus(lastFocus || container.find('.selector')[0], html);
                },
                left: function () { Lampa.Controller.toggle('menu'); },
                up: function () { Lampa.Controller.toggle("head"); },
                down: function () { 
                   // Принудительная навигация вниз если стандартная тупит
                },
    ...
    Lampa.Component.add('custom_themes_v10', CustomThemesPlugin);

    Lampa.Listener.follow('app', function (e) {
        if (e.type == 'ready') {
            Lampa.SettingsApi.addParam({
                component: "interface",
                param: { name: "custom_themes_fixed", type: "static" },
                field: { name: "Темы (Пульт FIX)", description: "Исправлена прокрутка всех цветов" },
                onRender: function (item) {
                    item.on("hover:enter", function () {
                        Lampa.Activity.push({ title: "Выбор темы", component: "custom_themes_v10", page: 1 });
                    });
                }
            });
        }
