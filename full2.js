(function() {
    ...
    // ---------- ТЕМЫ (только цвет и название) ----------
    const THEMES = [
        { title: 'Тёмная классика', color: '#69ffbd' },
        { title: 'Светлая тема', color: '#f0f0f0' },
        { title: 'Глубокий океан', color: '#004080' },
        { title: 'Зелёный лес', color: '#3d663d' },
        { title: 'Винная полночь', color: '#661a1a' },
        { title: 'Фиолетовый неон', color: '#4d1f7a' },
        { title: 'Матрица', color: '#00ff41' },
        { title: 'Киберпанк', color: '#ff0055' },
        { title: 'Мягкий шоколад', color: '#523a2f' },
        { title: 'Арктика', color: '#3498db' },
        { title: 'Королевское золото', color: '#d4af37' },
        { title: 'Графит', color: '#555555' },
        { title: 'Розовый фламинго', color: '#ff85a2' },
        { title: 'Кофейный микс', color: '#8e5d3e' },
        { title: 'Ядовитый плющ', color: '#00ff00' },
        { title: 'Синее пламя', color: '#0000ff' },
        { title: 'Оранжевый закат', color: '#ff6600' },
        { title: 'Морская пена', color: '#1abc9c' },
        { title: 'Баклажан', color: '#af7ac5' },
        { title: 'Сталь', color: '#85929e' },
        { title: 'Фиолетовый', color: '#8e44ad' },
        { title: 'Красный', color: '#e74c3c' },
        { title: 'Бордовый', color: '#8b0000' }
    ];

    // ---------- ГЕНЕРАЦИЯ ПОЛНОГО CSS ПО ЦВЕТУ ----------
    function generateFullCSS(color) {
        return `
            /* Подсветка фокуса (старое поведение) */
            ...
            /* Новые правила: заголовки получают цвет темы */
            .head__title,
            .category__title,
            .full-start__title,
            .info__title {
            ...
            /* Новые правила: обычные кнопки (не в фокусе) получают обводку и текст цвета темы */
            .simple-button,
            .full-start__button,
            .btn {
                color: ${color} !important;
            ...
            /* Для кнопок с фоном (например, основная кнопка плеера) можно оставить стандартным,
               но если хотите полностью перекрашивать – раскомментируйте строку ниже */
            /* .simple-button, .full-start__button, .btn { background: ${color}20 !important; } */
        `;
    }

    // ---------- РАБОТА С ХРАНИЛИЩЕМ ----------
    function getSavedThemeIndex() {
        // Сначала пробуем новый ключ
        let index = localStorage.getItem('max_theme_index');
        if (index !== null) return parseInt(index, 10);

        // Если есть старый ключ (CSS) – пытаемся извлечь индекс
        const oldCss = localStorage.getItem('max_theme_data');
        if (oldCss) {
            // Ищем тему, чей CSS совпадает со старым (грубое сравнение, ищем вхождение цвета)
            const matchedIndex = THEMES.findIndex(t => oldCss.includes(t.color));
            if (matchedIndex !== -1) {
                // Нашли – сохраняем индекс и удаляем старый ключ
                localStorage.setItem('max_theme_index', matchedIndex);
    ...
    function applyThemeByIndex(index) {
        ...
        if (!theme) return;
        const css = generateFullCSS(theme.color);
        // Удаляем предыдущий стиль и добавляем новый
        $('#max-style-tag').remove();
        $('<style id="max-style-tag">').html(css).appendTo('head');
        // Сохраняем индекс
        localStorage.setItem('max_theme_index', index);
    }

    // Применяем сохранённую тему при загрузке
    applyThemeByIndex(getSavedThemeIndex());

    // ---------- ГЕНЕРАЦИЯ ИКОНКИ (без изменений) ----------
    ...
    // Иконки для всех тем
    const themeIcons = THEMES.map(theme => generateIcon(theme.color));

    // ---------- ДОБАВЛЕНИЕ ИКОНКИ В ШАПКУ ----------
    function addThemeIcon() {
        const savedIndex = getSavedThemeIndex();
    ...
    function openThemeModal(iconElement) {
        ...
        resetBtn.on('hover:enter', function() {
            localStorage.removeItem('max_theme_index');
            localStorage.removeItem('max_theme_data'); // на всякий случай
            $('#max-style-tag').remove(); // удаляем кастомные стили
            iconElement.attr('src', generateIcon('#aaaaaa')); // серая иконка
            Lampa.Modal.close();
