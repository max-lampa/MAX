(function () {
  ...
  function open(data, card, host) {
    ...
    window.__ltDebug = get('lt_debug', false);
    if (window.__ltDebug) console.log('[LT] overlay open, items:', data.length);
  ...
  function getItemNodes(host) {
    ...
    if (!all.length) {
      var allDivs = [].slice.call(host.querySelectorAll('div,article,li,[role=listitem]'));
      all = allDivs.filter(function(n) { return looksLikeTorrentItem(n) && (!n.parentElement || !looksLikeTorrentItem(n.parentElement)); });
    }
    ...
    if (window.__ltDebug && all.length) console.log('[LT] found items:', all.length);
    return out;
  }
  ...
  function connect(force) {
    ...
    var h = findHost();
    if (!h) {
      if (window.__ltDebug) console.log('[LT] findHost failed');
      return;
    }
    if (window.__ltDebug) console.log('[LT] found host', h.className || h.tagName);
