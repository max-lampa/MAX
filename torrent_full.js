(function () {
  ...
  (function unifiedTorrentControls() {
    ...
    function metric(node, mode) {
      var el = $(node), text = mode === 'size' ? el.find('.torrent-item__size').text() :
        el.find('.torrent-item__seeds,.torrent-item__seeders').text();
      if (!text) text = el.text();
      var m = String(text).replace(',', '.').match(/(-?\d+(?:\.\d+)?)\s*(kb|kib|mb|mib|gb|gib|tb|tib)?/i);
      if (!m) return 0;
      var unit = (m[2] || '').toLowerCase();
      var power = {kb:1,kib:1,mb:2,mib:2,gb:3,gib:3,tb:4,tib:4}[unit] || 0;
      return parseFloat(m[1]) * Math.pow(1024, power);
    }
    function sort(mode) {
      var box = torrentRoot();
      if (!box.length) return;
      var cards = box.find('.nova-card').get();
      cards.sort(function(a,b) { return metric(b, mode) - metric(a, mode); });
