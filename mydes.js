(function () {
    ...
    function fetchExtendedData(id, type) {
        return new Promise(async function(resolve) {
            ...
            try {
                var langQuery = (langPref === 'ru' || langPref === 'ru_en') ? 'ru-RU'
                    : (sloganLang === 'uk' || sloganLang === 'uk_en' || descLang === 'uk' || descLang === 'uk_en' || langPref === 'uk' || langPref === 'uk_en') ? 'uk-UA'
                    : 'en-US';
    ...
    function injectCSS() {
        ...
        style.innerHTML = `
            ...
            .ydesign-active .card.focus .card__view {
                box-shadow: 0 0 0 var(--ydesign-cbw, 0.25em) #fff, 0 12px 30px rgba(0,0,0,0.9) !important;
            }
            ...
            .ydesign-active .card .card__view {
                ...
                box-shadow: 0 0 0 var(--ydesign-cbw, 0.25em) transparent;
                transition: background-color 0.5s ease, box-shadow 0.2s ease;
            }

            /* ── Цветная рамка постера (свой цвет из настроек YDesign) ── */
            body.ydesign-card-border .ydesign-active .card .card__view {
                box-shadow: 0 0 0 var(--ydesign-cbw, 0.20em) var(--ydesign-border-color, #2b7fff) !important;
            }
            body.ydesign-card-border .ydesign-active .card.focus .card__view {
                box-shadow: 0 0 0 var(--ydesign-cbw, 0.20em) var(--ydesign-border-color, #2b7fff),
    ...
    function applyDynamicCSS() {
        ...
        document.documentElement.style.setProperty('--ydesign-cbw', getSet('ydesign_card_border_width') + 'em');
        document.documentElement.style.setProperty('--ydesign-border-color', getSet('ydesign_card_border_color'));
    ...
    function createSettings() {
        ...
        Lampa.SettingsApi.addParam({ component: 'ydesign', param: { name: 'ydesign_slogan_lang', type: 'select', values: { 'uk':'Только Украинский', 'uk_en':'Укр (Англ. если нет)', 'en':'Только Английский' }, default: DefaultSettings.ydesign_slogan_lang }, field: { name: 'Язык слогана' } });
        Lampa.SettingsApi.addParam({ component: 'ydesign', param: { name: 'ydesign_desc_lang', type: 'select', values: { 'uk':'Только Украинский', 'uk_en':'Укр (Англ. если нет)', 'en':'Только Английский' }, default: DefaultSettings.ydesign_desc_lang }, field: { name: 'Язык описания под карточкой' } });
