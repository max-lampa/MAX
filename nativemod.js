  ...
  (function () {
    ...
    function injectStyle() {
      ...
      var text = [
        ...
        'body.' + BODY_CLASS + ' .agnative-topnav-clock { position:absolute; right:1.15em; top:.46em; z-index:20; display:inline-flex; align-items:center; justify-content:center; min-width:4.2em; height:2.6em; padding:0 .95em; border-radius:999px; background:rgba(22,24,30,.26); border:1px solid rgba(255,255,255,.10); box-shadow:inset 0 1px 0 rgba(255,255,255,.10), 0 8px 18px rgba(0,0,0,.12); backdrop-filter:blur(18px) saturate(140%); -webkit-backdrop-filter:blur(18px) saturate(140%); color:var(--lampa-accent,#568dff); text-shadow:0 0 12px var(--lampa-accent,#568dff), 0 1px 4px rgba(0,0,0,.5); font-size:.92em; font-weight:800; letter-spacing:.04em; }',
        ...
        '  background: var(--lampa-accent, rgba(86,141,255,.92)) !important;',
        '  color: #fff !important;',
    ...
    function buildRightdock() {
      ...
      if (!dock) {
        ...
        dock.className = 'agnative-rightdock';
      ...
      inner.className = 'agnative-rightdock__inner';
      ...
      Array.prototype.slice.call(sourceMenu.children).forEach(function (child) {
        ...
        if (child.classList.contains('menu__case')) {
          ...
          caseEl.className = 'agnative-rightdock__case';
          qsa('.menu__item.selector', child).forEach(function (item) {
            if (item.classList.contains('hidden')) return;
            var btn = document.createElement('div');
            btn.className = 'agnative-rightdock__item selector';
          ...
          split.className = 'agnative-rightdock__split';
      ...
      registerRightdockController(dock);
      syncRightdockActive();
      return dock;
    }
    ...
    function syncRightdockActive() {
      var dock = qs('.agnative-rightdock');
      if (!dock) return;
      qsa('.agnative-rightdock__item[data-action]', dock).forEach(function (btn) {
    ...
    function registerRightdockController(dock) {
      ...
      try {
        Lampa.Controller.add('agnative_rightdock', {
          toggle: function () {
            var d = qs('.agnative-rightdock');
            if (!d) return;
            showRightdock();
            var view = $(d);
            var current = qs('.agnative-rightdock__item.is-active', d)
              || qs('.agnative-rightdock__item.selector', d);
            Lampa.Controller.collectionSet(view);
            Lampa.Controller.collectionFocus(current || false, view, true);
          },
          update: function () { },
          gone: function () {
            hideRightdock(true, true);
          },
          up: function () {
