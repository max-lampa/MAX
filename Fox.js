(function () {
      ...
      if (item.ffprobe && Array.isArray(item.ffprobe)) {
        item.ffprobe.forEach(function(stream) {
          if (stream.codec_type === 'video') {
            ...
            if (stream.side_data_list && JSON.stringify(stream.side_data_list).includes('Vision')) best.dolbyVision = true;
  ...
  Lampa.Listener.follow('full', function(e) {
    ...
    if (description.length) {
      if (!$('.quality-badges-container').length) description.after('<div class="quality-badges-container"></div>');
  ...
  var style = '<style>\
    :root { --badge-size: 1.4em; }\
    .quality-badges-container { display: flex; gap: 0.4em; margin: 0.6em 0; min-height: var(--badge-size); pointer-events: none; flex-wrap: wrap; }\
    .quality-badge { height: var(--badge-size); opacity: 0; transform: translateY(8px); animation: qb_in 0.4s ease forwards; }\
    ...
    .card-quality-badge { height: calc(var(--badge-size) * 0.75); opacity: 0; transform: translateY(5px); animation: qb_in 0.3s ease forwards; }\
    ...
    @media (min-width: 1024px) { :root { --badge-size: 1.6em; } }\
  </style>';
  $('body').append(style);
