(function () {
  ...
  function parse(n) {
    ...
    var seasonField = pick(n, ['.torrent-item__season', '[class*="season"]']);
  ...
  function openNative(index) {
    ...
    var title = item.name;
    close(false);
    closedUntil = Date.now() + 1500;
    lastHost = null;
    lastSig = '';

    setTimeout(function () {
      try {
        // Lampa's torrent cards normally listen for hover:enter, not DOM click.
        // Restore the original node first, then send both compatible event forms.
        ...
        var sent = false;
        if (window.jQuery && window.jQuery(native).trigger) {
          window.jQuery(native).trigger('hover:enter');
          return true;
        }
        if (window.$ && window.$(native).trigger) {
          window.$(native).trigger('hover:enter');
          return true;
        }
        try {
          native.dispatchEvent(new CustomEvent('hover:enter', { bubbles: true, cancelable: true }));
        } catch (ignoreEvent) {}
  ...
  function findHost() {
    ...
    for (var i = 0; i < a.length; i++) {
      if (getItemNodes(a[i]).length) return a[i];
    }
