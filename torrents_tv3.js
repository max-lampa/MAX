(function () {
  ...
  window.nova_torrents_version = '10-tv';
  ...
  /* ------------------------- платформа / TV -------------------------- *
   * На Android TV нет тача: всё управление идёт через Lampa.Controller.
   * Поэтому важно знать, что мы на телевизоре, и не включать тяжёлую
   * анимацию и hover-стили, которых на пульте всё равно не видно.      */

  var tv_mode = null;

  function tvMode() {
    if (tv_mode !== null) return tv_mode;
    tv_mode = false;
    try {
      if (window.Lampa && Lampa.Platform && typeof Lampa.Platform.screen === 'function' && Lampa.Platform.screen('tv')) tv_mode = true;
    } catch (e) {}
    if (!tv_mode) {
      try {
        if (window.Lampa && Lampa.Platform && typeof Lampa.Platform.is === 'function' &&
          (Lampa.Platform.is('tizen') || Lampa.Platform.is('webos') || Lampa.Platform.is('nw') || Lampa.Platform.is('orsay'))) tv_mode = true;
      } catch (e) {}
    }
    if (!tv_mode) {
      try {
        var ua = String(navigator.userAgent || '').toLowerCase();
        if (/\btv\b|android tv|smart-?tv|aft[a-z]|bravia|shield|mibox|crkey|hisense|philips|netcast|dtv/.test(ua)) tv_mode = true;
      } catch (e) {}
    }
    if (!tv_mode) {
      try {
        if (!('ontouchstart' in window) && !navigator.maxTouchPoints && window.screen && window.screen.width >= 1280) tv_mode = true;
      } catch (e) {}
    }
    return tv_mode;
  }
  ...
  function markTvBody() {
    try {
      ...
      if (tvMode()) {
        if (cls.indexOf('nova-tor-tv') === -1) document.body.className = (cls + ' nova-tor-tv').replace(/^ +/, '');
      }
    } catch (e) {}
  }

  /* Один кадр вместо десятков: на пульте нажатия идут пачками, и без
     этого каждый шаг фокуса дёргал верстку заново. */
  ...
  function parseTitle(title) {
    ...
    /* Сезонные раздачи часто содержат диапазон серий:
       S01E01-08, S01E01-E08, S01 1-8 или «1 сезон: 1-8 серий».
       Раньше такой релиз превращался в одну строку S01, из-за чего
       пользователь видел только файлы сезона. */
    var episode_range = up.match(/\bS(\d{1,2})\s*E(\d{1,3})\s*[-–—]\s*E?(\d{1,3})\b/) ||
      up.match(/\bS(\d{1,2})\s+E?(\d{1,3})\s*[-–—]\s*(\d{1,3})\b/) ||
      up.match(/(\d{1,2})\s*СЕЗОН(?:А)?[^0-9]{0,12}(\d{1,3})\s*[-–—]\s*(\d{1,3})\s*СЕР(?:ИЯ|ИИ|ИЙ)?/);
    var season = episode_range || up.match(/\bS(\d{1,2}) ?E\d{1,3}\b/) || up.match(/\bS(\d{1,2})\b/) ||
    ...
    if (episode_range) {
      /* В русской форме первая группа — номер сезона, а во всех
         остальных формах — тоже сезон, поэтому серии берём со 2-й. */
      out.episode = parseInt(episode_range[2], 10) || out.episode;
      out.episode_end = parseInt(episode_range[3], 10) || out.episode;
    }
  ...
  /* Пульт сыпет нажатиями быстрее, чем браузер приставки успевает считать
     геометрию. Поэтому запоминаем только последнюю цель и двигаем скролл
     один раз за кадр: именно это раньше давало «тугую» реакцию. */
  function scrollTo(element) {
    var node = element && element[0] ? element[0] : element;
    if (!node) return;
    scroll_pending = node;
    if (scroll_queued) return;
    scroll_queued = true;
    nextFrame(function () {
      scroll_queued = false;
      var target = scroll_pending;
      scroll_pending = null;
      if (target) scrollNow(target);
    });
  }

  function scrollNow(node) {
     ...
     try {
       ...
       if (modal_panel.length && modal_panel[0]) {
         ...
         var margin = Math.max(24, Math.round(panel_rect.height * 0.18));
         var delta = 0;
         if (node_rect.top < panel_rect.top + margin) delta = node_rect.top - (panel_rect.top + margin);
         else if (node_rect.bottom > panel_rect.bottom - margin) delta = node_rect.bottom - (panel_rect.bottom - margin);
         if (delta) panel_node.scrollTop = Math.max(0, panel_node.scrollTop + delta);
         return;
       }
     } catch (e) {}
  ...
  function purgeLegacyNova() {
    var ids = ['nova-tor-css', 'nova-tor-css-v5', 'nova-tor-css-v7'];
  ...
  /* Скрытая родная раздача обязана перестать быть .selector: иначе
     на Android TV пульт уводит фокус в невидимые строки (у них нулевые
     координаты) и навигация «залипает». На тапе это незаметно. */
  function hideNative(node) {
    try {
      node.addClass('nova-tor-hidden').removeClass('focus');
      var list = node.filter('.selector').add(node.find('.selector'));
      list.each(function () {
        var item = $(this);
        item.addClass('nova-tor-unselector').removeClass('selector focus');
      });
    } catch (e) {}
  }

  function showNatives(box) {
    if (!box || !box.length) return;
    try {
      box.find('.nova-tor-unselector').addClass('selector').removeClass('nova-tor-unselector');
      if (box.hasClass('nova-tor-unselector')) box.addClass('selector').removeClass('nova-tor-unselector');
      box.find('.nova-tor-hidden').removeClass('nova-tor-hidden');
    } catch (e) {}
  }
  ...
  /* Lampa собирает список фокусируемых элементов один раз при сборке
     страницы. Мы дорисовываем свои .selector позже, поэтому коллекцию
     нужно пересобрать, иначе на пульте новые карточки просто не видны. */
  function refreshCollection(node) {
    if (!window.Lampa || !Lampa.Controller || typeof Lampa.Controller.collectionSet !== 'function') return;
    var container = null;
    try {
      var scroll = activeScroll(node || (ui.root && ui.root[0]) || (host && host[0]));
      if (scroll && typeof scroll.render === 'function') {
        var render = scroll.render();
        if (render && render.length) container = render;
      }
    } catch (e) {}
    if (!container || !container.length) {
      try {
        var box = $(node || (host && host[0])).closest('.scroll');
        if (box.length) container = box;
      } catch (e) {}
    }
    if ((!container || !container.length) && host && host.length) container = host;
    if (!container || !container.length) return;
    try { Lampa.Controller.collectionSet(container); } catch (e) {}
  }
  ...
  var IMAGE_CACHE_MAX_BYTES = 220000;
  var IMAGE_CACHE_MAX_ITEMS = 40;

  function imageCacheCount() {
    var count = 0;
    for (var key in image_cache) if (Object.prototype.hasOwnProperty.call(image_cache, key)) count++;
    return count;
  }
  ...
  function cacheImage(url) {
    ...
    /* На приставках памяти и квоты localStorage мало: base64-кадры быстро
       её забивают, поэтому держим кэш маленьким и не тянем тяжёлые файлы. */
    if (imageCacheCount() >= IMAGE_CACHE_MAX_ITEMS) return;
    ...
    xhr.onload = function () {
      ...
      reader.onload = function () {
        ...
        if (data_url.length > 16) {
          ...
          if (data_url.length <= IMAGE_CACHE_MAX_BYTES) localCacheSet(imageCacheKey(url), data_url);
  ...
  function hydrateCachedImages() {
    ...
    ui.root.find('img[data-nova-tor-frame]').each(function () {
      ...
      if (!url) return;
      /* На ТВ не лезем в localStorage за base64: разбор такого кэша на
         каждую картинку сам по себе подвешивал интерфейс. */
      var saved = tv ? image_cache[url] : (image_cache[url] || localCacheGet(imageCacheKey(url)));
      if (saved && saved !== 'loading') {
        image.removeAttr('data-nova-lazy');
        image.attr('src', saved);
      }
    });
  }
  ...
  function debugLog(label, value) {
    try {
      if (window.console && typeof console.log === 'function') {
        console.log('[Nova Torrents] ' + label + ':', value);
      }
    } catch (e) {}
  }
  ...
  function tmdbId() {
    ...
    var nested = [movie.ids, movie.external_ids, movie.source, movie.external, movie.ids_external];
    for (var n = 0; n < nested.length; n++) {
      if (!nested[n]) continue;
      var nested_value = nested[n].tmdb || nested[n].tmdb_id || nested[n].id_tmdb || nested[n].tmdb_external;
      if (nested_value != null && /^\d+$/.test(String(nested_value))) return String(nested_value);
    }
    /* Fallback: если всё равно не нашли, проверим весь объект фильма */
    for (var key in movie) {
      if (!Object.prototype.hasOwnProperty.call(movie, key)) continue;
      if (/tmdb_?id/.test(key) && /^\d+$/.test(String(movie[key]))) return String(movie[key]);
    }
    return '';
  }
  ...
  function tmdbRequest(url, done) {
    ...
    try {
      xhr.open('GET', url, true);
      xhr.timeout = 9000;
      xhr.ontimeout = function () { debugLog('tmdbRequest timeout', url.split('?')[0]); finish(null); };
      xhr.onerror = function () { debugLog('tmdbRequest error', url.split('?')[0]); finish(null); };
      xhr.send();
    } catch (e) { finish(null); }
  }
  ...
  /* Ответы TMDB приходят по одному на сезон, и раньше каждый из них
     полностью перерисовывал страницу, сбрасывая фокус пульта. Теперь
     копим их и рисуем один раз, сохранив выбранную карточку. */
  function softRender() {
    clearTimeout(soft_timer);
    soft_timer = setTimeout(function () {
      if (!host || !data) return;
      var calendar_open = ui.modal && ui.modal.hasClass('nova-tor-calendar-modal');
      if (ui.modal && !calendar_open) return;
      var keep = currentFocusKey();
      guard(function () { render(keep == null ? null : keep); }, 'soft render');
      if (calendar_open) guard(function () { openCalendarModal(true); }, 'calendar reopen');
    }, tvMode() ? 600 : 250);
  }
  ...
  function enrichEpisodePosters() {
    var key = tmdbKey();
    var id = tmdbId();
    debugLog('enrichEpisodePosters', 'key=' + (key ? '(set)' : 'MISSING') + ' id=' + (id || 'NOT_FOUND') + ' count=' + (data && data.all ? data.all.length : 0));
    ...
    for (var season_key in seasons) {
      ...
      (function (cache_key, season_no, season_items) {
        ...
        tmdbRequest(url, function (json) {
          ...
          debugLog('TMDB season response', 'season=' + season_no + ' episodes=' + (json && json.episodes ? json.episodes.length : 0));
  ...
  function posterHtml(item, cls) {
    ...
    var bar = progress > 0 ? '<div class="nova-tor-progress nova-tor-progress--' + (progress >= 90 ? 'done' : 'started') + '"><i style="width:' + progress + '%"></i><b>' + Math.round(progress) + '%</b></div>' : '';
     /* src подставляем позже: одновременная загрузка сотни кадров сериала
        и была главной причиной, почему приставка «думала» на каждый шаг. */
     return '<div class="' + cls + '">' + fallback + '<img data-nova-lazy="' + esc(url) + '" data-nova-tor-frame="' + esc(url) + '" onerror="this.style.display=\'none\'" />' + bar + '</div>';
  }
  ...
  function hydrateLazyNode(image) {
    if (!image || !image.length) return;
    var url = image.attr('data-nova-lazy') || '';
    if (!url) return;
    image.removeAttr('data-nova-lazy');
    var saved = image_cache[url];
    if (saved && saved !== 'loading') url = saved;
    else if (!tvMode()) cacheImage(image.attr('data-nova-tor-frame') || url);
    try { image.attr('src', url); } catch (e) {}
  }

  function hydrateLazyIn(node) {
    if (!node || !node.length) return;
    try {
      node.find('img[data-nova-lazy]').each(function () { hydrateLazyNode($(this)); });
      if (node.is && node.is('img[data-nova-lazy]')) hydrateLazyNode(node);
    } catch (e) {}
  }

  function startLazy(root) {
    clearTimeout(lazy_timer);
    if (!root || !root.length) return;
    var step = tvMode() ? 4 : 10;
    var pause = tvMode() ? 220 : 90;

    function pump() {
      if (!root || !root.length || !root.parent().length) return;
      var pending = root.find('img[data-nova-lazy]');
      if (!pending.length) return;
      pending.slice(0, step).each(function () { hydrateLazyNode($(this)); });
      lazy_timer = setTimeout(pump, pause);
    }
  ...
  function episodeLabel(item) {
    ...
    var end = item.info.episode_end;
    if (s && e && end && end > e && !item.virtual_episode) {
      return 'S' + (s < 10 ? '0' : '') + s + 'E' + (e < 10 ? '0' : '') + e + '–E' + (end < 10 ? '0' : '') + end;
    }
  ...
  function torrentDate(item) {
    var value = item && item.date ? clean(item.date) : '';
    if (!value) return '';
    var stamp = dateStamp(value);
    if (!stamp) return value;
    try {
  ...
  function calendarDate(item) {
    return item && item.tmdb_air_date ? premiereDate(item) : (torrentDate(item) || t('calendar_date_unknown'));
  }

  function calendarDateKey(item) {
    if (item && item.tmdb_air_date) return 'tmdb:' + String(item.tmdb_air_date);
    if (item && item.date) return 'torrent:' + String(item.date);
    return 'unknown';
  }

   /* ------------------- окна и управление пультом -------------------- *
    * Полноэкранные окна Nova раньше жили без своего контроллера: фокус
    * ставился «поверх» контроллера content. На телефоне это не мешает
    * (там тап), а на Android TV пульт продолжал ходить по списку под
    * окном, а Назад закрывал всю страницу. Теперь у окон свой
    * контроллер с up/down/left/right/back и своей коллекцией.          */

   var MODAL_CONTROLLER = 'nova_tor_modal';
   var modal_controller_ready = false;
   var modal_prev_controller = 'content';

   var modal_items = [];
   var modal_index = -1;

   /* Список фокусируемых строк собираем один раз при открытии окна.
      Раньше каждое нажатие пульта заново обходило весь DOM окна: на
      сериале с сотнями раздач это и есть та самая задержка. */
   function modalIndexList() {
     modal_items = [];
     modal_index = -1;
     if (!ui.modal) return;
     try {
       ui.modal.find('.selector').each(function () { modal_items.push(this); });
     } catch (e) { modal_items = []; }
   }

   function modalSelectors() {
     return modal_items.length ? $(modal_items) : $();
   }

   function modalFocused() {
     if (modal_index >= 0 && modal_items[modal_index]) return $(modal_items[modal_index]);
     for (var i = 0; i < modal_items.length; i++) {
       if ($(modal_items[i]).hasClass('focus')) { modal_index = i; return $(modal_items[i]); }
     }
     return $();
   }

   function modalSetFocus(target, index) {
     if (!target || !target.length) return;
     try {
       if (modal_index >= 0 && modal_items[modal_index]) $(modal_items[modal_index]).removeClass('focus');
       target.removeClass('nova-tor-hidden').addClass('focus');
     } catch (e) {}
     modal_index = index == null ? modal_items.indexOf(target[0]) : index;
     try { Lampa.Controller.collectionFocus(target[0], null); } catch (e) {}
     hydrateLazyIn(target);
     modalPrefetch(modal_index);
     scrollTo(target);
   }

   /* Кадры подгружаем только вокруг текущей строки. */
   function modalPrefetch(index) {
     var from = Math.max(0, index - 2);
     var to = Math.min(modal_items.length - 1, index + 6);
     for (var i = from; i <= to; i++) {
       if (modal_items[i]) hydrateLazyIn($(modal_items[i]));
     }
   }

   function modalMove(step) {
     if (!modal_items.length) return;
     var current = modalFocused();
     var index = current.length ? modal_index : -1;
     var next = index < 0 ? 0 : index + step;
     if (next < 0) next = 0;
     if (next > modal_items.length - 1) next = modal_items.length - 1;
     if (next === index) return;
     modalSetFocus($(modal_items[next]), next);
   }

   function ensureModalController() {
     if (modal_controller_ready) return true;
     if (!window.Lampa || !Lampa.Controller || typeof Lampa.Controller.add !== 'function') return false;
     try {
       Lampa.Controller.add(MODAL_CONTROLLER, {
         invisible: true,
         toggle: function () {
           if (!ui.modal) {
             try { Lampa.Controller.toggle(modal_prev_controller || 'content'); } catch (e) {}
             return;
           }
           try { Lampa.Controller.collectionSet(ui.modal); } catch (e) {}
           var target = modalFocused();
           if (!target.length && modal_items.length) target = $(modal_items[0]);
           modalSetFocus(target, target.length ? modal_items.indexOf(target[0]) : -1);
         },
         update: function () {},
         up: function () { modalMove(-1); },
         down: function () { modalMove(1); },
         left: function () {},
         right: function () {},
         enter: function () {
           var target = modalFocused();
           if (target.length) { try { target.trigger('hover:enter'); } catch (e) {} }
         },
         long: function () {
           var target = modalFocused();
           if (target.length) { try { target.trigger('hover:long'); } catch (e) {} }
         },
         back: function () { closeEpisodeModal(); }
       });
       modal_controller_ready = true;
     } catch (e) { modal_controller_ready = false; }
     return modal_controller_ready;
   }

   function bindModalKeys() {
     try {
       $(document).on('keydown.nova_tor', function (e) {
         if (!ui.modal || !e) return;
         var code = e.keyCode || e.which || 0;
         /* Escape, Back (Android), Return (Tizen), Back (webOS) */
         if (e.key === 'Escape' || code === 27 || code === 8 || code === 4 || code === 10009 || code === 461) {
   ...
   function activateModal(modal, first_key) {
     ui.modal = modal;
     modalIndexList();
     var target = first_key ? modal.find('[data-key="' + first_key + '"]').first() : $();
     if (!target.length) target = modal.find('.selector').first();
     modal_index = target.length ? modal_items.indexOf(target[0]) : -1;
     modalPrefetch(modal_index < 0 ? 0 : modal_index);
     bindModalKeys();
     try {
       if (window.Lampa && Lampa.Controller && typeof Lampa.Controller.enabled === 'function') {
         var active = Lampa.Controller.enabled();
         if (active && active.name && active.name !== MODAL_CONTROLLER) modal_prev_controller = active.name;
       }
     } catch (e) {}
     if (ensureModalController()) {
       modalSelectors().removeClass('focus');
       if (target.length) target.addClass('focus');
       try { Lampa.Controller.toggle(MODAL_CONTROLLER); } catch (e) { modalSetFocus(target); }
     } else {
       modalSetFocus(target);
     }
   }
   ...
   function openCalendarModal(reopen) {
    if (!data) return;
     ...
     debugLog('openCalendarModal', 'reopen=' + !!reopen + ' key=' + (tmdbKey() ? 'set' : 'NONE') + ' id=' + (tmdbId() || 'NONE'));
    ...
    /* Календарь строится из самих раздач, а не только из TMDB. */
    var entries = [];
    for (var i = 0; i < data.all.length; i++) {
      var source_item = data.all[i];
      if (source_item.info && source_item.info.season && source_item.info.episode) entries.push(source_item);
    }
    debugLog('calendar: torrent episode entries', entries.length + ' of ' + data.all.length);
    entries.sort(function (a, b) {
      var ad = dateStamp(a.tmdb_air_date || a.date);
      var bd = dateStamp(b.tmdb_air_date || b.date);
      if (!ad && bd) return 1;
      if (ad && !bd) return -1;
      return (ad - bd) || (a.info.season - b.info.season) || (a.info.episode - b.info.episode) || a.index - b.index;
    });
    var has_dates = false;
    for (var d = 0; d < entries.length; d++) {
      if (entries[d].tmdb_air_date || entries[d].date) { has_dates = true; break; }
    }
    if (!entries.length) {
      panel.append('<div class="nova-tor__empty"><div class="nova-tor__empty-title">' + esc(t('calendar_empty')) + '</div><div class="nova-tor__empty-text">' + esc(t('calendar_loading')) + '</div></div>');
    ...
    for (var e = 0; e < entries.length; e++) {
      var item = entries[e];
       var day = calendarDateKey(item);
      if (day !== last) {
        panel.append('<div class="nova-tor-calendar__day">' + esc(calendarDate(item)) + '</div>');
        last = day;
      }
      ...
      row.append('<div class="nova-tor-modal__ep">' + esc(episodeLabel(item)) + '</div>');
       row.append('<div class="nova-tor-modal__rowbody"><div class="nova-tor-modal__rowtitle">' + esc(item.tmdb_name || item.title) + '</div><div class="nova-tor-modal__rowmeta">' + esc(t('aired')) + ': ' + esc(calendarDate(item)) + ' · ' + metaLine(item) + '</div></div>');
    ...
    activateModal(modal, 'calendar-close');
  }

   function episodeRows(item) {
     var rows = [];
     var start = item && item.info ? parseInt(item.info.episode, 10) || 0 : 0;
     var end = item && item.info ? parseInt(item.info.episode_end, 10) || 0 : 0;

     /* Один torrent-пакет может содержать несколько серий. Для окна
        «Серии» создаём виртуальные строки, но сохраняем тот же origin:
        после выбора Lampa открывает родной список файлов и продолжение
        просмотра работает через её Timeline. */
     if (item && item.info && item.info.season && start && end > start) {
       ...
       for (var episode = start; episode <= end; episode++) {
         ...
         copy.info = {};
         for (var info_key in item.info) if (Object.prototype.hasOwnProperty.call(item.info, info_key)) copy.info[info_key] = item.info[info_key];
         copy.info.episode = episode;
         copy.info.episode_end = episode;
         copy.virtual_episode = true;
         copy.virtual_source = item;
         copy.virtual_number = episode;
         copy.index = String(item.index) + '-e' + episode;
         rows.push(copy);
       }
       return rows;
     }
   ...
   function episodeGroups() {
    ...
    for (var i = 0; i < list.length; i++) {
       var expanded = episodeRows(list[i]);
       for (var x = 0; x < expanded.length; x++) {
         var key = expanded[x].info.season ? String(expanded[x].info.season) : '0';
         if (!groups[key]) groups[key] = [];
         groups[key].push(expanded[x]);
       }
    }
    var out = [];
   ...
   function closeEpisodeModal(restore) {
    ...
    ui.modal = null;
     ...
     var back = modal_prev_controller || 'content';
     try {
       if (window.Lampa && Lampa.Controller && typeof Lampa.Controller.enabled === 'function') {
         var active = Lampa.Controller.enabled();
         if (active && active.name === MODAL_CONTROLLER) Lampa.Controller.toggle(back);
       } else {
  ...
  function openEpisodeModal() {
    if (!data) return;
     ...
     debugLog('openEpisodeModal', 'seasons=' + groups.length + ' data.all=' + data.all.length);
    ...
    for (var g = 0; g < groups.length; g++) {
      ...
      for (var i = 0; i < season.list.length; i++) {
        ...
        row.append(posterHtml(item, 'nova-tor-modal__poster'));
        row.append('<div class="nova-tor-modal__ep">' + esc(episodeLabel(item)) + '</div>');
         var row_title = item.virtual_episode ? (item.tmdb_name || 'Серия ' + item.virtual_number) : item.title;
         var row_meta = metaLine(item);
         if (item.virtual_episode) row_meta = 'Файл сезона · ' + row_meta;
         row.append('<div class="nova-tor-modal__rowbody"><div class="nova-tor-modal__rowtitle">' + esc(row_title) + '</div><div class="nova-tor-modal__rowmeta">' + esc(row_meta) + '</div></div>');
        row.append('<div class="nova-tor-modal__score">' + item.rate + '</div>');
          bind(row, function (chosen) { return function () { closeEpisodeModal(false); launch(chosen.virtual_source || chosen); }; }(item), function (chosen) { return function () { context(chosen.virtual_source || chosen); }; }(item));
        panel.append(row);
      }
    }
    modal.append(panel);
    ...
    activateModal(modal, 'modal-close');
  }
  ...
  function shimmerTitle(value) {
    ...
    for (var i = 0; i < text.length; i++) {
      ...
      if (ch === ' ') { html += ' '; continue; }
      var delay = '-' + ((index % 18) * 0.11).toFixed(2) + 's';
      html += '<span style="-webkit-animation-delay:' + delay + ';animation-delay:' + delay + '">' + esc(ch) + '</span>';
      index++;
    }
  ...
  function buildHero() {
    ...
    /* Раньше сам герой был .selector и содержал внутри ещё две кнопки-
       селектора. На тапе это работает, на пульте вложенные селекторы
       ломают навигацию, поэтому фокус живёт только на кнопках. */
    var hero = $('<div class="nova-tor-hero" data-key="hero"></div>');
    ...
    hero.on('click', function (e) {
      if (e && e.target === hero[0]) launch(best);
    });
    bind(hero.find('[data-key="hero-play"]'), function () { launch(best); }, function () { context(best); });
  ...
  function render(focus_key) {
    ...
    if (tvMode()) box.addClass('nova-tor--tv');
    var hero = buildHero();
    ...
    if (!data.list.length) {
      ...
      /* Первую порцию рисуем сразу, остальное догоняем следующим тиком:
         на Android TV сборка сотни карточек в один проход подвешивала UI. */
      var chunk = tvMode() ? 12 : 30;
      var first = Math.min(chunk, data.list.length);
      for (var i = 0; i < first; i++) list.append(buildCard(data.list[i], i + 1));
      box.append(list);
      if (data.list.length > first) queueRest(box, list, first, chunk);
    }
    ...
    for (var j = 0; j < natives.length; j++) {
      hideNative(natives[j]);
    }
    host.addClass('nova-tor-scope');
    host.append(box);
    hydrateCachedImages();

    /* Обязательно после вставки: без пересборки коллекции пульт не видит
       новые карточки, поэтому на Android TV страница выглядела мёртвой. */
    refreshCollection(box[0]);
    ...
    hydrateLazyIn(box.find('.nova-tor-hero__bg'));
    startLazy(box);

    autoplay();
  }

  /* Доклейка остатка списка небольшими порциями. */
  function queueRest(box, list, from, chunk) {
    clearTimeout(rest_timer);
    rest_timer = setTimeout(function () {
      if (!box || !box.length || !box.parent().length || ui.root !== box) return;
      if (!data || !data.list) return;
      var to = Math.min(from + chunk, data.list.length);
      mutating = true;
      for (var i = from; i < to; i++) list.append(buildCard(data.list[i], i + 1));
      refreshCollection(box[0]);
      setTimeout(function () { mutating = false; }, 0);
      if (to < data.list.length) queueRest(box, list, to, chunk);
    }, tvMode() ? 180 : 60);
  }

  /* Какая карточка сейчас под фокусом: чтобы пересборка списка не
     отбрасывала пользователя обратно на «лучшую раздачу». */
  function currentFocusKey() {
    if (!ui.root || !ui.root.length) return null;
    try {
      var node = ui.root.find('.focus[data-key]').first();
      return node.length ? node.attr('data-key') : null;
    } catch (e) { return null; }
  }
  ...
  function schedule() {
    ...
    timer = setTimeout(function () {
      /* Пока открыто окно Nova, пересборка списка убила бы фокус пульта. */
      if (ui.modal) { timer = setTimeout(schedule, 400); return; }
      guard(function () { draw(false); }, 'scheduled draw');
    }, tvMode() ? 260 : 120);
  }
   ...
   * Стандартная панель настроек Lampa - узкое окно по центру: на
   * телевизоре в него попадает 3-4 строки, и пультом это листать больно.
   * Пока открыт наш раздел, разворачиваем панель на весь экран.        */

  var SETTINGS_BODY_CLASS = 'nova-tor-settings-full';
  var settings_watch = null;

  function ourSettingsOpen() {
    try {
      var params = $('.settings-param[data-name^="nova_tor_"]');
      if (params.length) {
        var visible = params.filter(function () {
          var node = this;
          return !!(node.offsetWidth || node.offsetHeight || node.getClientRects && node.getClientRects().length);
        });
        if (visible.length) return true;
      }
      if ($('[data-component="nova_torrents"].focus, [data-component="nova_torrents_cache"].focus').length) return true;
    } catch (e) {}
    return false;
  }

  function toggleSettingsSkin(on) {
    try {
      var body = $('body');
      if (on) body.addClass(SETTINGS_BODY_CLASS);
      else body.removeClass(SETTINGS_BODY_CLASS);
    } catch (e) {}
  }

  function watchSettings() {
    clearInterval(settings_watch);
    var ticks = 0;
    settings_watch = setInterval(function () {
      var open = false;
      try { open = $('.settings').length > 0 && ourSettingsOpen(); } catch (e) {}
      toggleSettingsSkin(open);
      /* Настройки закрыли - слежку выключаем, чтобы не крутить таймер зря. */
      if (!$('.settings').length && ++ticks > 3) {
        clearInterval(settings_watch);
        settings_watch = null;
        toggleSettingsSkin(false);
      }
      if (open) ticks = 0;
    }, 400);
  }

  function hookSettingsSkin() {
    try {
      if (Lampa.Settings && Lampa.Settings.listener && typeof Lampa.Settings.listener.follow === 'function') {
        Lampa.Settings.listener.follow('open', function (e) {
          var name = e && (e.name || e.component) || '';
          toggleSettingsSkin(String(name).indexOf('nova_torrents') === 0);
          watchSettings();
        });
      }
    } catch (e) {}
    try {
      Lampa.Controller.listener.follow('toggle', function (e) {
        if (!e || !e.name) return;
        if (String(e.name).indexOf('settings') !== 0) return;
        watchSettings();
      });
    } catch (e) {}
  }
  ...
  var CSS = '' +
    ...
    '.nova-tor-hero__body{position:relative;max-width:70%;padding:2.2em}.nova-tor-hero__title{font-size:2.25em;font-weight:600;line-height:1.1;margin:.3em 0 .45em}.nova-tor-hero__title--shimmer span{display:inline-block;color:#f4f2ed;text-shadow:0 .06em .45em rgba(10,13,20,.88);-webkit-animation:novaLetterGlow 3.6s ease-in-out infinite;animation:novaLetterGlow 3.6s ease-in-out infinite}@-webkit-keyframes novaLetterGlow{0%,100%{color:#f4f2ed;text-shadow:0 .06em .45em rgba(10,13,20,.88);-webkit-transform:translateY(0)}45%{color:#f08a8e;text-shadow:0 0 .65em rgba(224,91,98,.72);-webkit-transform:translateY(-.025em)}60%{color:#f4f2ed;text-shadow:0 .06em .45em rgba(10,13,20,.88);-webkit-transform:translateY(0)}}@keyframes novaLetterGlow{0%,100%{color:#f4f2ed;text-shadow:0 .06em .45em rgba(10,13,20,.88);transform:translateY(0)}45%{color:#f08a8e;text-shadow:0 0 .65em rgba(224,91,98,.72);transform:translateY(-.025em)}60%{color:#f4f2ed;text-shadow:0 .06em .45em rgba(10,13,20,.88);transform:translateY(0)}}.nova-tor-hero__release{font-size:1em;opacity:.75;line-height:1.35}.nova-tor-hero__descr{max-width:46em;font-size:.95em;line-height:1.45;opacity:.72;margin-top:.8em;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden}.nova-tor-hero__descr span{opacity:.65}.nova-tor-hero__actions{display:-webkit-box;display:-webkit-flex;display:flex;-webkit-flex-wrap:wrap;flex-wrap:wrap;margin-top:1.2em}.nova-tor-hero__actions>div{margin:0 .6em .6em 0}.nova-tor-hero__play,.nova-tor-hero__episodes{padding:.65em 1.2em;border-radius:2em;background:rgba(255,255,255,.13)}.nova-tor-hero__play{background:#f4f2ed;color:#171b25}.nova-tor-hero__play.focus,.nova-tor-hero__episodes.focus{box-shadow:0 0 0 .15em #8fe0a4}' +
    ...
    /* На Android TV отключаем per-letter анимацию и hover-подсветку:
       старый WebView на этом проседает, а мыши там всё равно нет. */
    /* Настройки во весь экран, пока открыт раздел Nova. */
    'body.nova-tor-settings-full .settings{left:0!important;top:0!important;right:0!important;bottom:0!important;width:100%!important;height:100%!important;max-width:none!important;max-height:none!important;margin:0!important;padding:0!important;border-radius:0!important;-webkit-transform:none!important;transform:none!important;box-sizing:border-box}' +
    'body.nova-tor-settings-full .settings>*{max-height:none!important}' +
    'body.nova-tor-settings-full .settings__body,body.nova-tor-settings-full .settings__content,body.nova-tor-settings-full .settings__wrap{width:auto!important;max-width:none!important;height:100%!important;max-height:none!important}' +
    'body.nova-tor-settings-full .settings__content{-webkit-box-flex:1;-webkit-flex:1 1 auto;flex:1 1 auto}' +
    'body.nova-tor-settings-full .settings .scroll,body.nova-tor-settings-full .settings .scroll__content{height:100%!important;max-height:none!important}' +
    'body.nova-tor-settings-full .settings-param{padding-top:.9em;padding-bottom:.9em}' +
    'body.nova-tor-tv.nova-tor-settings-full .settings-param__name{font-size:1.15em}' +
    'body.nova-tor-tv.nova-tor-settings-full .settings-param__descr{font-size:.95em;opacity:.7}' +
    /* Телевизор: убираем всё, что стоит кадров и не видно с дивана. */
    'body.nova-tor-tv .nova-tor-card,body.nova-tor-tv .nova-tor__best,body.nova-tor-tv .nova-tor__chip,body.nova-tor-tv .nova-tor-modal__row{-webkit-transition:none!important;transition:none!important}' +
    'body.nova-tor-tv .nova-tor-card.focus,body.nova-tor-tv .nova-tor__chip.focus,body.nova-tor-tv .nova-tor__best.focus,body.nova-tor-tv .nova-tor-modal__row.focus{-webkit-box-shadow:none;box-shadow:inset 0 0 0 .14em #e05b62}' +
    'body.nova-tor-tv .nova-tor-hero__bg img{-webkit-filter:none;filter:none}' +
    'body.nova-tor-tv .nova-tor__best-poster{-webkit-mask-image:none;mask-image:none}' +
    'body.nova-tor-tv .nova-tor-hero__badge{-webkit-box-shadow:none;box-shadow:none}' +
    'body.nova-tor-tv .nova-tor-modal__panel{-webkit-overflow-scrolling:auto}' +
    '.nova-tor--tv .nova-tor-hero__title--shimmer span{-webkit-animation:none;animation:none;color:#f4f2ed}' +
    '.nova-tor--tv .nova-tor-card:not(.focus):hover,.nova-tor--tv .nova-tor__chip:not(.focus):hover,.nova-tor--tv .nova-tor-modal__row:not(.focus):hover{background:rgba(255,255,255,.05)}' +
    '.nova-tor--tv .nova-tor-card,.nova-tor--tv .nova-tor__chip,.nova-tor--tv .nova-tor__best{-webkit-transition:none;transition:none}' +
    '.nova-tor-modal__panel{-webkit-transform:translateZ(0)}' +
  ...
  function addCSS() {
    if (document.getElementById('nova-tor-css-v8')) return;
    var style = document.createElement('style');
    style.id = 'nova-tor-css-v8';
  ...
  function start() {
    ...
    try {
      Lampa.Controller.listener.follow('toggle', function (e) {
        if (e.name === MODAL_CONTROLLER) return;
