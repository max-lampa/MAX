(function () {
  ...
  function norm(x) {
    ...
    return {
      name: x.name || x.title || x.Title || x.filename || 'Раздача',
      quality: x.quality || x.resolution || 'FHD',
      size: x.size || x.Size || x.filesize || '0 ГБ',
  ...
  function openNative(index) {
    ...
    setTimeout(function () {
      try {
        ...
        } else if (window.$ && window.$(native).trigger) {
          window.$(native).trigger('hover:enter');
        } else if (typeof native.click === 'function') {
          native.click();
        } else {
  ...
  function getItemNodes(host) {
    ...
    return [].slice.call(host.querySelectorAll(items));
  }
