(function () {
  ...
  var OWN = {
    nova_plus_set_accent: { ru: 'Цвет акцента', uk: 'Колір акценту', en: 'Accent color' },
    nova_plus_set_accent_descr: { ru: 'Цвет полосы прогресса, процентов просмотра и кнопки просмотра', uk: 'Колір смуги прогресу, відсотків перегляду та кнопки перегляду', en: 'Color of the progress bar, watched percent and the watch button' },
    nova_plus_accent_default: { ru: 'Как в теме (белый)', uk: 'Як у темі (білий)', en: 'Theme default (white)' },
    nova_plus_accent_red: { ru: 'Красный (Netflix)', uk: 'Червоний (Netflix)', en: 'Red (Netflix)' },
    nova_plus_accent_crimson: { ru: 'Тёмно-красный', uk: 'Темно-червоний', en: 'Crimson' },
    nova_plus_accent_orange: { ru: 'Оранжевый', uk: 'Оранжевий', en: 'Orange' },
    nova_plus_accent_amber: { ru: 'Золотой', uk: 'Золотий', en: 'Amber' },
    nova_plus_accent_green: { ru: 'Зелёный', uk: 'Зелений', en: 'Green' },
    nova_plus_accent_teal: { ru: 'Бирюзовый', uk: 'Бірюзовий', en: 'Teal' },
    nova_plus_accent_blue: { ru: 'Синий', uk: 'Синій', en: 'Blue' },
    nova_plus_accent_violet: { ru: 'Фиолетовый', uk: 'Фіолетовий', en: 'Violet' },
    nova_plus_accent_pink: { ru: 'Розовый', uk: 'Рожевий', en: 'Pink' },
    nova_plus_accent_custom: { ru: 'Свой цвет (HEX)', uk: 'Свій колір (HEX)', en: 'Custom (HEX)' },
    nova_plus_set_accent_hex: { ru: 'Свой HEX-цвет', uk: 'Свій HEX-колір', en: 'Custom HEX color' },
    nova_plus_set_accent_hex_descr: { ru: 'Например #E50914 или E50914. Работает при выборе «Свой цвет (HEX)»', uk: 'Наприклад #E50914 або E50914. Працює при виборі «Свій колір (HEX)»', en: 'For example #E50914 or E50914. Used when "Custom (HEX)" is selected' },
  ...
  var ACCENT_KEY = 'nova_plus_accent';
  var ACCENT_HEX_KEY = 'nova_plus_accent_hex';
  var ACCENT_FALLBACK = '#E50914';

  var ACCENTS = {
    'default': null,
    red: ['#E50914', '#F40612'],
    crimson: ['#B00710', '#D40D18'],
    orange: ['#FF6A00', '#FF8524'],
    amber: ['#F5B301', '#FFC62B'],
    green: ['#28A745', '#34C759'],
    teal: ['#00BFA5', '#1AD7BD'],
    blue: ['#0A84FF', '#3D9BFF'],
    violet: ['#8B5CF6', '#A47BFF'],
    pink: ['#FF3D77', '#FF5C8F']
  };

  function cleanHex(value) {
    var raw = String(value == null ? '' : value).replace(/[^0-9a-fA-F]/g, '');
  ...
  function liftHex(hex, amount) {
    var out = '#';
    for (var i = 0; i < 3; i++) {
      var part = parseInt(hex.substr(1 + i * 2, 2), 16);
      part = Math.round(part + (255 - part) * amount);
      if (part < 0) part = 0;
      if (part > 255) part = 255;
      var piece = part.toString(16).toUpperCase();
      out += piece.length < 2 ? '0' + piece : piece;
    }
    return out;
  }

  function accentPair() {
    var key = get(ACCENT_KEY, 'red');

    if (key === 'custom') {
      var hex = cleanHex(get(ACCENT_HEX_KEY, ACCENT_FALLBACK)) || ACCENT_FALLBACK;
      return [hex, liftHex(hex, 0.18)];
    }

    if (!ACCENTS.hasOwnProperty(key)) key = 'red';
    return ACCENTS[key];
  }
  ...
  function accentCSS() {
    ...
    return [
      '.nova-hero__progress .time-line>div,' +
      '.nova-plus-root .nova-hero__progress .time-line>div,' +
      '.nova-card__line .time-line>div,' +
      '.nova-card__line--body .time-line>div,' +
      '.nova-plus-root .nova-card__line>.time-line>div,' +
      '.nova-plus-root .nova-card__line--body>.time-line>div,' +
      '.nova-card.focus .nova-card__line--body .time-line>div,' +
      '.nova-card--slim.focus .nova-card__line .time-line>div,' +
      'body.nova-plus-focus-ring .nova-card.focus .nova-card__line--body .time-line>div,' +
      'body.nova-plus-focus-ring .nova-card--slim.focus .nova-card__line .time-line>div' +
      '{background:' + main + '!important}',

      '.nova-btn--main,.nova-plus-root .nova-btn--main,.nova-toolbar .nova-btn--main,' +
      'body.nova-plus-focus-ring .nova-btn--main' +
      '{background:' + main + '!important;color:#fff!important;-webkit-box-shadow:none!important;box-shadow:none!important}',

      '.nova-btn--main.focus,.nova-plus-root .nova-btn--main.focus,.nova-toolbar .nova-btn--main.focus,' +
      'body.nova-plus-focus-ring .nova-btn--main.focus' +
      '{background:' + soft + '!important;color:#fff!important;-webkit-box-shadow:0 .25em .9em rgba(0,0,0,.45)!important;box-shadow:0 .25em .9em rgba(0,0,0,.45)!important}',

      '.nova-btn--main>svg,.nova-btn--main .nova-btn__label{color:#fff!important}',

      '.nova-card__pct,' +
      '.nova-plus-root .nova-card__pct,' +
      '.nova-plus-root .nova-card__strip>.nova-card__pct,' +
      '.nova-plus-root .nova-card--wide.focus .nova-card__pct,' +
      'body:not(.nova-plus-focus-ring) .nova-plus-root .nova-card--wide.focus .nova-card__pct' +
  ...
  function applyAccent() {
    try {
      var node = document.getElementById('nova-plus-accent-css');
      if (!node) {
        node = document.createElement('style');
        node.id = 'nova-plus-accent-css';
        (document.body || document.head).appendChild(node);
      }
      node.textContent = accentCSS();
    } catch (e) {}
  }

  function accentHexRow(body) {
    try {
      var scope = body && body.length ? body : $('.settings__content');
      var row = scope.find('[data-name="' + ACCENT_HEX_KEY + '"]');
      if (!row.length) return;

      if (get(ACCENT_KEY, 'red') === 'custom') row.removeClass('hide');
      else row.addClass('hide');

      var value = row.find('.settings-param__value');
      if (value.length) value.text(cleanHex(get(ACCENT_HEX_KEY, ACCENT_FALLBACK)) || ACCENT_FALLBACK);
    } catch (e) {}
  }
  ...
  function accentSettings() {
    try {
      var values = {};
      values['default'] = label('nova_plus_accent_default');
      values.red = label('nova_plus_accent_red');
      values.crimson = label('nova_plus_accent_crimson');
      values.orange = label('nova_plus_accent_orange');
      values.amber = label('nova_plus_accent_amber');
      values.green = label('nova_plus_accent_green');
      values.teal = label('nova_plus_accent_teal');
      values.blue = label('nova_plus_accent_blue');
      values.violet = label('nova_plus_accent_violet');
      values.pink = label('nova_plus_accent_pink');
      values.custom = label('nova_plus_accent_custom');

      Lampa.SettingsApi.addParam({
        component: 'nova_plus',
        param: { name: ACCENT_KEY, type: 'select', values: values, default: 'red' },
        field: {
          name: label('nova_plus_set_accent'),
          description: label('nova_plus_set_accent_descr')
        },
        onChange: function () {
          applyAccent();
          accentHexRow(null);
      ...
      Lampa.SettingsApi.addParam({
        component: 'nova_plus',
        param: { name: ACCENT_HEX_KEY, type: 'input', values: '', placeholder: ACCENT_FALLBACK, default: ACCENT_FALLBACK },
        field: {
          name: label('nova_plus_set_accent_hex'),
          description: label('nova_plus_set_accent_hex_descr')
        },
        onChange: function (value) {
          var hex = cleanHex(value);
          if (!hex) hex = cleanHex(get(ACCENT_HEX_KEY, ACCENT_FALLBACK)) || ACCENT_FALLBACK;
          save(ACCENT_HEX_KEY, hex);
          applyAccent();
          accentHexRow(null);
        }
      });
