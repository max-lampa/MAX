(function () {
  ...
  function component(object) {
    ...
    // Больше не нужна сложная логика фильтрации
    ...
    function applyUserSources(list) {
      // Игнорируем пользовательские сортировки, так как у нас только один источник
      return list;
    }
    function getBaseSources() {
      // Возвращаем только Filmix
      return ['filmix'];
    }
    function filterEnabledSources(list) {
      return list; // нет фильтрации
    }
    ...
    function getEnabledSources() {
      return null; // отключаем загрузку с сервера
    }
    function loadAvailableSources(call) {
      // Ничего не делаем, сразу вызываем call
      call();
    }
    ...
    this.initialize = function () {
      var _this = this;
      // Загружать источники с сервера не нужно, сразу устанавливаем filmix
      filter_sources = ['filmix'];
    ...
    this.createSource = function () {
      ...
      if (last_select_balanser[object.movie.id]) {
        ...
        balanser = Lampa.Storage.get('bandera_online_balanser', 'filmix');
      }
      if (!sources[balanser] || filter_sources.indexOf(balanser) === -1) {
        balanser = filter_sources[0] || 'filmix';
      }
  ...
  function initSettings() {
    ...
    // Убираем настройки источников, так как теперь только Filmix
    /*
    SettingsApi.addParam({
      component: 'bandera_online',
      param: {
