 ...
 *
 * Исправления:
 * - Полная поддержка пульта D‑pad (Android TV / Android 9)
 * - Исправлен скролл вниз между рядами каналов
 * - Убрана буферизация и предзагрузка YouTube плеера (мгновенный старт)
 * - Улучшена навигация: фокус всегда на первом видео при переключении строки
 */
(function () {
    'use strict';
    ...
    // =========================================================
    // YouTube iframe player helper (без буферизации и лоадера)
    // =========================================================
    var ytApiReady = false;
    var ytPendingCallbacks = [];

    function onYouTubeIframeAPIReady() {
        ytApiReady = true;
        for (var i = 0; i < ytPendingCallbacks.length; i++) {
            ytPendingCallbacks[i]();
        }
        ytPendingCallbacks = [];
    }

    // Предзагружаем API при старте плагина
    ...
    function playYouTubeCustom(key) {
        ...
        var initPlayer = function () {
            new YT.Player('yt-player-custom', {
                ...
                playerVars: {
                    ...
                    disablekb: 1,
                    fs: 1,
                    preload: 0,          // відключає попереднє завантаження
                    start: 0
                },
                events: {
                    onReady: function (event) {
                        event.target.playVideo(); // миттєвий старт
                    },
        ...
        if (ytApiReady) {
            ...
            ytPendingCallbacks.push(initPlayer);
        }
    }
    ...
    function KinoLine(data) {
        ...
        // Установить фокус на первый элемент строки
        ...
        this.create = function () {
            ...
            // Устанавливаем начальный фокус на первое видео, если строка станет активной
            if (items.length) {
                last = items[0].render();
            }
        };
        ...
        this.toggle = function () {
            ...
            Lampa.Controller.add('items_line', {
                toggle: function () {
                    ...
                    var focusTarget = last ? last : (items.length ? items[0].render() : null);
    ...
    function KinoComponent(object) {
        ...
        this.down = function () {
            if (active + 1 < items.length) {
                ...
                // Устанавливаем фокус на первый элемент новой строки
                items[active].focusFirst();
            }
        };
        ...
        this.up = function () {
            if (active > 0) {
                ...
                // Если уже на первой строке, передаём управление вверх (например, на заголовок)
