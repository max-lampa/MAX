(function () {
  ...
  function foreignSkinPresent(node) {
    ...
    var selectors = '.nova,.nova-skin-root,.z01,.z01__rows,.z01__list,.lampaconline,.onlyonline,.online';
  ...
  function readItem(node) {
    ...
    return {
      ...
      image: (raw && (raw.image || raw.poster || raw.img)) || origin.find('img').first().attr('src') || '',
  ...
  function tmdbId() {
    ...
    var keys = ['tmdb_id', 'tmdbId', 'id_tmdb', 'id'];
    for (var i = 0; i < keys.length; i++) {
  ...
  function enrichEpisodePosters() {
    var key = tmdbKey();
    var id = tmdbId();
    if (!key || !id || !data || !data.all || !data.all.length) return;
  ...
  function posterUrl(item) {
    ...
    if (!url) url = imageUrl(page_info && page_info.movie);
    return String(url || '');
  }

  function posterHtml(item, cls) {
    var url = posterUrl(item);
    if (!url) return '<div class="' + cls + ' ' + cls + '--fallback"><span>▶</span></div>';
    ...
    return '<div class="' + cls + '"><img src="' + esc(url) + '" data-nova-tor-frame="' + esc(url) + '" loading="lazy" onerror="this.style.display=\'none\'" />' + bar + '</div>';
  }
  ...
  function imageUrl(movie) {
    var url = '';
    try {
      if (movie && movie.backdrop_path) url = movie.backdrop_path;
      else if (movie && movie.backdrop) url = movie.backdrop;
      else if (movie && movie.background) url = movie.background;
      else if (movie && movie.poster_path) url = movie.poster_path;
  ...
  function openCalendarModal() {
    ...
    if (!entries.length) panel.append('<div class="nova-tor__empty"><div class="nova-tor__empty-title">' + esc(t('calendar_empty')) + '</div><div class="nova-tor__empty-text">' + esc(t('tmdb_loading')) + '</div></div>');
    var last = '';
    for (var e = 0; e < entries.length; e++) {
      var item = entries[e];
      var day = String(item.tmdb_air_date);
      if (day !== last) {
      ...
      row.append('<div class="nova-tor-modal__rowbody"><div class="nova-tor-modal__rowtitle">' + esc(item.tmdb_name || item.title) + '</div><div class="nova-tor-modal__rowmeta">' + esc(t('aired')) + ': ' + esc(item.tmdb_air_date) + ' · ' + metaLine(item) + '</div></div>');
      row.append('<div class="nova-tor-modal__score">' + item.rate + '</div>');
      bind(row, function (chosen) { return function () { closeEpisodeModal(); launch(chosen); }; }(item), function (chosen) { return function () { context(chosen); }; }(item));
      panel.append(row);
    }
    modal.append(panel);
    $('body').append(modal);
  ...
  function settings() {
    ...
    try {
      ...
      param({ name: STORE.tmdb_key, type: 'input', default: '' },
        { name: t('tmdb_key'), description: t('tmdb_key_descr') }, refresh);
  ...
  var CSS = '' +
    '.nova-tor-modal{position:fixed;inset:0;background:rgba(8,10,16,.88);z-index:700;display:flex;align-items:center;justify-content:center;padding:3em}' +
    '.nova-tor-modal__panel{width:min(62em,94vw);max-height:88vh;overflow:auto;background:#171b25;color:#f4f2ed;border-radius:1.2em;padding:1.5em;box-shadow:0 1em 4em rgba(0,0,0,.55)}' +
    '.nova-tor-modal__head{display:flex;justify-content:space-between;align-items:flex-start;gap:1em;margin-bottom:1em}' +
    ...
    '.nova-tor-calendar__day{font-size:.85em;letter-spacing:.1em;text-transform:uppercase;color:#e05b62;margin:1.2em 0 .55em;padding-bottom:.35em;border-bottom:1px solid rgba(224,91,98,.28)}' +
    '.nova-tor-modal__season{font-size:.8em;letter-spacing:.12em;text-transform:uppercase;opacity:.5;margin:1.3em 0 .55em}' +
    '.nova-tor-modal__row{display:flex;align-items:center;gap:1em;padding:.8em;border-radius:.75em;background:rgba(255,255,255,.05);margin-bottom:.45em}' +
    ...
    '.nova-tor-hero{position:relative;min-height:19em;overflow:hidden;border-radius:1.2em;margin-bottom:1.2em;background:#202632}' +
    '.nova-tor-hero__bg,.nova-tor-hero__shade{position:absolute;inset:0}.nova-tor-hero__bg img{width:100%;height:100%;object-fit:cover;opacity:.75}' +
    ...
    '@media screen and (max-width:580px){.nova-tor-hero{min-height:17em}.nova-tor-hero__body{max-width:100%;padding:1.3em}.nova-tor-hero__title{font-size:1.7em}.nova-tor__best-poster{width:9em;opacity:.25}.nova-tor__best-cap,.nova-tor__best-title,.nova-tor__best-badges,.nova-tor__best-meta,.nova-tor__best-play{max-width:100%}.nova-tor-modal{padding:1em}.nova-tor-modal__panel{max-height:92vh;padding:1em}.nova-tor-modal__rowmeta{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}}' +
    ...
    '.nova-tor__best-poster,.nova-tor-card__poster,.nova-tor-modal__poster{overflow:hidden;background:#2b3341;position:relative;flex-shrink:0}.nova-tor__best-poster img,.nova-tor-card__poster img,.nova-tor-modal__poster img{display:block;width:100%;height:100%;object-fit:cover}.nova-tor__best-poster{position:absolute;inset:0 0 0 auto;width:18em;opacity:.45}.nova-tor__best-poster--fallback,.nova-tor-card__poster--fallback,.nova-tor-modal__poster--fallback{display:flex;align-items:center;justify-content:center;color:#8fe0a4}.nova-tor__best-body{position:relative}' +
