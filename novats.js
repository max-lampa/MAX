(function () {
  ...
  function readItem(node) {
    ...
    return {
      ...
      image: (raw && (raw.image || raw.poster || raw.img || raw.poster_path || raw.backdrop_path)) || origin.find('img').first().attr('src') || origin.attr('data-poster') || origin.attr('data-image') || '',
  ...
  function pagePoster() {
    ...
    try {
      var found = page_info && page_info.root ? page_info.root.find('img').not('.torrent-item img').first() : $();
      return found && found.length ? normalizeImageUrl(found.attr('src') || found.attr('data-src') || '') : '';
    } catch (e) { return ''; }
  }
