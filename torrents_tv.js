(function () {
  ...
  window.nova_torrents_version = '7-tv';
  ...
  function premiereDate(item) {
   ...
   function modalSelectors() {
     if (!ui.modal) return $();
     try { return ui.modal.find('.selector').not('.nova-tor-hidden'); } catch (e) { return $(); }
   }

   function modalFocused() {
     var list = modalSelectors();
     var focused = list.filter('.focus').first();
     return focused.length ? focused : $();
   }

   function modalSetFocus(target) {
     ...
     try { target.trigger('hover:focus'); } catch (e) {}
     scrollTo(target);
   }
   ...
   function modalMove(step) {
     ...
     var index = current.length ? list.index(current) : -1;
     var next = index < 0 ? 0 : index + step;
     if (next < 0) next = 0;
     if (next > list.length - 1) next = list.length - 1;
     modalSetFocus(list.eq(next));
   }
     ...
     try {
       Lampa.Controller.add(MODAL_CONTROLLER, {
         ...
         toggle: function () {
           ...
           if (!target.length) target = modalSelectors().first();
