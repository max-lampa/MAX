(function(){
  ...
  // ======================================================
  //  JacRedRU v2.0 (перероблено на основі PlayUa)
  //  • Парсер: jac.red (Jacred API)
  //  • Пріоритет пошуку за оригінальною назвою (англ.)
  //  • Full HD фільтр + можливість вимкнути
  //  • Стабільні модальні вікна з фокусом
  //  • Групування серій по папках/релізах
  //  • Логування в консоль
  // ======================================================

  // ---------- Constants & helpers ----------
  var MIN_SEEDERS  = 1;
  var MIN_EP_MB    = 500;   // можна змінити через Lampa.Storage.field('jacred_min_ep_mb')
  try {
  ...
  function noty(m, t) { try{ if(window.Lampa && Lampa.Noty) Lampa.Noty.show(m, {time: t||2500}); }catch(e){} }
  function ensureScheme(u) { return /^https?:\/\//i.test(u) ? u : ('https://'+u); }
  function trimEnd(s) { return String(s||'').replace(/\/+$/,''); }
  function safeName(s) { return (s||'video').replace(/[^\w\d]+/g,'.').replace(/\.+/g,'.').replace(/^\.+|\.+$/g,'') || 'video'; }
  function isSerial(m) { return !!(m && m.first_air_date && !m.release_date); }
  function looksLikeVideo(p) { return VIDEO_EXT.test(String(p||'')); }
  function isFullHD(t) { return FHD_RE.test(String(t||'')); }
  function humanSize(b) { if(!b) return ''; var u=['B','KB','MB','GB','TB'],i=0,n=Number(b); while(n>=1024 && i<u.length-1){ n/=1024; i++; } return (i>=2? n.toFixed(1):Math.round(n))+' '+u[i]; }
  ...
  function parseJacredJSON(json) {
    ...
    var items = arr.map(function(x){
      ...
      return { title:title, link:link, magnet:magnet, size:size, seed:seed };
  ...
  async function jacSearch(query, isSerial) {
    ...
    // 1) JSON API з типом (якщо задано)
    try{
    ...
    }catch(e){ console.warn('[JacRedRU] Помилка запиту 1', e); }
    ...
    if(!results.length){
      try{
        ...
        console.log('[JacRedRU] Fallback (без type):', url2);
    ...
    console.log('[JacRedRU] Знайдено результатів:', results.length);
    return results;
  }
  ...
  function tsBase() {
    ...
    if(!raw) throw new Error('Укажіть torrserver_url у Налаштуваннях');
    return trimEnd(ensureScheme(raw));
  }
  ...
  function openModal(title) {
    ...
    function setLoading(text) {
      if(usingLampa){
        var row = $('<div class="jacredru-loader"><div style="width:26px;height:26px;border:3px solid rgba(255,255,255,.35);border-top-color:#fff;border-radius:50%;animation:jrloader .8s linear infinite;flex:0 0 auto"></div><div>'+(text||'Завантаження...')+'</div></div>');
        ...
        var d = document.createElement('div'); d.className='jacredru-loader'; d.textContent=String(text||'Завантаження...');
  ...
  function makePackHeader(text) {
    var $ = window.$ || window.jQuery;
    if($) return $('<div class="jacredru-pack-title"><span class="jacredru-folder">'+(text||'Реліз')+'</span></div>');
    var div = document.createElement('div'); div.className='jacredru-pack-title';
    var badge = document.createElement('span'); badge.className='jacredru-folder'; badge.textContent=String(text||'Реліз');
  ...
  async function showSeasons(meta) {
    var query = meta.orig; // тільки оригінальна назва (англ.)
    noty('JacRedRU: пошук сезонів — '+query);
    console.log('[JacRedRU] Пошук сезонів для:', query);

    var items = await searchFHD(query, true);
    if(!items.length) {
      items = await searchFHD(query, undefined); // без type
    }
    if(!items.length) throw new Error('Нічого не знайдено на jac.red для "'+query+'"');
    ...
    // Завантажуємо постери сезонів з TMDB
    var posters = {};
    ...
    var dlg = openModal('ВИБЕРІТЬ СЕЗОН');
    var $ = window.$ || window.jQuery;
    ...
    for(var si=0; si<seasonNums.length; si++){
      ...
      var subText = 'Варіантів: '+rels.length + (fhdCount ? ' • Full HD: '+fhdCount : '');
  ...
  async function fetchEpisodesAggregated(meta, sn, releases) {
    var dlg = openModal('СЕЗОН '+sn+' — епізоди');
    dlg.setLoading('Підготовка релізів...');
    ...
    for(var oi=0; oi<order.length; oi++){
      ...
      for(var ii=0; ii<items.length; ii++){
        var it = items[ii];
        var rowEl = makeEpisodeRow({season:sn, ep:it.ep}, names[it.ep]||('Серія '+it.ep), stills[it.ep]||'', humanSize(it.size), folder);
    ...
    if(!frag.children.length){
      dlg.setLoading('Не знайдено серій ≥'+MIN_EP_MB+'MB. Спробуйте інший реліз.');
      return;
    }
    dlg.setItems(frag);
  }
  ...
  async function runMovie(data) {
    ...
    if(!items.length) throw new Error('Нічого не знайдено в Full HD на jac.red');

    var best = items[0];
    var addLink = best.magnet || best.link;

    noty('JacRedRU: знайдено ' + (isFullHD(best.title) ? 'Full HD' : 'HD') + ' реліз');
