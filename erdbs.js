...
(function () {
    ...
    //  РЕЖИМ ПУБЛИКАЦИИ
    // ------------------------------------------------------------
    //  Поставьте EMBED_TOKEN = false ПЕРЕД ТЕМ, как выкладывать
    //  плагин публично (GitHub, форумы, чужие устройства). Тогда
    //  встроенный токен не будет использоваться, и каждый пользователь
    //  будет обязан добавить свой токен через панель «ERDB → Профили».
    //
    //  Для личного использования оставьте EMBED_TOKEN = true.
    // ============================================================
    var EMBED_TOKEN = true;
    var DEFAULT_TOKEN = 'Tk-fd1a36943472bbb5a5ec1b497e48a1aaef9222fcfc89787c';
    function embeddedToken() {
        return EMBED_TOKEN ? DEFAULT_TOKEN : '';
    }
    var DEFAULTS = {
        token: '',
        base: DEFAULT_BASE,
    ...
    function migrateLegacy() {
        ...
        // 1) Старый одиночный токен → профиль "Основной"
        ...
        // 2) Если профилей всё ещё нет, но в плагине зашит токен (EMBED_TOKEN=true) —
        //    создаём первый профиль автоматически, чтобы плагин работал
        //    "из коробки" сразу после установки.
        var embedded = embeddedToken();
        if (!list.length && tokenOk(embedded)) {
            list = [{
                id: uid(),
                name: 'По умолчанию (…' + embedded.slice(-4) + ')',
                token: embedded,
                base: DEFAULT_BASE
            }];
            saveProfiles(list);
    ...
    function activeToken() {
        ...
        if (tokenOk(legacy)) return legacy;
        // Финальный фолбэк — встроенный токен по умолчанию (только если EMBED_TOKEN=true).
        var embedded = embeddedToken();
        return tokenOk(embedded) ? embedded : '';
    }
    ...
    // На время подгрузки постера через прокси вешаем тонкую оранжевую
    // полоску прогресса + лёгкий фейд картинки. Когда image готова —
    // плавно показываем её (opacity 0→1) и снимаем индикатор.

    function findCardHost(imgEl) {
        if (!imgEl) return null;
        var node = imgEl.parentNode;
        while (node && node !== document) {
            if (node.classList && (
                node.classList.contains('card') ||
                node.classList.contains('card__view') ||
                node.classList.contains('full-start__poster') ||
                node.classList.contains('full-start-new__poster')
            )) return node;
            node = node.parentNode;
        }
        return imgEl.parentNode;
    }

    function showLoading(imgEl) {
        var host = findCardHost(imgEl);
        if (!host || host.querySelector('.erdb-loading')) return;
        try {
            // относительное позиционирование, чтобы прогресс прилип к карточке
            var cs = window.getComputedStyle(host);
            if (cs && cs.position === 'static') host.style.position = 'relative';
        } catch (e) {}
        var bar = document.createElement('div');
        bar.className = 'erdb-loading';
        bar.innerHTML = '<div class="erdb-loading__bar"></div>';
    ...
    function hideLoading(imgEl) {
        var host = findCardHost(imgEl);
        if (host) {
            var bar = host.querySelector('.erdb-loading');
    ...
    function applyImageSrc(imgEl, url, fallbackSrc) {
        if (!imgEl || !url) return;
        // Если это попадание в кеш — никакого индикатора не нужно.
        var cached = readCachedDataUrl(url);
        if (cached) {
            imgEl.src = cached;
            return;
        ...
        loadProxiedImage(url, function (dataUrl) {
            var finalize = function () { hideLoading(imgEl); };
            if (dataUrl) {
                imgEl.onload = finalize;
                imgEl.onerror = finalize;
                imgEl.src = dataUrl;
            } else {
                imgEl.onload = finalize;
    ...
    /* ----------------------- прогрев кеша (prefetch) ----------------------- */
    // Постеры/фоны для карточек, ещё не показанных пользователю,
    // подгружаются в фоне с ограничением параллельности.
    // Когда дойдёт очередь до показа — картинка уже в локальном base64-кеше,
    // и индикатор загрузки на ней даже не появится.

    var PREFETCH_QUEUE = [];        // массив URL'ов в очереди
    var PREFETCH_SEEN = {};         // url → true, чтобы не ставить в очередь дважды
    var PREFETCH_INFLIGHT = 0;
    var PREFETCH_MAX_PARALLEL = 2;  // не более N параллельных загрузок
    var PREFETCH_DELAY = 80;        // мс между запусками — не душим сеть

    function prefetchUrl(url) {
        if (!url || PREFETCH_SEEN[url]) return;
        if (IMG_MEM_CACHE[url]) return;
        PREFETCH_SEEN[url] = true;
        PREFETCH_QUEUE.push(url);
        runPrefetch();
    }

    function runPrefetch() {
        while (PREFETCH_INFLIGHT < PREFETCH_MAX_PARALLEL && PREFETCH_QUEUE.length) {
            var url = PREFETCH_QUEUE.shift();
            // если уже подтянуто за время ожидания — пропускаем
            if (readCachedDataUrl(url)) continue;
            PREFETCH_INFLIGHT++;
            (function (u) {
                setTimeout(function () {
                    loadProxiedImage(u, function () {
                        PREFETCH_INFLIGHT--;
                        // забываем, что видели — на случай ручного "сброса кеша"
                        // запись в SEEN остаётся, чтобы не дёргать одно и то же повторно
                        runPrefetch();
                    });
                }, PREFETCH_DELAY);
            })(url);
    ...
    // Поставить в прогрев постер (а если включено — и фон) для карточки.
    function prefetchCard(card) {
        if (!card) return;
        var id = makeId(card);
        if (!id) return;
        if (get('enable_poster')) {
            var p = buildUrl('poster', id);
            if (p) prefetchUrl(p);
        }
        if (get('enable_backdrop')) {
            var b = buildUrl('backdrop', id);
            if (b) prefetchUrl(b);
        }
    }
    ...
    function injectQualityCss() {
        ...
        s.textContent = [
            ...
            '.erdb-loading{position:absolute;left:0;right:0;bottom:0;height:3px;background:rgba(0,0,0,0.35);overflow:hidden;z-index:6;pointer-events:none;border-radius:0 0 0.4em 0.4em}',
            '.erdb-loading__bar{position:absolute;left:0;top:0;bottom:0;width:40%;background:linear-gradient(90deg,transparent,#f59e0b,#fbbf24,#f59e0b,transparent);animation:erdbSlide 1.1s ease-in-out infinite}',
            '@keyframes erdbSlide{0%{left:-40%}100%{left:100%}}'
        ].join('\n');
        ...
        return [
            '<div class="erdb-panel">',
                '<div class="erdb-panel__head">',
                    '<div class="erdb-panel__title">ERDB Постеры <span class="erdb-mode erdb-mode--' + (EMBED_TOKEN ? 'private' : 'public') + '">' + (EMBED_TOKEN ? 'личная сборка' : 'публичная сборка') + '</span></div>',
                ...
                '<div class="erdb-panel__actions">',
                    ...
                    '<div class="erdb-btn selector" data-act="wizard">Мастер настройки</div>',
                '</div>',
    ...
    function injectPanelCss() {
        ...
        s.textContent = [
            ...
            '.erdb-mode{display:inline-block;font-size:0.5em;vertical-align:middle;margin-left:0.7em;padding:0.25em 0.7em;border-radius:1em;letter-spacing:1px;text-transform:uppercase;font-weight:700}',
            '.erdb-mode--private{background:rgba(34,197,94,0.2);color:#86efac;border:1px solid rgba(34,197,94,0.4)}',
            '.erdb-mode--public{background:rgba(59,130,246,0.2);color:#93c5fd;border:1px solid rgba(59,130,246,0.4)}',
    ...
    function handleAction(act, refresh) {
        ...
        if (act === 'wizard') { Lampa.Modal.close(); setTimeout(openOnboardingWizard, 200); return; }
        ...
        Lampa.Listener.follow('line', function (e) {
            ...
            setTimeout(function () {
                try {
                    ...
                    // Прогрев кеша на всю строку, чтобы при прокрутке
                    // постеры справа подтягивались мгновенно из памяти.
                    if (tokenOk(activeToken())) {
                        items.forEach(function (it) { prefetchCard((it && (it.data || it)) || null); });
                    }
                } catch (err) { /* silent */ }
            }, 50);
        });

        // Когда строка катится — на каждое подведение фокуса к карточке
        // греем картинки следующих 5 соседей. Это даёт «бесконечную»
        // плавность прокрутки.
        Lampa.Listener.follow('card', function (e) {
            if (e.type !== 'focus') return;
            try { prefetchNeighbors(e); } catch (err) { /* silent */ }
        });
    }

    function prefetchNeighbors(focusEvent) {
        if (!tokenOk(activeToken())) return;
        // Lampa передаёт в focus event объект с .object (контейнер строки) и .card (текущая карточка).
        var lineObj = focusEvent && focusEvent.object;
        if (!lineObj) return;
        var items = lineObj.items || (lineObj.line && lineObj.line.items) || [];
        if (!items.length) return;

        var currentCard = focusEvent.data || (focusEvent.card && (focusEvent.card.data || focusEvent.card));
        var idx = -1;
        for (var i = 0; i < items.length; i++) {
            var it = items[i];
            var d = it && (it.data || it);
            if (d && currentCard && d === currentCard) { idx = i; break; }
            if (d && currentCard && d.id && currentCard.id && d.id === currentCard.id) { idx = i; break; }
        }
        if (idx < 0) idx = 0;

        // Прогреваем 5 карточек справа от текущей
        for (var k = 1; k <= 5; k++) {
            var nxt = items[idx + k];
            if (!nxt) break;
            prefetchCard(nxt.data || nxt);
        }
    }

    /* ----------------------- мастер первого запуска ----------------------- */

    var WIZARD_DONE_KEY = 'erdb_onboarded';

    function shouldRunWizard() {
        // Мастер показывается, если:
        // - встроенного токена нет (публичная сборка),
        // - у пользователя нет ни одного профиля,
        // - мастер ещё ни разу не закрывали.
        if (EMBED_TOKEN) return false;
        if (getProfiles().length > 0) return false;
        if (Lampa.Storage.get(WIZARD_DONE_KEY, false)) return false;
        return true;
    }

    function markWizardDone() {
        Lampa.Storage.set(WIZARD_DONE_KEY, true);
    }

    function injectWizardCss() {
        if (document.getElementById('erdb-wizard-style')) return;
        var s = document.createElement('style');
        s.id = 'erdb-wizard-style';
        s.textContent = [
            '.erdb-wiz{padding:2em;color:#fff;font-family:inherit;max-width:780px;margin:0 auto}',
            '.erdb-wiz__steps{display:flex;justify-content:center;gap:0.5em;margin-bottom:1.5em}',
            '.erdb-wiz__dot{width:0.75em;height:0.75em;border-radius:50%;background:rgba(255,255,255,0.2)}',
            '.erdb-wiz__dot.is-active{background:#f59e0b;box-shadow:0 0 12px rgba(245,158,11,0.7)}',
            '.erdb-wiz__dot.is-done{background:#22c55e}',
            '.erdb-wiz__title{font-size:1.8em;font-weight:800;text-align:center;margin-bottom:0.5em;background:linear-gradient(135deg,#f59e0b,#ef4444);-webkit-background-clip:text;-webkit-text-fill-color:transparent}',
            '.erdb-wiz__sub{text-align:center;opacity:0.75;margin-bottom:1.5em;font-size:1.05em;line-height:1.5}',
            '.erdb-wiz__body{background:rgba(255,255,255,0.05);padding:1.5em;border-radius:0.8em;margin-bottom:1.5em;line-height:1.6}',
            '.erdb-wiz__list{margin:0;padding-left:1.3em}',
            '.erdb-wiz__list li{margin-bottom:0.6em}',
            '.erdb-wiz__url{display:block;background:rgba(245,158,11,0.15);padding:1em;border-radius:0.5em;text-align:center;font-weight:700;font-size:1.1em;word-break:break-all;margin:1em 0;border:1px dashed rgba(245,158,11,0.5)}',
            '.erdb-wiz__field{padding:1em;background:rgba(0,0,0,0.3);border-radius:0.5em;margin:1em 0;text-align:center;font-family:monospace;font-size:1.1em;letter-spacing:1px;min-height:1.5em;border:2px dashed rgba(255,255,255,0.2)}',
            '.erdb-wiz__field.has-value{border-color:#22c55e;color:#86efac}',
            '.erdb-wiz__btns{display:flex;gap:0.7em;justify-content:center;flex-wrap:wrap}',
            '.erdb-wiz__btn{padding:0.9em 1.6em;border-radius:0.6em;font-weight:700;cursor:pointer;border:2px solid transparent;font-size:1em}',
            '.erdb-wiz__btn--primary{background:linear-gradient(135deg,#f59e0b,#b45309);color:#fff}',
            '.erdb-wiz__btn--ghost{background:rgba(255,255,255,0.08);color:#fff}',
            '.erdb-wiz__btn--success{background:linear-gradient(135deg,#22c55e,#15803d);color:#fff}',
            '.erdb-wiz__btn.focus,.erdb-wiz__btn.hover{border-color:#fff;transform:translateY(-2px)}',
            '.erdb-wiz__hint{margin-top:1em;text-align:center;opacity:0.5;font-size:0.85em}'
        ].join('\n');
        document.head.appendChild(s);
    }
    ...
    function openOnboardingWizard() {
        ...
        function dotsHtml() {
            return '<div class="erdb-wiz__steps">' +
                [1, 2, 3].map(function (n) {
                    var cls = 'erdb-wiz__dot';
                    if (n === step) cls += ' is-active';
                    else if (n < step) cls += ' is-done';
        ...
        function render() {
            ...
            if (step === 1) {
                html = [
                    '<div class="erdb-wiz">',
                        dotsHtml(),
                        '<div class="erdb-wiz__title">Добро пожаловать в ERDB!</div>',
                        '<div class="erdb-wiz__sub">Этот плагин заменяет постеры в Lampa на варианты с рейтингами IMDb, Кинопоиска, TMDB и бейджами качества (4K, HDR, Dolby Vision и т.п.).</div>',
                        '<div class="erdb-wiz__body">',
                            '<b>Что нужно сделать за 3 шага:</b>',
                            '<ol class="erdb-wiz__list">',
                                '<li>Зарегистрироваться на сайте <b>easyratingsdb.com</b> и настроить вид постеров.</li>',
                                '<li>Скопировать персональный токен <b>Tk-...</b>.</li>',
                                '<li>Вставить его сюда — и плагин заработает.</li>',
                            '</ol>',
                        '</div>',
                        '<div class="erdb-wiz__btns">',
                            '<div class="erdb-wiz__btn erdb-wiz__btn--primary selector" data-wact="next">Поехали →</div>',
                            '<div class="erdb-wiz__btn erdb-wiz__btn--ghost selector" data-wact="skip">Пропустить</div>',
                        '</div>',
                    '</div>'
                ].join('');
            } else if (step === 2) {
                var url = DEFAULT_BASE + '/configurator';
                html = [
                    '<div class="erdb-wiz">',
                        dotsHtml(),
                        '<div class="erdb-wiz__title">Шаг 1. Получить токен</div>',
                        '<div class="erdb-wiz__sub">Откройте на телефоне или компьютере ссылку ниже:</div>',
                        '<div class="erdb-wiz__url">' + escapeHtml(url) + '</div>',
                        '<div class="erdb-wiz__body">',
                            '<b>На сайте:</b>',
                            '<ol class="erdb-wiz__list">',
                                '<li>Зарегистрируйтесь или войдите.</li>',
                                '<li>Откройте раздел <b>Configurator</b>.</li>',
                                '<li>Настройте провайдеры рейтингов, стиль и бейджи качества (Stream Badges).</li>',
                                '<li>Скопируйте выданный токен — он начинается с <b>Tk-</b>.</li>',
                            '</ol>',
                        '</div>',
                        '<div class="erdb-wiz__btns">',
                            '<div class="erdb-wiz__btn erdb-wiz__btn--ghost selector" data-wact="back">← Назад</div>',
                            '<div class="erdb-wiz__btn erdb-wiz__btn--primary selector" data-wact="next">Токен у меня есть →</div>',
                        '</div>',
                    '</div>'
                ].join('');
            } else if (step === 3) {
                var fieldCls = 'erdb-wiz__field' + (enteredToken ? ' has-value' : '');
                var fieldText = enteredToken
                    ? '••••••••••••' + enteredToken.slice(-6)
                    : 'Нажмите «Ввести токен» ниже';
                html = [
                    '<div class="erdb-wiz">',
                        dotsHtml(),
                        '<div class="erdb-wiz__title">Шаг 2. Вставить токен</div>',
                        '<div class="erdb-wiz__sub">Откроется виртуальная клавиатура. Вставьте или введите токен Tk-...</div>',
                        '<div class="' + fieldCls + '">' + fieldText + '</div>',
                        '<div class="erdb-wiz__btns">',
                            '<div class="erdb-wiz__btn erdb-wiz__btn--ghost selector" data-wact="back">← Назад</div>',
                            '<div class="erdb-wiz__btn erdb-wiz__btn--primary selector" data-wact="enter">' + (enteredToken ? 'Изменить токен' : 'Ввести токен') + '</div>',
                            (enteredToken ? '<div class="erdb-wiz__btn erdb-wiz__btn--success selector" data-wact="finish">Готово ✓</div>' : ''),
                        '</div>',
                        '<div class="erdb-wiz__hint">Токен сохраняется только на этом устройстве.</div>',
                    '</div>'
                ].join('');
            }
            $root.html(html);
        ...
        function bindActions() {
            $root.find('[data-wact]').each(function () {
                var $el = Lampa.$(this);
                var act = $el.attr('data-wact');
                $el.on('hover:enter', function () { handle(act); });
            });
        }
        ...
        function handle(act) {
            if (act === 'next') {
                ...
                promptInput('Tk-... токен', enteredToken, function (val) {
                    val = String(val || '').trim();
                    if (val && !tokenOk(val)) {
                        Lampa.Noty && Lampa.Noty.show('Токен должен начинаться с Tk-');
                    } else {
                        enteredToken = val;
                    }
                    render();
                ...
                if (!tokenOk(enteredToken)) return;
                var name = 'Профиль 1 (…' + enteredToken.slice(-4) + ')';
                addProfile(name, enteredToken, DEFAULT_BASE);
                markWizardDone();
                Lampa.Modal.close();
                Lampa.Noty && Lampa.Noty.show('ERDB готов к работе! Откройте любой фильм — постер заменится.');
                setTimeout(function () { try { openMenuPanel(); } catch (e) {} }, 300);
            } else if (act === 'skip') {
                markWizardDone();
                Lampa.Modal.close();
                Lampa.Noty && Lampa.Noty.show('Мастер закрыт. Открыть его снова можно из панели «ERDB».');
            }
        }
    ...
    function startPlugin() {
        ...
        // Мастер первого запуска для публичной сборки без профилей
        if (shouldRunWizard()) {
            // даём Lampa дорисовать главный экран, потом открываем мастер
            setTimeout(function () { try { openOnboardingWizard(); } catch (e) {} }, 1500);
        }
        ...
        Lampa.Manifest && Lampa.Manifest.plugins && Lampa.Manifest.plugins.push({
            ...
            version: '1.6.0',
            name: 'ERDB Posters',
            description: 'Постеры с рейтингами + бейджи + меню + профили + кеш + мастер + прогрев',
            component: PLUGIN_ID
        });
    }
