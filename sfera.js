...
(function() {
    ...
    fire: [
      {
        name: 'Камин HD 1',
        type: 'mp4',
        url: 'https://videos.pexels.com/video-files/35042279/14844511_1920_1080_30fps.mp4'
      },
      {
        name: 'Камин HD 2',
        type: 'mp4',
        url: 'https://videos.pexels.com/video-files/2034100/2034100-hd_1920_1080_24fps.mp4'
      },
      {
        name: 'Камин HD 3',
        type: 'mp4',
        url: 'https://videos.pexels.com/video-files/6232461/6232461-hd_1920_1080_24fps.mp4'
      }
    ],
    aqua: [
      {
        name: 'Аквариум HD 1',
        type: 'mp4',
        url: 'https://videos.pexels.com/video-files/12142670/12142670-hd_1920_1080_30fps.mp4'
      },
      {
        name: 'Аквариум HD 2',
        type: 'mp4',
        url: 'https://videos.pexels.com/video-files/8950644/8950644-hd_1920_1080_30fps.mp4'
      },
      {
        name: 'Аквариум HD 3',
        type: 'mp4',
        url: 'https://videos.pexels.com/video-files/3147101/3147101-hd_1920_1080_25fps.mp4'
      }
    ]
  };
  ...
  function addStyleOnce() {
    ...
    style.textContent = '' +
      ...
      '#atm-saver .atm-status{position:absolute;left:50%;bottom:42px;transform:translateX(-50%);z-index:12;color:rgba(255,255,255,.72);font:18px Arial,sans-serif;text-align:center;text-shadow:0 2px 12px rgba(0,0,0,.8);transition:opacity .45s ease;}' +
      ...
      '@media (max-width:700px){#atm-clock{top:24px;right:28px}.atm-status{font-size:15px!important;bottom:28px!important}}';
  ...
  function loadHlsJs(callback) {
    ...
    function finish(ok) {
      ...
      var callbacks = hlsCallbacks.slice(0);
      hlsCallbacks = [];
  ...
  function startClock(container) {
    ...
    setTimeout(function() {
      if (clock && clock.parentNode) clock.style.opacity = '1';
    }, 300);
  }
  ...
  function cleanupVideoHandlers() {
    videoHandlers.forEach(function(item) {
  ...
  function showStatus(status, text, keepVisible) {
    if (!status) return;

    status.textContent = text || '';
    status.style.opacity = keepVisible ? '1' : '0.72';
  }

  function hideStatusLater(status) {
    setTimeout(function() {
      if (status && status.parentNode) status.style.opacity = '0';
    }, 1800);
  }

  function playVideo(video, status, onFail) {
    ...
    promise.catch(function(error) {
      ...
      video.muted = true;
      showStatus(status, 'Запуск без звука...', true);

      try {
        var mutedPromise = video.play();
        if (mutedPromise && mutedPromise.catch) {
          mutedPromise.catch(onFail);
        }
      } catch (e) {
        onFail(error || e);
      }
  ...
  function nextStream(reason) {
    ...
    setTimeout(function() {
      if (screensaverDiv && activeVideo) loadStream(activeVideo, screensaverDiv.querySelector('.atm-status'), reason);
    }, 600);
  }

  function startWatchdog(video, status) {
    clearWatchdog();

    watchdogInterval = setInterval(function() {
      if (!screensaverDiv || !video) {
        clearWatchdog();
        return;
      }

      if (video.paused && video.readyState >= 2) {
        playVideo(video, status, function() {
          nextStream('paused');
        });
        return;
      }
      ...
      if (stuckTicks >= 3) {
        showStatus(status, 'Поток завис, переключаю...', true);
        nextStream('stuck');
      }
    }, 7000);
  }

  function loadMp4(video, status, stream, token) {
    ...
    onVideo(video, 'playing', function() {
      if (token !== loadSeq) return;
      clearStartTimeout();
      showStatus(status, stream.name, false);
      hideStatusLater(status);
      startWatchdog(video, status);
    });
    onVideo(video, 'stalled', function() {
      showStatus(status, 'Буферизация...', true);
    });
    onVideo(video, 'waiting', function() {
      showStatus(status, 'Буферизация...', true);
    });
  ...
  function loadNativeHls(video, status, stream, token) {
    var playRequested = false;
  ...
  function loadHls(video, status, stream, token) {
    video.loop = false;
    ...
    loadHlsJs(function(ok) {
      ...
      if (!ok || !window.Hls || !Hls.isSupported()) {
        if (video.canPlayType('application/vnd.apple.mpegurl')) {
          loadNativeHls(video, status, stream, token);
        } else {
          nextStream('hls-not-supported');
        }
        return;
      }
  ...
  function loadStream(video, status, reason) {
    ...
    updateVideoAudio(video);
    showStatus(status, (reason ? 'Переключение: ' : '') + stream.name, true);

    startTimeout = setTimeout(function() {
      if (token !== loadSeq) return;
      showStatus(status, 'Поток долго не отвечает...', true);
      nextStream('start-timeout');
    }, 16000);

    if (type === 'hls') loadHls(video, status, stream, token);
    else loadMp4(video, status, stream, token);
  }
  ...
  function showScreensaver() {
    ...
    startClock(root);
    setTimeout(function() {
      if (root && root.parentNode) root.classList.add('atm-visible');
    }, 30);
  ...
  function addSettings() {
    ...
    Lampa.SettingsApi.addParam({
      component: 'atm_settings',
      param: { name: 'atm_enabled', type: 'select', values: { 'true': 'Включена', 'false': 'Выключена' }, default: 'true' },
      field: { name: 'Заставка' },
      onChange: resetTimer
    });
    ...
    Lampa.SettingsApi.addParam({
      component: 'atm_settings',
      param: { name: 'atm_timeout', type: 'select', values: { '1': '1 мин', '2': '2 мин', '3': '3 мин', '5': '5 мин', '10': '10 мин', '15': '15 мин', '30': '30 мин' }, default: '5' },
    ...
    Lampa.SettingsApi.addParam({
      component: 'atm_settings',
      param: { name: 'atm_sound', type: 'select', values: { 'true': 'Включен', 'false': 'Выключен' }, default: 'false' },
      field: { name: 'Звук' },
      onChange: function() { updateVideoAudio(activeVideo); }
    });

    Lampa.SettingsApi.addParam({
      component: 'atm_settings',
      param: { name: 'atm_volume', type: 'select', values: { '0': '0%', '10': '10%', '30': '30%', '50': '50%', '70': '70%', '100': '100%' }, default: '50' },
    ...
    Lampa.SettingsApi.addParam({
      component: 'atm_settings',
      param: { type: 'button', name: 'atm_test_now' },
      field: { name: 'Запустить заставку сейчас' },
      onChange: showScreensaver
    });
  }
  ...
  function bindActivityEvents() {
    ['keydown', 'mousemove', 'click', 'touchstart', 'wheel', 'pointerdown'].forEach(function(eventName) {
      try {
