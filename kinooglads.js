...
(function () {
    ...
    // =========================================================
    // External player launchers via Android Intent URI
    // window.location.href is the ONLY reliable way from WebView
    // =========================================================

    /**
     * Build Android Intent URI for a video URL and app package.
     * Format: intent://<host><path>#Intent;scheme=<scheme>;package=<pkg>;end
     */
    function buildIntentUri(videoUrl, packageName) {
        // Strip scheme from url for intent:// format
        var stripped = videoUrl.replace(/^https?:\/\//, '');
        return 'intent://' + stripped + '#Intent;scheme=https;package=' + packageName + ';end';
    }

    /**
     * Try to open an Android app via Intent URI.
     * Falls back to browser URL after timeout.
     */
    function openViaIntent(intentUri, fallbackUrl, timeoutMs) {
        var t = timeoutMs || 2000;
        var fallbackTimer;

        // Detect if we're on Android (Lampa always is on TV/phone)
        var isAndroid = /android/i.test(navigator.userAgent) || (typeof Android !== 'undefined');

        if (isAndroid) {
            // Set a timer: if app not opened in time → fallback to browser
            fallbackTimer = setTimeout(function () {
                openInBrowser(fallbackUrl);
            }, t);

            // Navigating via location.href fires the Android intent system
            try {
            ...
            // Cancel fallback if page becomes hidden (app launched successfully)
            ...
            // Not Android — just open in browser
            openInBrowser(fallbackUrl);
        }
    }
    ...
    function playInExternalPlayer(videoId, mode) {
        ...
        switch (mode) {
            case 'vlc':
                // VLC for Android package: org.videolan.vlc
                openViaIntent(
                    buildIntentUri(ytWatch, 'org.videolan.vlc'),
                    ytWatch
                );
                break;
            ...
            case 'mx':
                ...
                openViaIntent(
                    buildIntentUri(ytWatch, 'com.mxtech.videoplayer.ad'),
                    ytWatch
                );
                break;

            case 'vimu':
                // ViMU Player package: litvak.ru.vimu (or ru.vimu)
                // ViMU uses its own intent scheme with video URL
                openViaIntent(
                    buildIntentUri(ytWatch, 'litvak.ru.vimu'),
                    ytWatch
                );
                break;

            case 'ddd':
                // DDD Player / 3D Player
                openViaIntent(
                    buildIntentUri(ytWatch, 'com.ddd.player'),
                    ytWatch
                );
                break;
            ...
            case 'youtube':
                ...
                var ytAppIntent = 'intent://www.youtube.com/watch?v=' + videoId +
                    '#Intent;scheme=https;package=com.google.android.youtube;end';
                openViaIntent(ytAppIntent, ytShort);
                break;

            case 'browser':
                openInBrowser(ytShort);
                break;
