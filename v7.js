(function () {
    ...
    // Author: Maks TV
    // Netflix Premium Style v13.0
    // GitHub: maks-tv/netflix-premium-style

    var isExoticOS = /Vidaa|Web0S|Tizen|SmartTV|Metrological|NetCast/i.test(navigator.userAgent);

    // Автооптимизация: бережный профиль для слабых ТВ, без изменения левого меню.
    function isWeakDevice() {
        var ua = navigator.userAgent || '';
        var lowEndUA = /Android TV|AFT|BRAVIA|Web0S|NetCast|Vidaa|Tizen|SmartTV|TV Box/i.test(ua);
        var cores = navigator.hardwareConcurrency || 0;
        var memory = navigator.deviceMemory || 0;
        return isExoticOS || lowEndUA || (cores > 0 && cores <= 2) || (memory > 0 && memory <= 2);
    }

    function autoOptimize() {
        return storeBool('nfx_auto_optimize', true) && isWeakDevice();
    }
    ...
    function storeMeta() {
        // Metadata for plugin discovery and credit tracking (hidden settings)
        return {
            name: 'Netflix Premium Style',
            version: '13.0',
            author: 'Maks TV',
            description: 'UI customization for Lampa with quality badges, hi-res posters, auto-optimization'
        };
    }
    ...
    var QualityEngine = {
        ...
        mode: function () {
            ...
            return autoOptimize() && m === 'full' ? 'fast' : m;
        },
    ...
    var CardProcessor = {
        ...
        posterSize: function () {
            ...
            if (autoOptimize() && sizeWeight(size) > sizeWeight('w342')) return 'w342'; // экономия памяти
            return size;
        },
        ...
        fadePoster: function (card, img) {
            if (!storeBool('nfx_poster_fade', true) || autoOptimize()) return;
    ...
    function initCardProcessor() {
        ...
        var edgeTimer = null;
        var pendingRoots = [];
        var delay = autoOptimize() ? 260 : 120;
    ...
    function applyBodyFlags() {
        var b = document.body;
        if (!b) return;
        // Изоляция всегда включена, чтобы этот плагин не мог быть отключён
        // случайной настройкой и не возвращал глобальные CSS-селекторы.
        b.classList.add('nfx-scope');
        if (storeBool('nfx_hero_zoom', true) && !autoOptimize()) b.classList.add('nfx-zoom');
        else b.classList.remove('nfx-zoom');
        b.classList.toggle('nfx-auto-lite', autoOptimize());
        b.classList.toggle('nfx-no-focus-animation', !storeBool('nfx_focus_animation', true));
    }
    ...
    function scopeSelector(selector) {
        ...
        // body должен быть тем же корнем, а не потомком самого себя.
        if (selector === ':root') return 'body.nfx-scope';
        if (selector === 'body') return 'body.nfx-scope';
        if (selector.indexOf('body') === 0) {
            return 'body.nfx-scope' + selector.substring(4);
        }

        // Псевдоэлемент scrollbar требует корня перед ним.
        return 'body.nfx-scope ' + selector;
    }

    function scopeCSS(source) {
        // Префиксируем только селекторы. Значения, @media и keyframes не ломаем.
        var out = '';
        var start = 0;
        var stack = [];
        var commentEnd = 0;
        ...
        for (var i = 0; i < source.length; i++) {
            if (source.substr(i, 2) === '/*') {
                var endComment = source.indexOf('*/', i + 2);
            ...
            var ch = source[i];
            if (ch === '{') {
                var pre = source.slice(start, i);
                var trimmed = pre.trim();
                var isAt = trimmed.charAt(0) === '@';
                var inKeyframes = false;
                for (var k = 0; k < stack.length; k++) {
                    if (stack[k] === 'keyframes') { inKeyframes = true; break; }
                }

                if (!isAt && !inKeyframes && trimmed) {
                    var lastComment = pre.lastIndexOf('*/');
                    var selectorPart = lastComment >= 0 ? pre.slice(lastComment + 2) : pre;
                    var beforeSelector = lastComment >= 0 ? pre.slice(0, lastComment + 2) : '';
                    var leadMatch = /^\s*/.exec(selectorPart);
                    var lead = leadMatch ? leadMatch[0] : '';
                    var selectorText = selectorPart.slice(lead.length);
                    var scoped = selectorText.split(',').map(scopeSelector).join(', ');
                    out += beforeSelector + lead + scoped;
                } else {
                    out += pre;
                }

                out += '{';
                stack.push(isAt && /@(-webkit-)?keyframes/i.test(trimmed) ? 'keyframes' : (isAt ? 'at' : 'rule'));
                start = i + 1;
            } else if (ch === '}') {
    ...
    function injectCSS() {
        var old = document.getElementById('nfx-premium-v13');
        if (old) old.remove();
        var legacyIds = ['nfx-premium-v9', 'nfx-premium-v11', 'nfx-premium-v12'];
        for (var li = 0; li < legacyIds.length; li++) {
            var legacy = document.getElementById(legacyIds[li]);
        ...
        var qualitySize = storeGet('nfx_quality_size', 'auto');
        var qualityPreset = storeGet('nfx_quality_preset', 'auto');
        if (qualityPreset === 'weak_tv') qualitySize = 'tiny';
        if (qualityPreset === 'balanced_tv') qualitySize = 'small';
        if (qualityPreset === 'clear_tv') qualitySize = 'medium';
        var qualityPad = qualityStyle === 'wide' ? '2px 6px' : (qualityStyle === 'pill' ? '1px 5px' : '0px 3px');
        var qualityRadius = qualityStyle === 'pill' ? '999px' : '3px';
        var qualityFont = qualityStyle === 'wide' ? '0.55em' : '0.46em';
        var qualitySizes = {
            'tiny':   { pad: '0px 2px', font: '0.40em' },
            'small':  { pad: '0px 3px', font: '0.46em' },
            'medium': { pad: '1px 5px', font: '0.52em' },
            'large':  { pad: '2px 7px', font: '0.62em' },
            'xlarge': { pad: '3px 10px', font: '0.70em' }
        };
        if (qualitySizes[qualitySize]) {
            qualityPad = qualitySizes[qualitySize].pad;
            qualityFont = qualitySizes[qualitySize].font;
        }
...
/* Автопрофиль снижает только тяжёлые эффекты интерфейса. */
body.nfx-auto-lite .card { transition-duration: 140ms !important; }
body.nfx-auto-lite .card__view::before { box-shadow: 0 0 6px var(--nfx-accent-gl) !important; }
body.nfx-auto-lite .items-line, body.nfx-auto-lite .scroll__body { scroll-behavior: auto !important; }
...
.card {
    position: relative !important;
    transition: transform 120ms linear, z-index 0s !important;
    z-index: 1 !important;
...
.card__view {
    ...
    clip-path: inset(0 round var(--nfx-radius)) !important;
...
body.nfx-no-focus-animation .card,
body.nfx-no-focus-animation .card.focus,
body.nfx-no-focus-animation .card.hover,
body.nfx-no-focus-animation .card:hover,
body.nfx-no-focus-animation .card.focus ~ .card,
body.nfx-no-focus-animation .card.hover ~ .card,
body.nfx-no-focus-animation .card:hover ~ .card {
    transition: none !important;
}

body.nfx-no-focus-animation .card__view,
body.nfx-no-focus-animation .card__view::before {
    transition: none !important;
}
...
/* Без дерганья: на TV фокус меняется мгновенно, без hover-конфликта */
.card.focus,
.card.hover,
.card:hover,
.card.focus ~ .card,
...
`;
    ...
    function initSettings() {
        ...
        var i18n = {
            'en': {
                ...
                'auto_optimize': 'Auto-optimize for weak TVs',
                ...
                'badge_preset': 'TV Badge Preset',
                'preset_weak_tv': 'Weak TV: Tiny',
                'preset_balanced_tv': 'Balanced TV: Small',
                'preset_clear_tv': 'Clear TV: Medium'
            },
            'uk': {
                'ps_title': 'Premium Style',
                ...
                'auto_optimize': 'Автооптимізація для слабких ТВ',
                'focus_animation': 'Анімація фокусу',
                'quality_style': 'Стиль метки качества',
                'quality_compact': 'Компактная',
                'quality_pill': 'Капсула',
                'quality_wide': 'Широка',
                'badge_size': 'Розмір бейджа',
                'badge_preset': 'Пресет бейджа для ТВ',
                'preset_weak_tv': 'Слабкий ТВ: дуже малий',
                'preset_balanced_tv': 'Збалансований ТВ: малий',
                'preset_clear_tv': 'Чіткий ТВ: середній'
            },
            'ru': {
                'ps_title': 'Premium Style',
                ...
                'auto_optimize': 'Автооптимизация для слабых ТВ',
                'focus_animation': 'Анимация фокуса',
                'quality_style': 'Стиль метки качества',
                'quality_compact': 'Компактная',
                'quality_pill': 'Капсула',
                'quality_wide': 'Широкая',
                'badge_size': 'Размер бейджа',
                'badge_preset': 'Пресет бейджа для ТВ',
                'preset_weak_tv': 'Слабый ТВ: очень маленький',
                'preset_balanced_tv': 'Сбалансированный ТВ: маленький',
                'preset_clear_tv': 'Чёткий ТВ: средний'
            }
        };
        ...
        var prm = [
            ...
            { name: 'nfx_quality_preset', type: 'select', values: { 'auto': t('auto'), 'weak_tv': t('preset_weak_tv'), 'balanced_tv': t('preset_balanced_tv'), 'clear_tv': t('preset_clear_tv') }, default: 'auto', title: t('badge_preset') },
            { name: 'nfx_quality_size', type: 'select', values: { 'auto': t('auto'), 'tiny': t('tiny'), 'small': t('small'), 'medium': t('medium'), 'large': t('large'), 'xlarge': t('xlarge') }, default: 'auto', title: t('badge_size') },
            ...
            { name: 'nfx_focus_animation', type: 'trigger', default: true, title: t('focus_animation') },
            { name: 'nfx_auto_optimize', type: 'trigger', default: true, title: t('auto_optimize') },
    ...
    function bootstrap() {
        if (window.__nfx_premium_v13) return;
        window.__nfx_premium_v13 = true;

        // Store plugin metadata in hidden settings
        try { storeSet('nfx_plugin_meta', JSON.stringify(storeMeta())); } catch (e) {}
        ...
        var meta = storeMeta();
        console.log('[' + meta.author + '] ' + meta.name + ' v' + meta.version + ' loaded');
    }
