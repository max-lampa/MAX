(function () {
  ...
  var styles = {
    ...
    // ── Разрешение ──
    '.ts-nuvio-badge.ts-res-4k': {
      color: '#ffc371',
      'background-color': 'rgba(255, 195, 113, 0.16)',
      border: '0.15em solid rgba(255, 195, 113, 0.92)',
      'box-shadow': '0 0 0.9em rgba(255, 195, 113, 0.34)'
    },
    '.ts-nuvio-badge.ts-res-1080p': {
      color: '#88ff80',
      'background-color': 'rgba(136, 255, 128, 0.10)',
      border: '0.15em solid rgba(136, 255, 128, 0.72)'
    },
    '.ts-nuvio-badge.ts-res-720p': {
      color: '#70cc68',
      'background-color': 'rgba(112, 204, 104, 0.10)',
      border: '0.15em solid rgba(112, 204, 104, 0.65)'
    },
    '.ts-nuvio-badge.ts-res-480p': {
      color: '#909e38',
      'background-color': 'rgba(158, 176, 69, 0.08)',
      border: '0.15em solid rgba(158, 176, 69, 0.55)'
    },
    ...
    // ── HDTV / IMAX ──
    '.ts-nuvio-badge.ts-rel-hdtv': {
      color: '#5cd4c0',
      'background-color': 'rgba(16, 128, 142, 0.15)',
      border: '0.15em solid rgba(16, 128, 142, 0.88)',
      'box-shadow': '0 0 0.6em rgba(16, 128, 142, 0.24)'
    },
    '.ts-nuvio-badge.ts-rel-imax': {
      color: '#ffc371',
      'background-color': 'rgba(255, 195, 113, 0.14)',
      border: '0.15em solid rgba(255, 195, 113, 0.88)',
      'box-shadow': '0 0 0.7em rgba(255, 195, 113, 0.28)'
    },
    '.ts-nuvio-badge.ts-rel-imax-e': {
      color: '#4db6ff',
      'background-color': 'rgba(77, 182, 255, 0.14)',
      border: '0.15em solid rgba(77, 182, 255, 0.88)',
      'box-shadow': '0 0 0.7em rgba(77, 182, 255, 0.28)'
    },

    // ── Язык ──
    '.ts-nuvio-badge.ts-lang': {
      color: '#5cd4b0',
      'background-color': 'rgba(92, 212, 176, 0.10)',
      border: '0.15em solid rgba(92, 212, 176, 0.65)'
    },
  ...
  function isStylesEnabled() {
    try {
      ...
      var value = Lampa.Storage.get(TS_ENABLED_KEY, null);
  ...
  // ── HDR-тип (паттерны из Nuvio JSON) ──
  function tsExtractHdrType(text) {
    text = text || '';
    var hasDV = /(?:\bdv\b|\bdovi\b|dolby[\s._-]?vision)/i.test(text);
    // Dolby Vision + Atmos combo
    if (hasDV && /\batmos\b/i.test(text)) return { label: 'Atmos+DV', cls: 'ts-hdr-dv' };
    // Dolby Vision + TrueHD combo
    if (hasDV && /\btrue[\s._-]?hd\b/i.test(text)) return { label: 'TrueHD+DV', cls: 'ts-hdr-dv' };
    // Dolby Vision
    if (hasDV) return { label: 'Dolby Vision', cls: 'ts-hdr-dv' };
    // HDR10+
    if (/\bhdr[\s._-]?10[\s._-]?(?:\+|plus|p(?:lus)?)\b/i.test(text)) {
      return { label: 'HDR10+', cls: 'ts-hdr-hdr10plus' };
    }
    // HDR10
    if (/\bhdr[\s._-]?10\b/i.test(text)) return { label: 'HDR10', cls: 'ts-hdr-hdr10' };
    // HLG
    if (/\bhlg\b/i.test(text)) return { label: 'HLG', cls: 'ts-hdr-hlg' };
    // HDR / PQ (generic)
    if (/\b(?:hdr|pq)\b/i.test(text)) return { label: 'HDR', cls: 'ts-hdr-hdr10' };
    return null;
  }

  // ── Аудиоформат (расширенный по Nuvio JSON) ──
  function tsExtractAudioFormat(text) {
    text = text || '';
    // DTS:X (приоритет над DTS-HD MA)
    if (/\bdts[\s._-:]*x\b/i.test(text)) return { label: 'DTS:X', cls: 'ts-audio-dts' };
    // Atmos
    if (/\batmos\b/i.test(text)) return { label: 'Atmos', cls: 'ts-audio-premium' };
    // TrueHD
    if (/\btrue[\s._-]?hd\b/i.test(text)) return { label: 'TrueHD', cls: 'ts-audio-premium' };
    // DTS-HD MA
    ...
    // DTS (без модификаторов hd/x/ma)
    if (/\bdts\b(?![\s._-]*(?:hd|x\b|ma))/i.test(text)) return { label: 'DTS', cls: 'ts-audio-dts' };
    // DD+ / EAC3 / Dolby Digital Plus
    if (/\b(?:ddp\d?|dd\+|eac[\s._-]?3|e[\s._-]ac[\s._-]?3|dolby[\s._-]digital[\s._-]plus)\b/i.test(text)) {
      return { label: 'DD+', cls: 'ts-audio-ac3' };
    }
    // DD / AC3 / Dolby Digital (без Plus)
    if (/\b(?:dd[25][\s.]?[01]|ac[\s._-]?3|dolby[\s._-]digital)\b/i.test(text)) {
      return { label: 'DD', cls: 'ts-audio-ac3' };
    }
    // FLAC
  ...
  // Стриминговый сервис (паттерны из Nuvio JSON)
  function tsExtractStreamingService(text) {
    ...
    if (/\b(?:atvp|appl|appletv|apple[\s._-]?tv)\b/i.test(text)) return { label: 'APPLE TV+', cls: 'ts-stream-atvp' };
    if (/\b(?:dsnp|dsny|ds\+|disney\+?)\b/i.test(text)) return { label: 'DISNEY+', cls: 'ts-stream-disney' };
    if (/\b(?:hmax|hbomax|hbo[\s._-]?max)\b/i.test(text)) return { label: 'MAX', cls: 'ts-stream-max' };
    ...
    if (/\b(?:crave|crav)\b/i.test(text)) return { label: 'CRAVE', cls: 'ts-stream-crave' };
    if (/\bstan\b/i.test(text)) return { label: 'STAN', cls: 'ts-stream-crave' };
    if (/\b(?:stzn|starz)\b/i.test(text)) return { label: 'STARZ', cls: 'ts-stream-crave' };
    return null;
  }

  // Уровень релиза (паттерны из Nuvio JSON)
  function tsExtractReleaseTier(text) {
    text = text || '';
    // REMUX — паттерн из JSON: [_. |\d{4}p-|Hybrid-](BD|UHD)?Remux
    if (/(?:[_. ]|\d{4}p-|\bHybrid-)(?:(?:BD|UHD)[-_. ]?)?[Rr]emux\b|(?:(?:BD|UHD)[-_. ]?)?[Rr]emux[_. ]\d{4}p|\bremux\b/i.test(text)) {
      return { label: 'REMUX', cls: 'ts-tier-remux' };
    }
    // BLU-RAY — паттерн из JSON: BluRay|Blu-Ray|HD-?DVD|BDMux (не REMUX, не WEB)
    if (/\b(?:blu[\s._-]?ray|bluray|hd-?dvd|bdmux)\b/i.test(text) &&
        !/\bremux\b/i.test(text) &&
        !/\bweb[\s._-]?(?:dl|rip)\b/i.test(text)) {
      return { label: 'BLU-RAY', cls: 'ts-tier-bluray' };
    }
    // WEB-DL — паттерн из JSON: WEB-DL|WEBDL|AmazonHD|NetflixUHD|WebHD и пр.
    if (/\b(?:web[\s._-]?dl|webdl|amazon(?:hd|sd)|ituneshd|netflixu?hd|webhd|hbomaxhd|disneyhd)\b/i.test(text)) {
      return { label: 'WEB-DL', cls: 'ts-tier-web' };
    }
    // WEBRip — паттерн из JSON: WebRip|Web-Rip|WEBMux
    if (/\b(?:webrip|web[\s._-]rip|webmux)\b/i.test(text)) {
      return { label: 'WEBRip', cls: 'ts-tier-web' };
    }
    // HDTV
    if (/\bhdtv\b/i.test(text)) return { label: 'HDTV', cls: 'ts-rel-hdtv' };
    // DVD Rip
    if (/\b(?:dvdrip|dvd[\s._-]?rip)\b/i.test(text)) return { label: 'DVD', cls: 'ts-tier-web' };
    return null;
  }
  ...
  // Аудиоканалы (паттерны из Nuvio JSON)
  function tsExtractChannels(text) {
    text = text || '';
    var has78 = /[^0-9][7-8][. ][01](?![0-9])/.test(text);
    if (has78) return { label: '7.1', cls: 'ts-ch-71' };
  ...
  // Доп. аудиоформаты — перенесены в tsExtractAudioFormat
  ...
  // Разрешение (паттерны из Nuvio JSON)
  function tsExtractResolution(text) {
    text = text || '';
    if (/\b(?:2160[pi]?|4k|uhd)\b/i.test(text) && !/\b(?:1080[pi]?|720[pi]?)\b/i.test(text)) {
      return { label: '4K', cls: 'ts-res-4k' };
    }
    if (/\b1080[pi]?\b/i.test(text)) return { label: '1080p', cls: 'ts-res-1080p' };
    if (/\b720[pi]?\b/i.test(text)) return { label: '720p', cls: 'ts-res-720p' };
    if (/\b480[pi]?\b/i.test(text)) return { label: '480p', cls: 'ts-res-480p' };
    return null;
  }

  // IMAX (паттерны из Nuvio JSON)
  function tsExtractImax(text) {
    text = text || '';
    if (/\bimax[\s._-]?enhanced\b/i.test(text) ||
        (/\b(?:dsnp|ds\+|disney\+?|bcore|bc)\b/i.test(text) &&
         /\bweb[\s._-]?(?:dl|rip)\b/i.test(text) &&
         /\bimax\b/i.test(text))) {
      return { label: 'IMAX E', cls: 'ts-rel-imax-e' };
    }
    if (/\bimax\b/i.test(text) && !/enhanced/i.test(text)) {
      return { label: 'IMAX', cls: 'ts-rel-imax' };
    }
    return null;
  }

  // Языки (паттерны из Nuvio JSON)
  function tsExtractLanguages(text) {
    text = text || '';
    var langs = [];
    if (/(?:english|eng)(?![\s._-]?sub)/i.test(text)) langs.push({ label: '🇬🇧', cls: 'ts-lang' });
    if (/(?:spanish|spa|esp|latino|lat)(?![\s._-]?sub)/i.test(text)) langs.push({ label: '🇪🇸', cls: 'ts-lang' });
    if (/(?:french|fra|vf|vff|vfi|truefrench)(?![\s._-]?sub)/i.test(text)) langs.push({ label: '🇫🇷', cls: 'ts-lang' });
    if (/(?:deu(?:tsch)?|german)(?![\s._-]?sub)/i.test(text)) langs.push({ label: '🇩🇪', cls: 'ts-lang' });
    if (/(?:italian|ita)(?![\s._-]?sub)/i.test(text)) langs.push({ label: '🇮🇹', cls: 'ts-lang' });
    if (/(?:pt[\s._-]?br|brazilian)(?![\s._-]?sub)/i.test(text)) langs.push({ label: '🇧🇷', cls: 'ts-lang' });
    if (/(?:portuguese|pt[\s._-]?pt)(?![\s._-]?sub)/i.test(text) && !/pt[\s._-]?br/i.test(text)) langs.push({ label: '🇵🇹', cls: 'ts-lang' });
    if (/(?:turkish|tur)(?![\s._-]?sub)/i.test(text)) langs.push({ label: '🇹🇷', cls: 'ts-lang' });
    if (/(?:polish|pol)(?![\s._-]?sub)/i.test(text)) langs.push({ label: '🇵🇱', cls: 'ts-lang' });
    if (/(?:ukrainian|ukr)(?![\s._-]?sub)/i.test(text)) langs.push({ label: '🇺🇦', cls: 'ts-lang' });
    if (/(?:russian|rus)(?![\s._-]?sub)/i.test(text)) langs.push({ label: '🇷🇺', cls: 'ts-lang' });
    if (/(?:japanese|jap|jpn)(?![\s._-]?sub)/i.test(text) || /[぀-ゟ゠-ヿ]{3,}/.test(text)) langs.push({ label: '🇯🇵', cls: 'ts-lang' });
    if (/(?:korean|kor)(?![\s._-]?sub)/i.test(text) || /[가-힯]{3,}/.test(text)) langs.push({ label: '🇰🇷', cls: 'ts-lang' });
    if (/(?:chinese|chi|zho|mandarin|cantonese)(?![\s._-]?sub)/i.test(text) || /[一-鿿]{3,}/.test(text)) langs.push({ label: '🇨🇳', cls: 'ts-lang' });
    if (/(?:hindi|hin)(?![\s._-]?sub)/i.test(text) || /[ऀ-ॿ]{3,}/.test(text)) langs.push({ label: '🇮🇳', cls: 'ts-lang' });
    if (/(?:arabic|ara)(?![\s._-]?sub)/i.test(text) || /[؀-ۿ]{3,}/.test(text)) langs.push({ label: '🇸🇦', cls: 'ts-lang' });
    if (/(?:indonesian|ind)(?![\s._-]?sub)/i.test(text)) langs.push({ label: '🇮🇩', cls: 'ts-lang' });
    if (/(?:thai|tha)(?![\s._-]?sub)/i.test(text)) langs.push({ label: '🇹🇭', cls: 'ts-lang' });
    if (/(?:vietnamese|vie)(?![\s._-]?sub)/i.test(text)) langs.push({ label: '🇻🇳', cls: 'ts-lang' });
    return langs;
  }
  ...
  function tsAppendNuvioBadges(row, text) {
    ...
    // 1. Специальные теги (SEADEX, DIR CUT и пр.)
    ...
    // 3. Разрешение (4K / 1080p / 720p / 480p)
    var res = tsExtractResolution(text);
    if (res) {
      chip = document.createElement('span');
      chip.className = 'ts-nuvio-badge ' + res.cls;
      chip.textContent = res.label;
      row.appendChild(chip);
    }

    // 4. Уровень релиза (REMUX / BLU-RAY / WEB-DL / WEBRip / HDTV / DVD)
    ...
    var imax = tsExtractImax(text);
    if (imax) {
      chip = document.createElement('span');
      chip.className = 'ts-nuvio-badge ' + imax.cls;
      chip.textContent = imax.label;
      row.appendChild(chip);
    }
    ...
    // 9. Языки
    var langs = tsExtractLanguages(text);
    for (i = 0; i < langs.length; i++) {
      chip = document.createElement('span');
      chip.className = 'ts-nuvio-badge ' + langs[i].cls;
      chip.textContent = langs[i].label;
      row.appendChild(chip);
    }
  }
