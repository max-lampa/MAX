(function() {
    ...
    // =========================================================
    // ВСТРОЕННЫЕ ТЕМЫ – можно добавлять свои
    // =========================================================
    ...
    Lampa.Component.add('themes_browser_safe', function() {
        ...
        this.build = function() {
            ...
            BUILTIN_THEMES.forEach((theme, index) => {
                const card = $(`
                    <div class="card card--collection focusable" style="width: 14.2%; margin: 0.5%; text-align: center; box-sizing: border-box;">
                        <div class="card__view" style="position: relative;">
                            ...
                            <div class="card__title" style="padding: 0.5em 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${theme.title}</div>
                        </div>
                    </div>
                `);
                ...
                // Обработчики фокуса – только запоминаем элемент, прокруткой займётся Lampa
                card.on('hover:focus', () => {
                    currentFocus = card[0];
                });
            ...
            // Обновляем скролл после добавления всех карточек
            setTimeout(() => {
                try {
                    scroll.update();
                } catch (e) {
                    console.warn('Scroll update error:', e);
                }
            }, 100);
        ...
        this.start = function() {
            Lampa.Controller.add('content', {
                toggle: () => {
                    ...
                    // Если есть сохранённый фокус, устанавливаем его
                    if (currentFocus) {
                        Lampa.Controller.collectionFocus(currentFocus, scroll.render());
                    }
                },
