(function () {
    ...
    function fetchExtendedData(id, type) {
        ...
        // Совместимость с прежними сохранёнными значениями украинской версии.
        if (langPref === 'uk') langPref = 'ru';
        if (langPref === 'uk_en') langPref = 'ru_en';
        if (sloganLang === 'uk') sloganLang = 'ru';
        if (sloganLang === 'uk_en') sloganLang = 'ru_en';
        if (descLang === 'uk') descLang = 'ru';
        if (descLang === 'uk_en') descLang = 'ru_en';
        ...
        return new Promise(function (resolve) {
            var langQuery = (sloganLang === 'ru' || sloganLang === 'ru_en' || descLang === 'ru' || descLang === 'ru_en' || langPref === 'ru' || langPref === 'ru_en') ? 'ru-RU' : 'en-US';
    ...
    function buildCardCustomDOM(cardHtml, data) {
        ...
        if (!data.id) return;
         ...
         if (!posterCaption) {
             ...
             if (nativeView) el.insertBefore(posterCaption, nativeView);
         }
    ...
    function createSettings() {
        ...
        // -------------------------------------------------------------
         ...
         Lampa.SettingsApi.addParam({ component: 'ydesign_texts', param: { name: 'ydesign_lang', type: 'select', values: { 'ru': 'Только русский', 'ru_en': 'Русский → английский → оригинал', 'en_orig': 'Английский → оригинал' }, default: DefaultSettings.ydesign_lang }, field: { name: 'Язык логотипа и названия' } });
         Lampa.SettingsApi.addParam({ component: 'ydesign_texts', param: { name: 'ydesign_slogan_lang', type: 'select', values: { 'ru': 'Только русский', 'ru_en': 'Русский (английский, если нет)', 'en': 'Только английский' }, default: DefaultSettings.ydesign_slogan_lang }, field: { name: 'Язык слогана' } });
         Lampa.SettingsApi.addParam({ component: 'ydesign_texts', param: { name: 'ydesign_desc_lang', type: 'select', values: { 'ru': 'Только русский', 'ru_en': 'Русский (английский, если нет)', 'en': 'Только английский' }, default: DefaultSettings.ydesign_desc_lang }, field: { name: 'Язык описания' } });
         Lampa.SettingsApi.addParam({ component: 'ydesign_texts', param: { name: 'ydesign_add_title_lang', type: 'select', values: { 'ru': 'Всегда русский', 'en': 'Всегда английский', 'auto': 'Зависит от логотипа' }, default: DefaultSettings.ydesign_add_title_lang }, field: { name: 'Язык дополнительного названия' } });
