(function () {
  ...
  function move(direction) {
    ...
    if (direction === 'left' || direction === 'right') {
      ...
      if (type === 'row') {
        next = st.focus === rowStart ? chipStart : Math.max(rowStart, st.focus - 1);
      } else if (type === 'chip') {
        next = 0;
      }
    }
  ...
  function bindClick(r) {
    r.addEventListener('click', function (e) {
      var el = e.target.closest ? e.target.closest('.ltrf-btn,.ltrf-chip,.ltrf-row') : null;
  ...
  function open(data, card) {
    ...
    r.innerHTML =
      ...
      '</nav><div class="ltrf-found"></div><div class="ltrf-list"></div>' +
  ...
  function connect(force) {
    if (Date.now() < closedUntil || get(K.auto, true) === false ||
