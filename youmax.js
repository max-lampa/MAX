 ...
 * YouTube channel browser: add channels, browse videos, play via external iframe player.
 *
 * FIXES:
 *   1. Removed "На канал автора" redirect card — videos now display without the channel button.
 *   2. Replaced YouTube IFrame API player with direct nocookie iframe embed to avoid
 *      "sign in / subscribe" prompts and reduce buffering.
 *   3. KinoCard.play() always uses the custom iframe player regardless of Lampa version.
 *
...
(function () {
    ...
    // =========================================================
    // External iframe player (no sign-in / subscribe prompts, less buffering)
    // Uses youtube-nocookie.com to bypass channel-promotion overlays.
    // =========================================================
    function playYouTubeCustom(videoId) {
        var params = [
            "autoplay=1",
            "controls=1",
            "rel=0",
            ...
            "origin=" +
                encodeURIComponent(
                    window.location.origin || "https://lampa.mx",
                ),
        ].join("&");
        ...
        var overlay = $(
            [
                '<div class="kino-ext-overlay" style="',
        ...
        overlay.append(loader).append(iframe);
        $("body").append(overlay);
        ...
        Lampa.Controller.add("kino_ext_player", {
            toggle: function () {},
            up: function () {},
            down: function () {},
            left: function () {},
        ...
        Lampa.Controller.toggle("kino_ext_player");

        // Close on click outside the iframe (e.g. TV back button via Lampa)
        overlay.on("click", function (e) {
            if (e.target === overlay[0]) closePlayer();
        });
    }
    ...
    var KinoApi = {
        ...
        resolveHandleToChannelId: function (handle, callback) {
            ...
            function tryProxy(idx) {
                if (idx >= _this.proxies.length) {
                    console.error(
                        "Kinooglad: All proxies failed for handle @" +
                            cleanHandle,
                    );
                ...
                var url =
                    proxy.indexOf("corsproxy") > -1
                        ...
                        : proxy + encodedPage;
                console.log(
                    "Kinooglad: Trying proxy " +
                        (idx + 1) +
                        "/" +
                        _this.proxies.length +
                        ": " +
                        proxy,
                );
                $.get(url)
                    .done(function (html) {
                        ...
                        if (m && m[1]) {
                            console.log(
                                "Kinooglad: Successfully resolved @" +
                                    cleanHandle +
                                    " to " +
                                    m[1],
                            );
        ...
        resolveVideoToChannelId: function (videoId, callback) {
            ...
            function tryProxy(idx) {
                if (idx >= _this.proxies.length) {
                    console.error(
                        "Kinooglad: All proxies failed for video " + cleanId,
                    );
                    callback(new Error("resolve_failed"));
                    return;
                }
                ...
                $.get(url)
                    .done(function (html) {
                        ...
                        if (m && m[1]) {
                            ...
                            console.log(
                                "Kinooglad: Successfully resolved video " +
                                    cleanId +
                                    " to channel " +
                                    m[1],
                            );
                            ...
                            console.log(
                                "Kinooglad: Proxy " +
                                    (idx + 1) +
                                    " failed to find channel ID for video",
                            );
                            tryProxy(idx + 1);
                        }
                    })
            ...
            function doFetch(feedUrl) {
                ...
                function tryFetch(index) {
                    if (index >= _this.proxies.length) {
                        console.log(
                            "Kinooglad: All proxies failed for " + channel.name,
                        );
                        onerror();
                        return;
                    }
        ...
        main: function (oncomplite, onerror) {
            ...
            var promises = channels.map(function (channel, index) {
                return new Promise(function (resolve) {
                    console.log(
                        "Kinooglad: Starting load for channel " +
                            (index + 1) +
                            "/" +
                            channels.length +
                            ": " +
                    ...
                    var timeout = setTimeout(function () {
                        console.log(
                            "Kinooglad: Timeout for channel " + channel.name,
                        );
                        resolve({
                            title: channel.name,
            ...
            Promise.all(promises).then(function (results) {
                console.log(
                    "Kinooglad: All channels loaded in " +
                        (Date.now() - startTime) +
                        "ms",
                );
    ...
    function KinoCard(data) {
        this.build = function () {
            if (data.is_button) {
                this.card = $(
                    '<div class="card selector card--wide layer--render layer--visible kino-card show-more-button">' +
        ...
        // FIX: Always use the custom nocookie iframe player — no Lampa.YouTube / redirect prompts
        this.play = function (id) {
            if (data.is_button) {
                ...
                Lampa.Activity.push({
                    url: "",
                    title: channelTitle + " — стр. " + (currentPage + 1),
            ...
            // Use our external iframe player directly — avoids sign-in/subscribe prompts
            playYouTubeCustom(id);
        };
    ...
    // FIX: "На канал автора" redirect card removed entirely
    // =========================================================
    function KinoLine(data) {
        ...
        this.create = function () {
            ...
            // "На канал автора" redirect card intentionally NOT added here (fix #1)
        };
    ...
    function KinoChannelView(object) {
        ...
        this.create = function () {
            ...
            html.append(
                $('<div class="kino-head" style="display:none;"></div>'),
            );

            var channel = object.channel || {
                id: object.channel_id || object.id,
                name: object.title || "Канал",
            };
    ...
    function startPlugin() {
        ...
        $("body").append(
            '<style id="kinooglad-css">' +
                [
                    ...
                    ".kino-ext-overlay{position:fixed;top:0;left:0;width:100%;height:100%;z-index:10000;background:#000;}",
                    ".kino-ext-overlay iframe{position:absolute;top:0;left:0;width:100%;height:100%;border:none;}",
                ].join("") +
                "</style>",
        );
        ...
        if (Lampa.SettingsApi && Lampa.SettingsApi.addParam) {
            ...
            Lampa.SettingsApi.addParam({
                ...
                onChange: function (v) {
                    Lampa.Storage.set("kinooglad_enabled", v);
                },
            });
            ...
            Lampa.SettingsApi.addParam({
                ...
                param: {
                    name: "kinooglad_add",
                    type: "input",
                    placeholder: tr("kino_add_channel_input"),
                    default: "",
                },
                field: {
                ...
                onChange: function (v) {
                    ...
                    var parsed = parseChannelInput(rawId);
                    if (!parsed) {
                        Lampa.Noty.show("Не распознан формат ввода", {
                            time: 3000,
                        });
                        return;
                    }
                    ...
                    function showMsg(msg, isError) {
                        Lampa.Noty.show(msg, { time: isError ? 4000 : 2500 });
                    }

                    if (/^UC[\w-]{22}$/.test(rawId)) {
                        var existsById = ch.some(function (c) {
