    ...
    function checkAvailability(url, callback) {
        ...
        if (auth) {
            head.headers = {
                Authorization: "Basic " + Lampa.Base64.encode(
                    Lampa.Storage.get('torrserver_login') + ':' + Lampa.Storage.get('torrserver_password')
                )
            };
        }
        ...
        network.native(Lampa.Utils.checkEmptyUrl(url), function() {
            ...
            if (a.status == 401) {
                ...
                callback(true); // 401 считается "доступным", как у Лампы
            } else {
    ...
    function showServerSelection(callback) {
        ...
        Lampa.Select.show({
            ...
            onBack: function() {
                ...
                callback(null); // Пользователь отменил выбор
            },
