(function () {
  ...
  var OWN = {
    ...
    nova_skin_set_all: { ru: 'Загружать все раздачи на ТВ', uk: 'Завантажувати всі роздачі на ТВ', en: 'Load all torrents on TV' },
    nova_skin_set_all_descr: { ru: 'Сначала прокрутить исходный список, чтобы Lampa отдала все результаты', uk: 'Спочатку прокрутити вихідний список, щоб Lampa віддала всі результати', en: 'Prime the native list so Lampa returns every result' },
  ...
  function scrollTo(element) {
    ...
    if (scroll) {
      try {
        ...
        var activeBox = $(node).closest('.scroll').first();
        if (activeBox.length) {
          var activeRect = activeBox[0].getBoundingClientRect();
          var activeNodeRect = node.getBoundingClientRect();
          if (activeNodeRect.top >= activeRect.top + 12 &&
              activeNodeRect.bottom <= activeRect.bottom - 12) return;
        }
      } catch (e) {}
    }
    try {
      ...
      var top = nodeRect.top - boxRect.top;
      var bottom = nodeRect.bottom - boxRect.top;
      var height = box[0].clientHeight || box[0].offsetHeight;
      if (top >= 12 && bottom <= height - 12) return;

      var delta = bottom > height - 12 ? bottom - height + 12 : top - 12;
  ...
  function torrentTitle(origin) {
    ...
    if (!value) {
      value = origin.clone().find('.torrent-item__info,.torrent-item__details,.torrent-item__meta,' +
        '.torrent-item__size,.torrent-item__seeders,.torrent-item__peers').remove().end().text()
        .replace(/\s+/g, ' ').trim();
    }
    return value;
  }
  ...
  function torrentPicture(origin) {
    var img = origin.find('.torrent-item__img img,.torrent-item__poster img,' +
      '.torrent-item__cover img,img').first();
    return img;
  }

  function readCard(node, index) {
    var origin = $(node);
    var title = torrentTitle(origin);
    var allText = origin.text().replace(/\s+/g, ' ').trim();
    ...
    if (!meta.length && allText) meta = [allText];

    var quality = shortQuality(title + ' ' + meta.join(' '));
    var size = torrentText(origin, ['.torrent-item__size']).join(' ');
    var tracker = torrentText(origin, ['.torrent-item__tracker']).join(' ');
    if (tracker && meta.indexOf(tracker) < 0) meta.unshift(tracker);
    ...
    return {
      ...
      num: index + 1,
      numbered: false,
      title: title || ('Раздача ' + (index + 1)),
      meta: meta,
      time: size,
      quality: quality,
      picture: torrentPicture(origin)
    };
  }
  ...
  function currentSkinNode() {
    if (!inSkin()) return null;

    var focused = ui.root.find('.focus.selector').first();
    if (focused.length) return focused[0];

    if (last && $.contains(ui.root[0], last) && $(last).hasClass('selector')) {
      return last;
    }
  ...
  function skinNodes() {
    ...
    ui.root.find('.selector').each(function () {
      var node = this;
      if ($(node).hasClass('nova-card--soon')) return;
  ...
  /*
   * Lampa's geometric Navigator can see the hidden native torrent cards
   * behind Nova Skin. Android TV then keeps the focus on the header instead
   * of entering the visible list. Keep D-pad navigation inside our own
   * selectors while the skin is active, and let collectionFocus do the
   * actual focus bookkeeping.
   */
  function navigateSkin(direction) {
    var current = currentSkinNode();
    if (!current) return false;

    var origin = current.getBoundingClientRect();
    var ox = origin.left + origin.width / 2;
    var oy = origin.top + origin.height / 2;
    var candidates = skinNodes();
    var best = null;
    ...
    candidates.forEach(function (node) {
      ...
      if (direction === 'down') {
        primary = y - oy;
        secondary = Math.abs(x - ox);
      } else if (direction === 'up') {
        primary = oy - y;
        secondary = Math.abs(x - ox);
      } else if (direction === 'right') {
        primary = x - ox;
        secondary = Math.abs(y - oy);
      } else {
        primary = ox - x;
        secondary = Math.abs(y - oy);
      }
      ...
      /*
       * A direction must actually cross into another row/column. This keeps
       * Down from jumping to a button beside the current button and makes
       * grid navigation predictable on a TV remote.
       */
      if (primary <= 2) return;
  ...
  function novaUp() {
    if (!inSkin()) return false;
    if (navigateSkin('up')) return true;
  ...
  function novaDown() {
    if (!inSkin()) return false;
    if (navigateSkin('down')) {
      lockRelease();
      return true;
    }

    /*
     * Pages deliberately render only a small set of cards. When the last
     * visible card has no DOM neighbour, Down must open the next page rather
     * than handing control back to the native explorer.
     */
    if (!ui_open && listFocused() && !nav && items.length > JUMP_FROM) {
      var index = keyIndex(ui_focus);
      if (index < 0 && last) index = keyIndex($(last).attr('data-nova-focus'));
      var page = pageAt(pages(items.length), index < 0 ? ui_page : index);
      if (page.end < items.length - 1) {
        showPage(page.end + 1, page.end + 1);
        return true;
      }
    }
  ...
  function novaRight() {
    if (!inSkin()) return false;
    if (navigateSkin('right')) return true;
    return toolbarFocus();
  }

  function novaLeft() {
    if (!inSkin()) return false;
    if (navigateSkin('left')) return true;
    return false;
  }
  ...
  var expanding = false;
  var expand_key = '';
  var expand_ready_key = '';
  var expand_timer = null;

  function tvDevice() {
    try {
      var ua = String(navigator.userAgent || '');
      if (/(smart-tv|smarttv|android tv|googletv|bravia|aft[a-z]*|shield|mitv|mi tv|hbbtv)/i.test(ua)) return true;
      return /Android/i.test(ua) && !('ontouchstart' in window) && window.innerWidth >= 900;
    } catch (e) {
      return false;
    }
  }
  ...
  function forceAllOn() {
    return tvDevice() && get('nova_skin_force_all', true) !== false;
  }

  function expandStamp() {
    return [
      movie ? movie.id : 0,
      currentSourceKey(),
      sourceTitle(),
  ...
  function nativeTorrentNodes() {
    if (!host) return [];
    var out = [];
    $(host).find('.torrent-item').each(function () {
      if (!$(this).closest('.nova-skin-root').length) out.push(this);
    });
    return out;
  }

  /*
   * Android TV Lampa can virtualize the native torrent list and initially
   * put only the two visible rows into the DOM. Nova Skin used to hide those
   * rows immediately, which stopped Lampa from ever creating the remaining
   * results. Prime the native list first; every scroll/update gives Lampa a
   * chance to append the next rows.
   */
  function expandNativeList(key) {
    if (!forceAllOn() || !key) return false;
    if (expand_ready_key === key) return false;
    if (expanding) return true;

    var wantedHost = host;
    var wantedMovie = movie;
    var rounds = 0;
    var stable = 0;
    var previous = nativeTorrentNodes().length;
    expanding = true;
    expand_key = key;
    ...
    var finish = function () {
      ...
      expand_timer = null;
      expanding = false;
      if (host === wantedHost && movie === wantedMovie) {
        expand_ready_key = key;
        signature = '';
        schedule();
      }
    };
    ...
    var pump = function () {
      if (host !== wantedHost || movie !== wantedMovie || !inSkin()) {
        expanding = false;
        return;
      }

      var nodes = nativeTorrentNodes();
      var tail = nodes.length ? nodes[nodes.length - 1] : null;
      try {
      ...
      try {
        ...
        var box = $(host).closest('.scroll').first();
        if (box.length) box[0].scrollTop = box[0].scrollHeight;
      } catch (e) {}

      if (tail) {
        var scroll = activeScroll(tail);
        if (scroll) {
          try { scroll.update($(tail), true); } catch (e) {}
        }
      }

      expand_timer = setTimeout(function () {
        var now = nativeTorrentNodes().length;
        ...
        rounds++;
        if (rounds < 24 && stable < 3) return pump();
        finish();
      }, 90);
    };
  ...
  function draw() {
    ...
    if (expandNativeList(expandStamp())) return;
    ...
    busy = false;
    relayout();
    if (host && $(host).find('.torrent-item').length === 0) probeSchedule();
  }
  ...
  function settings() {
    try {
      ...
      Lampa.SettingsApi.addParam({
        component: 'nova_skin',
        param: { name: 'nova_skin_force_all', type: 'trigger', default: true },
        field: {
          name: label('nova_skin_set_all'),
          description: label('nova_skin_set_all_descr')
        },
        onChange: function () { redraw(); }
      });
  ...
  EXTRA_CSS += '.nova-skin-root .nova-hero__progress{height:.45em!important;background:rgba(255,255,255,.3)!important;border:0!important;-webkit-box-shadow:none!important;box-shadow:none!important}' +
    '.nova-skin-root .nova-hero__progress .time-line>div{background:#ff2d20!important}' +
    '.nova-skin-root .nova-card__line{height:.35em!important;background:rgba(255,255,255,.22)!important}' +
    '.nova-skin-root .nova-card__line .time-line>div,' +
    '.nova-skin-root .nova-card.focus .nova-card__line--body .time-line>div,' +
    '.nova-skin-root .nova-card--slim.focus .nova-card__line .time-line>div{background:#ff2d20!important}';
