(function () {
  ...
  var TS_ROW_VERSION = 'no-source-tracker-v3';
  ...
  function cleanupTorrentStyles() {
    try {
      ...
      console.error(config.name, 'cleanup error:', e);
    }
  }
  ...
  function tsCompatibility(text) {
    ...
    if (hasHdr || hasHardAudio) {
      return {
        label: 'Може не грати',
        cls: 'ts-compat-bad',
        reason: hasHdr ? 'HDR/Dolby Vision/10-bit часто не підтримується системними плеєрами' : 'DTS/TrueHD/Atmos/FLAC/EAC3 часто не підтримується системними плеєрами'
      };
    }
    ...
    if (isH265 || is2160) {
      return {
        label: 'Ризик',
        cls: 'ts-compat-warn',
        reason: isH265 ? 'H.265/HEVC може не програватися на частині TV' : '2160p може бути важким для системного плеєра'
      };
    }
    ...
    if ((isH264 && isSdr) || (isSdr && !isH265)) {
      return {
        label: 'Сумісно',
        cls: 'ts-compat-good',
        reason: 'SDR/H.264 або простий SDR зазвичай сумісний із системними плеєрами'
      };
    }
  ...
  function tsParseOnlineMeta(card, sourceText) {
    ...
    var trackerMatch = full.match(/\b(toloka|mazepa|rutracker|kinozal|nnm|selezen|megapeer|lostfilm|anilibria)\b/i);
  ...
  function tsAdaptOnlineTorrentCards() {
    ...
    row.className = 'ts-online-torrent-row';
      ...
      console.error(config.name, 'online torrent adapter error:', e);
    }
  }
  ...
  function tsAdaptRegularOnlineCards() {
    try {
      ...
      console.error(config.name, 'online source adapter error:', e);
    }
  }
  ...
  function registerSettings() {
    try {
      ...
      Lampa.SettingsApi.addParam({
        ...
        field: { name: 'Стиль видачі онлайн/торентів' },
        onChange: function () {
      ...
      console.error(config.name, 'settings error:', e);
      return false;
    }
  }
