(function () {
    ...
    // Если в localStorage уже сохранена тема – применяем её
    ...
    Lampa.SettingsApi.addParam({
        ...
        onRender: function (item) {
            setTimeout(() => {
                ...
                item.on('hover:enter', () => {
                    setTimeout(() => {
                        // Если открыты другие настройки – возвращаемся назад
    ...
    function ThemesComponent(options) {
        const request = new Lampa.Reguest();       // для загрузки JSON категории
        ...
        let infoBlock;                              // блок с информацией о выделенной теме
        let currentFocusedCard;                      // текущая фокусируемая карточка
        ...
        this.create = function () {
            ...
            request.silent(
                options.url,                         // URL загружаемой категории
        ...
        this.append = function (themes) {
            themes.forEach(theme => {
                ...
                // Метка "Установлена", если текущая тема совпадает
        ...
        // Строим интерфейс после загрузки данных
        this.build = function (data) {
            ...
            this.append(data);                 // добавляем карточки
        ...
        function installTheme(cssUrl, card) {
            // Удаляем все ранее установленные темы (по классу)
            ...
            // Убираем старые метки и добавляем новую
            $('.card__quality').remove();
            addInstalledLabel(card);

            // Сохраняем и сбрасываем другие настройки (фон, стекло, чёрный стиль)
    ...
    // Автозапуск после готовности приложения
    if (window.appready) {
        // всё уже готово
    } else {
