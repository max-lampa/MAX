(function() {
    ...
    function applyTheme(css) {
        ...
        if (css) $('<style id="flora-style-tag">').html(css).appendTo('head');
    }
    ...
    Lampa.SettingsApi.addParam({
        component: 'interface',
        param: { name: 'flora_themes_btn', type: 'static' },
        field: { name: 'Темы Flora', description: 'Выбрать цветовую схему интерфейса' },
        onRender: function(item) {
            setTimeout(() => {
                const button = item.node || $('.settings-param[data-name="flora_themes_btn"]');
                if (button.length) {
                    button.insertAfter($('.settings-param[data-name="interface_size"]'));
                    button.off('hover:enter').on('hover:enter', () => {
                        Lampa.Activity.push({
                            component: 'flora_browser',
    ...
    Lampa.Component.add('flora_browser', function() {
        let scroll = new Lampa.Scroll({ mask: true, over: true });
        let container = $('<div class="flora-list"></div>');
        let items = [];
        let last_focus;
