(function () { 
    ...
    // ─── Модифицированные Хелперы (Без авторизации) ─────────────────────
    function account(url) {
        // Убрали получение токенов из Storage, возвращаем URL как есть
        return url;
    }
    ...
    function apiUrl(path) {
        ...
        return Manifest.api_host + normalizedPath;
    }
    ...
    function YouTubeFeed(object) {
        ...
        var controller_head = {
            toggle: function () {
                ...
                var target = (last_tab && $.contains(document.documentElement, last_tab)) ? last_tab : tabButtons[activeTab][0];
                Lampa.Controller.collectionFocus(target, head);
            },
            left: function () { if (Navigator.canmove('left')) Navigator.move('left'); else Lampa.Controller.toggle('menu'); },
            right: function () { if (Navigator.canmove('right')) Navigator.move('right'); },
            up: function () {},
            down: function () { if (body.find('.selector').length > 0) Lampa.Controller.toggle('content'); },
        ...
        var controller_content = {
            toggle: function () {
                active_zone = 'content';
                var target = (last_card && $.contains(document.documentElement, last_card)) ? last_card : body.find('.selector').eq(0)[0];
                if (target) {
        ...
        function resetBody() { body.empty(); network.clear(); last_card = null; }
        ...
        function loadHome() {
            ...
            network.silent(apiUrl('/lite/youtube/feed'), function (data) {
                ...
                if (!categories.length) { showEmpty('Нет данных'); activateContent(); return; }
                categories.forEach(function (cat) { buildRow(cat.title, cat.results || []); });
                activateContent();
            }, function () {
                object.activity.loader(false);
        ...
        function openSearch() {
            Lampa.Input.edit({ title: 'Поиск YouTube', value: lastSearch, free: true, nosave: true }, function (value) {
                if (value && value.trim()) {
        ...
        function doSearch(query) {
            ...
            network.silent(apiUrl(path), function (data) {
                object.activity.loader(false);
                var results = [];
                (data.categories || []).forEach(function (cat) { results = results.concat(cat.results || []); });
                if (!results.length) { showEmpty('Ничего не найдено'); activateContent(); return; }
        ...
        function buildRow(title, rowItems) { buildGrid(title, rowItems); }

        function buildGrid(title, gridItems) {
            if (title) body.append($('<div class="items-line__head"><div class="items-line__title">' + escapeHtml(title) + '</div></div>'));
            var grid = $('<div class="category-full"></div>');
            gridItems.forEach(function (item) { grid.append(makeVideoCard(item)); });
            body.append(grid);
        }
        ...
        function makeVideoCard(item) {
            ...
            if (item.channel) card.find('.card__age').text(item.channel).addClass('ytfeed-channel');
            if (item.duration) card.find('.card__view').append('<div class="card__type">' + escapeHtml(item.duration) + '</div>');
        ...
        function openVideo(item) {
            ...
            if (!videoID) return;
            Lampa.Loading.start();
            var path = '/lite/youtube?videoID=' + encodeURIComponent(videoID);
            $.ajax({
                url: apiUrl(path),
                dataType: 'json',
                success: function (data) {
                    Lampa.Loading.stop();
                    if (data && data.method === 'play') Lampa.Player.play(data);
                    else Lampa.Noty.show(data.error || 'Ошибка видео');
                },
        ...
        function activateContent() { scroll.update(body); Lampa.Controller.toggle(body.find('.selector').length > 0 ? 'content' : 'head'); }
    ...
    function addMenuButton() {
        ...
        var button = $('<li class="menu__item selector" data-action="youtube_feed"><div class="menu__ico"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg></div><div class="menu__text">YouTube</div></li>');
        button.on('hover:enter', function () {
            Lampa.Activity.push({ title: 'YouTube', component: 'youtube_feed', page: 1 });
        });
