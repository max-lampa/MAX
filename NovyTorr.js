(function () {
  ...
  if (window.nova_torrents && window.nova_torrents_version === '8-android-tv') return;
  window.nova_torrents = true;
  window.nova_torrents_version = '8-android-tv';
  ...
  function dateStamp(value) {
    ...
    if (!text) return 0;
    /* TMDB returns ISO dates.  Check them before DD.MM.YYYY: the old
       order read 2026-09-01 as 26-09-01 and moved calendar entries to
       the wrong year. */
    var iso = text.match(/(?:^|\D)(\d{4})-(\d{1,2})-(\d{1,2})(?:\D|$)/);
    if (iso) {
      var iso_year = parseInt(iso[1], 10) || 0;
      var iso_month = parseInt(iso[2], 10) || 0;
      var iso_day = parseInt(iso[3], 10) || 0;
      if (iso_month >= 1 && iso_month <= 12 && iso_day >= 1 && iso_day <= 31) {
        var iso_stamp = Date.UTC(iso_year, iso_month - 1, iso_day);
        return isNaN(iso_stamp) ? 0 : iso_stamp;
      }
    }
    var dmy = text.match(/(?:^|\D)(\d{1,2})[.\/-](\d{1,2})[.\/-](\d{2,4})(?:\D|$)/);
    if (dmy) {
      var year = parseInt(dmy[3], 10) || 0;
      if (year < 100) year += 2000;
      var month = parseInt(dmy[2], 10) || 0;
      var day = parseInt(dmy[1], 10) || 0;
  ...
  /* ------------------------- Android TV input ------------------------ *
   * Lampa's controller differs slightly between builds.  Android TV 9
   * also sends numeric key codes on some remotes, so keep a small,
   * self-contained spatial navigator as a reliable fallback. */

  function dpadCode(event) {
    var key = String(event && event.key || '').toLowerCase();
    var code = event && (event.keyCode || event.which) || 0;
    if (key === 'arrowup' || key === 'dpad_up' || code === 19 || code === 38) return 'up';
    if (key === 'arrowdown' || key === 'dpad_down' || code === 20 || code === 40) return 'down';
    if (key === 'arrowleft' || key === 'dpad_left' || code === 21 || code === 37) return 'left';
    if (key === 'arrowright' || key === 'dpad_right' || code === 22 || code === 39) return 'right';
    if (key === 'enter' || key === 'select' || key === 'dpad_center' || code === 13 || code === 23 || code === 66 || code === 160) return 'enter';
    if (key === 'escape' || key === 'back' || key === 'browserback' || key === 'go back' ||
      code === 4 || code === 8 || code === 27 || code === 461 || code === 10009) return 'back';
    return '';
  }

  function visibleSelectors(root) {
    var nodes = [];
    if (!root || !root.length) return nodes;
    root.find('.selector').each(function () {
      var node = $(this);
      if (!node.length || node.hasClass('nova-tor-hidden') || node.attr('aria-hidden') === 'true') return;
      /* jQuery :visible is unreliable in the old WebView used by some
         Android TV 9 firmware.  Check the computed style and rectangle
         directly instead. */
      try {
        var style = window.getComputedStyle(this);
        var rect = this.getBoundingClientRect();
        if (style.display === 'none' || style.visibility === 'hidden' ||
          !rect || (!rect.width && !rect.height)) return;
      } catch (e) {}
      /* Nested selectors confuse collectionFocus on older Lampa builds.
         Hero itself is deliberately not a selector; this also protects
         against third-party markup accidentally being included. */
      nodes.push(this);
    });
    return nodes;
  }
  ...
  function focusedSelector(root) {
    var current = root && root.length ? root.find('.selector.focus').first() : $();
    if (current && current.length) return current[0];
    if (last_focused && root && root.length) {
      try { if ($.contains(root[0], last_focused)) return last_focused; } catch (e) {}
    }
    return null;
  }
  ...
  function focusNode(node) {
    ...
    last_focused = node;
    try {
      var collection = ui.modal && ui.modal.length ? null : (host && host.length ? host[0] : activeScroll(node));
      Lampa.Controller.collectionFocus(node, collection);
    } catch (e) {}
    /* collectionFocus is asynchronous in some Lampa 0.x builds.  Keeping
       the class here makes focus visible immediately on Android TV 9. */
    try {
      $('.selector.focus').removeClass('focus');
  ...
  function refreshControllerCollection() {
    if (!host || !host.length || !window.Lampa || !Lampa.Controller) return;
    try { Lampa.Controller.collectionSet(host[0], false, true); } catch (e) {}
    try {
      var active = typeof Lampa.Controller.enabled === 'function' ? Lampa.Controller.enabled() : null;
      if (!active || active.name !== 'content') Lampa.Controller.enable('content');
    } catch (e) {}
  }

  function spatialTarget(root, current, direction) {
    var nodes = visibleSelectors(root);
    if (!nodes.length) return null;
    if (!current || nodes.indexOf(current) === -1) return nodes[0];
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      if (horizontal && Math.abs(dy) > Math.max(from.height, rect.height) * 2.4 && overlap <= 0) continue;
      if (!horizontal && Math.abs(dx) > Math.max(from.width, rect.width) * 2.4 && overlap <= 0) continue;

      var secondary = horizontal ? Math.abs(dy) : Math.abs(dx);
      /* Prefer an item in the same row/column, then the nearest item in
         the requested direction.  The large secondary multiplier prevents
         right/left from jumping to another wrapped row. */
      var score = primary + secondary * (overlap > 0 ? 1.8 : 4.5);
      if (overlap > 0) score -= Math.min(from.width, from.height) * 0.7;
      if (score < bestScore) {
        bestScore = score;
        best = candidate;
      }
  ...
  function navigateDpad(direction) {
    var root = ui.modal && ui.modal.length ? ui.modal : ui.root;
    if (!root || !root.length) return false;

    var current = focusedSelector(root);
    if (direction === 'enter') {
      if (!current) return false;
      try { $(current).trigger('hover:enter'); } catch (e) {}
      return true;
    }

    var next = spatialTarget(root, current, direction);
    if (next) {
      focusNode(next);
      return true;
    }
    /* A modal is a closed focus scope: do not let the underlying Lampa
       activity receive an arrow when the focus is at an edge. */
    return !!ui.modal;
  }

  function hookController() {
    if (controller_add_wrapped || !window.Lampa || !Lampa.Controller ||
      typeof Lampa.Controller.add !== 'function') return;
    controller_add_wrapped = true;
    var add = Lampa.Controller.add;

    Lampa.Controller.add = function (name, object) {
      if (name === 'content' && object && !object.nova_tor_hooked) {
        object.nova_tor_hooked = true;
        controller_hooked = true;
        var directions = ['up', 'down', 'left', 'right'];
        for (var i = 0; i < directions.length; i++) {
          (function (direction) {
            if (typeof object[direction] !== 'function') return;
            var original = object[direction];
            object[direction] = function () {
              if (navigateDpad(direction)) return;
    ...
    Lampa.Controller.nova_tor_add_wrapped = true;
  }

  function installDpadNavigation() {
    if (nav_bound) return;
    nav_bound = true;
    try {
      /* The controller hook below is the primary route, matching the
         proven Nova Skin approach.  This native listener remains a
         fallback for modal/older builds that never expose content.add. */
      document.addEventListener('keydown', function (event) {
        var direction = dpadCode(event);
        if (!direction) return;

        var root = ui.modal && ui.modal.length ? ui.modal : ui.root;
        if (!root || !root.length) return;
        if ((direction === 'up' || direction === 'down' || direction === 'left' || direction === 'right') && controller_hooked) return;
  ...
  var nav_bound = false;
  var controller_hooked = false;
  var controller_add_wrapped = false;

  function purgeLegacyNova() {
    var ids = ['nova-tor-css', 'nova-tor-css-v5', 'nova-tor-css-v6', 'nova-tor-css-v7'];
  ...
  function menu(title, list, current, onPick, back_key) {
    ...
    /* Do not use Lampa.Select here.  Its panel is narrow in several
       Android TV 9 builds and its focus scope can remain behind the
       torrent activity.  A native Nova full-screen panel gives the
       remote a single, predictable focus scope. */
    var modal = $('<div class="nova-tor-modal nova-tor-choice-modal"></div>');
    var panel = $('<div class="nova-tor-modal__panel nova-tor-choice"></div>');
    panel.append('<div class="nova-tor-modal__head"><div><div class="nova-tor-modal__eyebrow">' + esc(t('sort')) + '</div><div class="nova-tor-modal__title">' + esc(title) + '</div></div><div class="nova-tor-modal__close selector" data-key="choice-close">' + esc(t('close')) + '</div></div>');

    for (var i = 0; i < list.length; i++) {
      var row = $('<div class="nova-tor-modal__choice selector" data-key="choice-' + i + '"></div>');
      row.append('<span class="nova-tor-modal__choice-title">' + esc(list[i].title) + '</span>');
      if (String(list[i].value) === String(current)) row.addClass('nova-tor-modal__choice--selected');
      bind(row, function (entry) {
        return function () {
          closeEpisodeModal(false);
          onPick(entry.value);
        };
    ...
    modal.on('click', function (event) {
      if (event && event.target === modal[0]) closeEpisodeModal();
    });
    bind(modal.find('[data-key="choice-close"]'), function () {
      closeEpisodeModal();
      focusKeyed(back_key);
    });
    try { Lampa.Controller.collectionFocus(modal.find('[data-key="choice-close"]')[0], null); } catch (e) {}
    installDpadNavigation();
  }
  ...
  function tmdbId() {
    ...
    /* `movie.id` is often a Kinopoisk or an internal Lampa id.  Treating
       it as TMDB was the main source of wrong calendar data.  Only use
       fields that explicitly identify a TMDB id. */
    var keys = ['tmdb_id', 'tmdbId', 'id_tmdb', 'tmdb'];
    for (var i = 0; i < keys.length; i++) {
  ...
  function tmdbResultScore(result, title, year) {
    var name = clean(result && (result.name || result.original_name || ''));
    var wanted = clean(title).toLowerCase();
    var candidate = name.toLowerCase();
    var score_value = 0;
    if (candidate === wanted) score_value += 1000;
    else if (candidate.indexOf(wanted) === 0 || wanted.indexOf(candidate) === 0) score_value += 500;
    if (year) {
      var date = String(result && (result.first_air_date || '')).slice(0, 4);
      if (date === String(year)) score_value += 240;
      else if (date) score_value -= Math.min(120, Math.abs(parseInt(date, 10) - parseInt(year, 10)) * 20);
    }
    score_value += Math.min(100, parseFloat(result && result.vote_average) * 8 || 0);
    score_value += Math.min(80, parseInt(result && result.popularity, 10) || 0);
    return score_value;
  }

  function bestTmdbResult(list, title, year) {
    var best = null;
    var best_score = -Infinity;
    for (var i = 0; i < (list || []).length; i++) {
      if (!list[i] || !list[i].id) continue;
      var score_value = tmdbResultScore(list[i], title, year);
  ...
  function attachEpisodeMetadata(items, episodes) {
    var list = episodes || [];
    for (var x = 0; x < (items || []).length; x++) {
      var item = items[x];
      if (!item || !item.info || !item.info.episode) continue;
      item.tmdb_episodes = item.tmdb_episodes || {};
      for (var y = 0; y < list.length; y++) {
        var episode = list[y];
        var number = parseInt(episode && episode.episode_number, 10) || 0;
        var first = parseInt(item.info.episode, 10) || 0;
        var last = parseInt(item.info.episode_end, 10) || first;
        if (!number || number < first || number > Math.max(first, last)) continue;
        item.tmdb_episodes[String(number)] = {
          name: episode.name || '',
          overview: episode.overview || '',
          air_date: episode.air_date || '',
          still_path: episode.still_path || episode.poster_path || ''
        };
      }
    }
  }
  ...
  function tmdbResolveId(done) {
    ...
    var cache_key = 'v2:' + title.toLowerCase() + ':' + tmdbSearchYear();
    ...
    tmdbRequest(url, function (json) {
      var result = bestTmdbResult(json && json.results, title, year);
      if (!result) {
        ...
        tmdbRequest(fallback, function (multi) {
          ...
          var tv = [];
          for (var i = 0; i < list.length; i++) {
            if (list[i].media_type === 'tv') tv.push(list[i]);
          }
          result = bestTmdbResult(tv, title, year);
    ...
    for (var season_key in seasons) {
      ...
      if (saved_season && saved_season.episodes) {
        tmdb_cache[season_key] = saved_season;
         attachEpisodeMetadata(seasons[season_key].items, saved_season.episodes);
      ...
      (function (cache_key, season_no, season_items) {
        ...
        tmdbRequest(url, function (json) {
          ...
          if (json && json.episodes) localCacheSet('nova_tor_tmdb_' + cache_key + '_' + tmdbLanguage(), json);
           ...
           attachEpisodeMetadata(season_items, episodes);
  ...
  function premiereDate(item) {
    ...
    try {
       /* Construct a local date from the ISO parts.  new Date('YYYY-MM-DD')
          is UTC and shifts one day backwards on some TV time zones. */
       var date = new Date(parseInt(exact[1], 10), parseInt(exact[2], 10) - 1, parseInt(exact[3], 10));
  ...
  function calendarEntries() {
    ...
    for (var i = 0; i < list.length; i++) {
      var rows = episodeRows(list[i]);
      for (var x = 0; x < rows.length; x++) {
        var row = rows[x];
        if (!row.tmdb_air_date) continue;
        var episode_key = row.info.season && row.info.episode
          ? String(row.info.season) + ':' + String(row.info.episode)
          : 'item:' + String(row.index);
        if (!unique[episode_key] || row.score > unique[episode_key].score) unique[episode_key] = row;
      }
    }
    var out = [];
    for (var key in unique) {
      if (Object.prototype.hasOwnProperty.call(unique, key)) out.push(unique[key]);
    }
    out.sort(function (a, b) {
      return dateStamp(a.tmdb_air_date) - dateStamp(b.tmdb_air_date) ||
        (a.info.season - b.info.season) || (a.info.episode - b.info.episode) ||
        a.index - b.index;
    });
    return out;
  }
   ...
   function openCalendarModal(reopen) {
    ...
    for (var e = 0; e < entries.length; e++) {
      ...
      row.append('<div class="nova-tor-modal__ep">' + esc(episodeLabel(item)) + '</div>');
        row.append('<div class="nova-tor-modal__rowbody"><div class="nova-tor-modal__rowtitle">' + esc(item.tmdb_name || item.title) + '</div><div class="nova-tor-modal__rowmeta">' + esc(t('aired')) + ': ' + esc(premiereDate(item)) + ' · ' + metaLine(item) + '</div></div>');
     ...
     if (item && item.info && item.info.season && start && end > start) {
       ...
       for (var episode = start; episode <= end; episode++) {
         ...
         copy.info.episode_end = episode;
          var episode_meta = item.tmdb_episodes && item.tmdb_episodes[String(episode)];
          if (episode_meta) {
            copy.tmdb_name = episode_meta.name || '';
            copy.tmdb_overview = episode_meta.overview || '';
            copy.tmdb_air_date = episode_meta.air_date || '';
            if (episode_meta.still_path) copy.image = tmdbImage(episode_meta.still_path, 'w300');
          } else {
            copy.tmdb_name = '';
            copy.tmdb_overview = '';
            copy.tmdb_air_date = '';
  ...
  function detach() {
    ...
    if (page_info && page_info.root) {
      try { page_info.root.removeClass('nova-tor-scope nova-tor-fullscreen'); } catch (e) {}
    }
    unwatch();
    host = null;
    natives = [];
  ...
  function draw(force) {
    ...
    data = evaluate(raw);
    try { if (page_info && page_info.root) page_info.root.addClass('nova-tor-scope nova-tor-fullscreen'); } catch (e) {}
    render(null);
    enrichEpisodePosters();
  }
  ...
  var CSS = '' +
    ...
    '.nova-tor-modal__rowbody{min-width:0;flex:1}.nova-tor-modal__rowtitle{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.nova-tor-modal__rowmeta{font-size:.82em;opacity:.58;margin-top:.25em}.nova-tor-modal__score{font-variant-numeric:tabular-nums;opacity:.7}' +
     '.nova-tor-choice{padding-left:2em;padding-right:2em;padding-left:clamp(2em,8vw,8em);padding-right:clamp(2em,8vw,8em)}' +
     '.nova-tor-modal__choice{display:-webkit-box;display:-webkit-flex;display:flex;-webkit-box-align:center;-webkit-align-items:center;align-items:center;min-height:3.25em;padding:.85em 1.1em;margin-bottom:.55em;border-radius:.7em;background:rgba(255,255,255,.06);font-size:1.15em}' +
     '.nova-tor-modal__choice:before{content:"";width:.55em;height:.55em;margin-right:.8em;border-radius:50%;background:rgba(255,255,255,.25);-webkit-flex-shrink:0;flex-shrink:0}' +
     '.nova-tor-modal__choice--selected:before{background:#8fe0a4;box-shadow:0 0 .6em #8fe0a4}' +
     '.nova-tor-modal__choice.focus{background:#f4f2ed;color:#171b25;box-shadow:0 0 0 .14em rgba(224,91,98,.9),0 .7em 1.8em rgba(0,0,0,.24)}' +
     '.nova-tor-modal__choice.focus:before{background:#e05b62;box-shadow:none}' +
    ...
    '.nova-tor-modal__panel{scrollbar-color:#e05b62 #252b36;scrollbar-width:thin}' +
     ...
     '.nova-tor{display:block;width:100%;max-width:none;padding:0 0 2em 0;position:relative}' +
    '.nova-tor *{-webkit-box-sizing:border-box;box-sizing:border-box}' +
     '.nova-tor-fullscreen .explorer__left{display:none!important}.nova-tor-fullscreen .explorer__files{width:100%!important;left:0!important}' +
  ...
  function addCSS() {
    if (document.getElementById('nova-tor-css-v7')) return;
    var style = document.createElement('style');
    style.id = 'nova-tor-css-v7';
