(function () {
  ...
  var I18N = {
    ru: {
      ...
      set_about_desc: 'Версия 0.2.0 • Автор llowmikee',
    ...
    en: {
      ...
      set_about_desc: 'Version 0.2.0 • Author llowmikee',
    ...
    uk: {
      ...
      set_about_desc: 'Версія 0.2.0 • Автор llowmikee',
  ...
  function resetCardSwitches() {
    qsa('.card[data-nfx-switched]').forEach(function (c) {
      ...
      var posterLogo = c.querySelector('.nfx-card-poster-logo');
      if (posterLogo) posterLogo.remove();
    });
  }
  ...
  function injectStyle() {
    ...
    style.textContent = [
      ...
      '  box-shadow: none !important;',
      '}',
      'body.' + BODY_CLASS + ' .agnative-topnav-shell { position:absolute; left:50%; top:.38em; transform:translateX(-50%); z-index:20; width:max-content; max-width:calc(100vw - 20em); display:inline-flex; align-items:center; }',
      'body.' + BODY_CLASS + ' .agnative-topnav-shell__inner { display:inline-flex; align-items:center; gap:.22em; padding:.28em .42em; border-radius:16px; background:rgba(10,12,18,.82); border:2px solid rgba(255,255,255,.18); box-shadow:0 4px 32px rgba(0,0,0,.55), inset 0 1px 0 rgba(255,255,255,.12); backdrop-filter:blur(24px) saturate(160%); -webkit-backdrop-filter:blur(24px) saturate(160%); }',
      'body.' + BODY_CLASS + ' .agnative-topnav-shell__items { display:flex; align-items:center; justify-content:center; gap:.14em; }',
      'body.' + BODY_CLASS + ' .agnative-topnav-shell__right { display:flex; align-items:center; gap:.14em; margin-left:.18em; padding-left:.22em; border-left:1px solid rgba(255,255,255,.14); }',
      'body.' + BODY_CLASS + ' .agnative-topnav-shell__item.selector { appearance:none; -webkit-appearance:none; border:2px solid transparent; background:rgba(255,255,255,.07); color:rgba(255,255,255,.9); height:2.6em; display:inline-flex; align-items:center; justify-content:center; text-align:center; padding:0 1.1em; border-radius:10px; font-size:.9em; font-weight:700; line-height:1; white-space:nowrap; transition:background .15s ease, transform .15s ease, box-shadow .15s ease, border-color .15s ease; letter-spacing:.02em; text-transform:uppercase; }',
      'body.' + BODY_CLASS + ' .agnative-topnav-shell__item--icon.selector { width:2.6em; min-width:2.6em; padding:0; }',
      'body.' + BODY_CLASS + ' .agnative-topnav-shell__item--icon svg { width:1.2em; height:1.2em; }',
      'body.' + BODY_CLASS + ' .agnative-topnav-shell__item.is-active { background:rgba(86,141,255,.22) !important; border-color:rgba(86,141,255,.5) !important; color:#fff !important; }',
      'body.' + BODY_CLASS + ' .agnative-topnav-shell__item.hover { background:rgba(255,255,255,.14) !important; border-color:rgba(255,255,255,.25) !important; }',
      'body.' + BODY_CLASS + ' .agnative-topnav-shell__item.focus { background:rgba(86,141,255,.85) !important; border-color:#568dff !important; color:#fff !important; box-shadow:0 0 0 3px rgba(86,141,255,.45), 0 4px 20px rgba(86,141,255,.4) !important; transform:scale(1.06) !important; }',
      'body.' + BODY_CLASS + ' .agnative-topnav-clock { position:absolute; right:1em; top:.38em; z-index:20; display:inline-flex; align-items:center; justify-content:center; min-width:4.5em; height:2.96em; padding:0 1em; border-radius:12px; background:rgba(10,12,18,.82); border:2px solid rgba(255,255,255,.18); box-shadow:0 4px 22px rgba(0,0,0,.45); backdrop-filter:blur(24px) saturate(160%); -webkit-backdrop-filter:blur(24px) saturate(160%); color:rgba(255,255,255,.95); font-size:.98em; font-weight:800; letter-spacing:.03em; }',
      ...
      'body.' + BODY_CLASS + '[' + BACKDROP_ATTR + '="off"] .nfx-card-overlay { display:none !important; }',
      'body.' + BODY_CLASS + '[' + BACKDROP_ATTR + '="off"] .nfx-card-rating { display:none !important; }',
      'body.' + BODY_CLASS + '[' + BACKDROP_ATTR + '="off"] .nfx-card-logo { display:none !important; }',
      'body.' + BODY_CLASS + '[' + BACKDROP_ATTR + '="off"] .card__title { display:block !important; }',
      'body.' + BODY_CLASS + '[' + BACKDROP_ATTR + '="off"] .nfx-card-poster-logo { display:flex !important; }',
      'body.' + BODY_CLASS + '[' + BACKDROP_ATTR + '="on"] .nfx-card-poster-logo { display:none !important; }',
      '.nfx-card-poster-logo { display:none; position:absolute; left:0; right:0; bottom:0; z-index:3; align-items:flex-end; justify-content:center; padding:.6em .6em .85em; background:linear-gradient(0deg,rgba(4,6,12,.92) 0%,rgba(4,6,12,.65) 42%,rgba(4,6,12,.1) 78%,rgba(4,6,12,0) 100%); border-radius:0 0 1.35em 1.35em; pointer-events:none; min-height:4em; }',
      '.nfx-card-poster-logo img { max-height:2.2em; max-width:78%; object-fit:contain; filter:drop-shadow(0 2px 8px rgba(0,0,0,.7)); }',
      '.nfx-card-poster-logo .nfx-card-poster-logo__title { color:#fff; font-size:.9em; font-weight:800; text-align:center; text-shadow:0 2px 10px rgba(0,0,0,.7); max-width:90%; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }',
  ...
  function bindAction(btn, fn) {
    ...
    btn.addEventListener('keydown', function (e) {
      if (e.keyCode === 13 || e.keyCode === 23 || e.keyCode === 66 ||
          e.key === 'Enter' || e.key === ' ' || e.key === 'DPadCenter') {
        run(e);
      }
    });
  ...
  function registerTopnavController(shell) {
    ...
    try {
      var items = shell.querySelectorAll('.agnative-topnav-shell__item.selector');
      items.forEach(function (item, idx) {
        item.setAttribute('data-nav-idx', String(idx));
      });

      Lampa.Controller.collectionSet($(shell));

      try {
        Lampa.Controller.collectionFocus($(shell.querySelector('.agnative-topnav-shell__item.is-active') || items[0]), $(shell));
      } catch (e) { }

      try {
        $(shell).on('hover:focus hover:hover', function (e) {
          var target = e.target || e.currentTarget;
          if (target && target.classList) target.classList.add('focus');
        });
        $(shell).on('hover:blur hover:out', function (e) {
          var target = e.target || e.currentTarget;
          if (target && target.classList) target.classList.remove('focus');
        });
      } catch (e) { }
    } catch (e) { }
  }
  ...
  function switchCardToBackdrop(cardEl) {
    ...
    if (!useBackdrop) {
      var viewPoster = cardEl.querySelector('.card__view');
      if (viewPoster && !viewPoster.querySelector('.nfx-card-poster-logo')) {
        var posterLogoWrap = document.createElement('div');
        posterLogoWrap.className = 'nfx-card-poster-logo';

        var tmdbTypePoster = data.name ? 'tv' : 'movie';
        var titlePoster = data.title || data.name || '';

        fetchLogo(data.id, tmdbTypePoster, function (logo) {
          if (logo) {
            var imgP = document.createElement('img');
            imgP.src = logoImgUrl(logo.path);
            imgP.alt = titlePoster;
            imgP.loading = 'lazy';
            imgP.onerror = function () {
              var titleEl2 = document.createElement('div');
              titleEl2.className = 'nfx-card-poster-logo__title';
              titleEl2.textContent = titlePoster;
              posterLogoWrap.innerHTML = '';
              posterLogoWrap.appendChild(titleEl2);
            };
            posterLogoWrap.appendChild(imgP);
          } else if (titlePoster) {
            var titleEl = document.createElement('div');
            titleEl.className = 'nfx-card-poster-logo__title';
            titleEl.textContent = titlePoster;
            posterLogoWrap.appendChild(titleEl);
          }
        });

        viewPoster.appendChild(posterLogoWrap);
      }
      return;
    }
